const supportedSections = new Set(["new-group", "news", "signals", "settings", "target-presets"]);
...
export function normalizeWorkspaceState(section, value = {}) {
  ...
  if (section === "target-presets") {
    const presets = Array.isArray(value.presets) ? value.presets : [];
    return {
      presets: presets.slice(0, 50).map(preset => ({
        id: text(preset?.id || crypto.randomUUID(), 40),
        name: text(preset?.name, 100) || "未命名",
        targets: uniqueStrings(preset?.targets, 200, 100)
      })).filter(preset => preset.name && preset.targets.length > 0)
    };
  }
  ...
  return {
    ...
    telegramPublishMode: oneOf(value.telegramPublishMode, ["bot", "user"], "bot"),
    telegramForwardMode: oneOf(value.telegramForwardMode, ["bot", "user"], "bot"),
    telegramPublishUserId: text(value.telegramPublishUserId, 64),
    telegramForwardUserId: text(value.telegramForwardUserId, 64)
  };
}
