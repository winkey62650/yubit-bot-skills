...
function assertImageAsset(asset) {
  if (!asset || asset.mimeType !== "image/png" || asset.width !== 1200 || asset.height !== 675
      || !Number.isSafeInteger(asset.byteLength) || asset.byteLength < 45
...
function assertHealth(bundle, tier) {
  ...
  if (!health || !pageStates.has(health.page) || !imageStates.has(health.image)
  ...
  if (bundle.status === "verified") {
    const expectedPage = tier === "secondary" ? "not-applicable" : "ok";
    if (health.page !== expectedPage || health.image !== "ok" || !validInstant(health.checkedAt) || health.error !== undefined) {
      throw new Error("Verified market publication health violates its status invariant.");
    }
  }
  if (bundle.status === "draft" && (health.image === "ok" || health.page === "ok" || health.page === "not-applicable")) {
    throw new Error("Draft market publication health violates its status invariant.");
  }
  if (health.error !== undefined) {
    if (!health.error || !["page", "image"].includes(health.error.stage)
        || typeof health.error.name !== "string" || typeof health.error.message !== "string"
...
function assertBundle(bundle, identity) {
  ...
  if (!bundle || typeof bundle !== "object" || fields.some((field) => !Object.hasOwn(bundle, field))
  ...
  canonicalJson(bundle.sourceManifest);
  if (contentDigest(bundle.article, bundle.communityDocument, bundle.posterModel, bundle.sourceManifest) !== bundle.contentHash) {
...
function validateResponseBoundary(response, expected, allowlist) {
  ...
  if (plainText(response.url)) {
    let actual;
    try { actual = new URL(response.url); } catch { throw new Error("Publication response URL is invalid."); }
    if (actual.username || actual.password || actual.protocol !== "https:" || isPrivateHostname(actual.hostname)
        || !allowlist.has(actual.origin) || actual.origin !== expected.origin
...
function pngDimensions(bytes) {
  ...
  if (!dimensions || !sawIdat || !sawIend) throw new Error("Editorial PNG is incomplete and requires IHDR, IDAT and IEND chunks.");
  return dimensions;
}
...
function hashMarkerMatches(html, hash) {
  ...
  return [...visibleMarkup.matchAll(/<article\b[^>]*>/gi)].some((match) => {
    const tag = match[0];
    let cursor = "<article".length;
    while (cursor < tag.length - 1) {
      while (/\s/.test(tag[cursor])) cursor += 1;
      if (tag[cursor] === "/" || tag[cursor] === ">") break;
      const nameMatch = tag.slice(cursor).match(/^[A-Za-z_:][A-Za-z0-9_.:-]*/);
      ...
      if (name === "data-content-hash" && value === hash) return true;
    }
    return false;
  });
}
