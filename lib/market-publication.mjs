...
import { marketPosterCanvas } from "./market-poster-templates.mjs";
...
const MAX_EDITORIAL_PIXELS = 1080 * 1350;
...
function assertImageAsset(asset, expectedCanvas) {
  ...
  if (!isPlainObject(asset) || Object.keys(asset).length !== fields.length || fields.some((field) => !Object.hasOwn(asset, field))
      || asset.mimeType !== "image/png" || asset.width !== expectedCanvas.width || asset.height !== expectedCanvas.height
...
function assertBundle(bundle, identity) {
  ...
  if (bundle.status === "draft") {
    ...
    assertImageAsset(bundle.imageAsset, marketPosterCanvas(bundle.posterModel));
  }
  ...
  if ((renderedTime !== null && (renderedTime < createdTime || renderedTime > updatedTime))
      ...
      || (checkedTime !== null && renderedTime !== null && checkedTime < renderedTime)) {
...
function ipv6InCidr(parts, base, prefixLength) {
  const baseParts = ipv6Parts(base);
  if (!baseParts || !Number.isInteger(prefixLength) || prefixLength < 0 || prefixLength > 128) return false;
  const wholeParts = Math.floor(prefixLength / 16);
  for (let index = 0; index < wholeParts; index += 1) {
    if (parts[index] !== baseParts[index]) return false;
  }
  const remainingBits = prefixLength % 16;
  if (!remainingBits) return true;
  const mask = (0xffff << (16 - remainingBits)) & 0xffff;
  return (parts[wholeParts] & mask) === (baseParts[wholeParts] & mask);
}

// More-specific globally reachable assignments inside IANA's otherwise
// non-global 2001::/23 block remain eligible public origins.
const GLOBAL_IPV6_EXCEPTIONS = Object.freeze([
  ["2001:1::1", 128],
  ["2001:1::2", 128],
  ["2001:1::3", 128],
  ["2001:3::", 32],
...
// IANA special-purpose ranges unsuitable as public HTTP origins. NAT64 and
// ORCHID ranges are rejected conservatively even where IANA marks reachability.
...
function isPrivateHostname(hostname) {
  ...
  if (GLOBAL_IPV6_EXCEPTIONS.some(([base, prefixLength]) => ipv6InCidr(parts, base, prefixLength))) return false;
  return NON_GLOBAL_IPV6_CIDRS.some(([base, prefixLength]) => ipv6InCidr(parts, base, prefixLength));
}
...
function pngDimensions(bytes) {
  ...
  while (offset < bytes.byteLength) {
    ...
    if (!/[A-Z]/.test(type[2])) throw new Error("Editorial PNG chunk type has an invalid reserved bit.");
    if (/[A-Z]/.test(type[0]) && !PNG_CRITICAL_CHUNKS.has(type)) {
      throw new Error(`Editorial PNG contains unknown critical chunk ${type}.`);
    }
...
export async function captureEditorialImage({
  ...
  try {
    ...
    const dimensions = pngDimensions(bytes);
    const expectedCanvas = marketPosterCanvas(bundle.posterModel);
    if (dimensions.width !== expectedCanvas.width || dimensions.height !== expectedCanvas.height) {
      throw new Error(`Editorial image must be exactly ${expectedCanvas.width}×${expectedCanvas.height}; received ${dimensions.width}×${dimensions.height}.`);
    }
    const stamp = timestamp(now);
    const imageAsset = {
      mimeType: "image/png",
...
function hashMarkerMatches(html, hash) {
  ...
  while (cursor < html.length) {
    ...
    if (!tag.closing && namespace !== HTML) {
      const encoding = parent?.attributes.find((attribute) => attribute.name === "encoding")?.value?.toLowerCase();
      const integrationPoint = (namespace === SVG && svgIntegrationPoints.has(parent.name))
        || (namespace === MATHML && (
          (mathTextIntegrationPoints.has(parent.name) && !["mglyph", "malignmark"].includes(tag.name))
          || (parent.name === "annotation-xml" && ["text/html", "application/xhtml+xml"].includes(encoding))
        ));
      if (integrationPoint) namespace = HTML;
    }
    if (!tag.closing && namespace === HTML && tag.name === "svg") namespace = SVG;
    if (!tag.closing && namespace === HTML && tag.name === "math") namespace = MATHML;

    if (!tag.closing && namespace === HTML && tag.name === "plaintext") {
      cursor = html.length;
      break;
    }
    if (!tag.closing && namespace === HTML && rawElements.has(tag.name) && !tag.selfClosing) {
    ...
    if (tag.closing) {
      const closingElement = openElements.at(-1);
      if (!closingElement || closingElement.name !== tag.name
          || (closingElement.namespace === HTML && voidElements.has(tag.name))) return false;
      openElements.pop();
      if (closingElement.namespace !== HTML || tag.name !== "article") continue;
      if (articleDepth === 0) return false;
      if (markedDepth === articleDepth) markerClosed = true;
      articleDepth -= 1;
      continue;
    }
    const isVoid = namespace === HTML && voidElements.has(tag.name);
    if (!tag.selfClosing && !isVoid) openElements.push({ name: tag.name, namespace, attributes: tag.attributes });
    if (namespace !== HTML || tag.name !== "article") continue;
    articleDepth += 1;
