...
const UNFINISHED_VALUE = /\b(?:pending|awaiting|tbd|not\s+(?:available|published)|n\/?a|null)\b/i;

function completeValue(value) {
  return nonEmpty(value) && !UNFINISHED_VALUE.test(value);
}

function marketSymbol(value) {
  const text = String(typeof value === "object" && value !== null
    ? value.symbol ?? value.asset ?? value.market ?? value.label ?? ""
    : value ?? "").toUpperCase();
  const patterns = [
    ["BTC", /\b(?:BTC|BITCOIN)\b/],
    ["ETH", /\b(?:ETH|ETHEREUM)\b/],
    ["US2Y", /\b(?:US\s*2Y|2Y|TWO[- ]YEAR)\b/],
    ["DXY", /\b(?:DXY|DOLLAR INDEX|USD INDEX)\b/],
    ["NASDAQ", /\b(?:NASDAQ|NDX)\b/],
    ["SPX", /\b(?:SPX|S&P\s*500)\b/],
    ["SOL", /\b(?:SOL|SOLANA)\b/],
  ];
  return patterns.find(([, pattern]) => pattern.test(text))?.[0] ?? "";
}

function hasCompleteCrossAssetTape(values) {
  if (!Array.isArray(values) || values.length < 3 || values.some((value) => !completeValue(String(value ?? "")))) return false;
  const symbols = new Set(values.map(marketSymbol).filter(Boolean));
  return symbols.has("BTC") && [...symbols].filter((symbol) => symbol !== "BTC").length >= 2;
}

function weeklyEventDay(event) {
  const value = String(event?.timeUtc ?? event?.timeUtc8 ?? event?.date ?? "");
  return value.match(/^\d{4}-\d{2}-\d{2}/)?.[0] ?? value;
}

function selectedWeeklyEvents(events) {
  const byDay = new Map();
  for (const event of Array.isArray(events) ? events : []) {
    const importance = Number(event?.importance);
    if (!Number.isInteger(importance) || importance < 3 || !nonEmpty(event?.title)) continue;
    const day = weeklyEventDay(event);
    const current = byDay.get(day);
    const order = (left, right) => Number(right.importance) - Number(left.importance)
      || String(left.timeUtc ?? left.timeUtc8 ?? "").localeCompare(String(right.timeUtc ?? right.timeUtc8 ?? ""))
      || String(left.title).localeCompare(String(right.title));
    if (!current || order(event, current) < 0) byDay.set(day, event);
  }
  return [...byDay.values()].sort((left, right) => weeklyEventDay(left).localeCompare(weeklyEventDay(right))
    || String(left.timeUtc ?? left.timeUtc8 ?? "").localeCompare(String(right.timeUtc ?? right.timeUtc8 ?? "")));
}
...
function validateQuality(input, checkedAt) {
  ...
  if (input?.cta != null) reasons.push("Content product CTA is not allowed; inject destination CTA during delivery.");
  ...
  if (!intelligence || typeof intelligence !== "object") {
    ...
    if (input.product === "weekly-catalyst-calendar" && selectedWeeklyEvents(intelligence.events).length === 0) reasons.push("Weekly Catalysts requires at least one importance 3–5 event after the one-event-per-day selection gate.");
    if (input.product === "data-flash") {
      ...
      if (!completeValue(intelligence.release?.indicator)) reasons.push("Data Flash requires a named release indicator.");
      if (!validInstant(intelligence.release?.releaseTime)) reasons.push("Data Flash requires a canonical release time.");
      if (!hasCompleteCrossAssetTape(intelligence.release?.initialReaction)) reasons.push("Data Flash requires completed BTC plus two cross-asset reactions; pending values are not publishable.");
    }
    if (input.product === "market-follow-up") {
      ...
      if (!hasCompleteCrossAssetTape(intelligence.followUp?.marketMoves)) reasons.push("Market Follow-up requires completed BTC plus two cross-asset market moves; pending values are not publishable.");
    }
  }
  if (input?.product === "market-follow-up") {
    ...
    if (!validInstant(start) || !validInstant(end) || new Date(start) >= new Date(end)) {
      ...
      const durationMinutes = (Date.parse(end) - Date.parse(start)) / 60_000;
      if (durationMinutes < 10 || durationMinutes > 30) reasons.push("Market follow-up observation window must be 10–30 minutes.");
      if (Date.parse(end) > Date.parse(checkedAt)) reasons.push("Market follow-up observation window must be complete before publication.");
    }
...
function appendPlainProductBody(lines, input) {
  ...
  if (input.product === "daily-market-brief") {
    ...
    for (const item of selectedWeeklyEvents(intelligence.events)) {
...
function canonicalDocument(input) {
  const intelligence = input.intelligence ?? {};
  const lines = [
    PLAIN_PRODUCT_KICKERS[input.product] ?? "MARKET INTELLIGENCE",
    input.title,
...
const TELEGRAM_PRODUCT_ACCENTS = Object.freeze({
  "daily-market-brief": "📊",
  "weekly-catalyst-calendar": "🗓",
  "data-flash": "🚨",
  "market-follow-up": "",
...
const DISCORD_PRODUCT_ACCENTS = Object.freeze({
  "daily-market-brief": "📊",
  "weekly-catalyst-calendar": "🗓",
  "data-flash": "🚨",
  "market-follow-up": "",
...
const NATIVE_ASSET_GLYPHS = Object.freeze({
  BTC: "₿",
  ETH: "Ξ",
  SOL: "◎",
});

function decorateAssetEntry(value) {
  const text = String(value ?? "");
  if (/^[₿Ξ◎]\s/u.test(text)) return text;
  const match = text.match(/^(BTC|ETH|SOL)(?=$|[-/:\s])/i);
  if (!match) return text;
  return `${NATIVE_ASSET_GLYPHS[match[1].toUpperCase()]} ${text}`;
}

function decorateAssetEntries(values) {
  return (Array.isArray(values) ? values : []).map(decorateAssetEntry);
}

function telegramBias(value) {
  const direction = String(value ?? "");
  if (/bullish|positive|supportive/i.test(direction)) return { icon: "🟢", label: "Positive" };
  if (/bearish|negative|defensive/i.test(direction)) return { icon: "🔴", label: "Negative" };
...
function telegramEditorialChunks(record) {
  ...
  const accent = TELEGRAM_PRODUCT_ACCENTS[record.product] ?? "";
  const productTitle = [accent, kicker].filter(Boolean).join(" ");
  const intelligence = record.intelligence ?? {};
  const importance = Number(intelligence.importance) || 0;
  const bias = telegramBias(intelligence.bias?.direction);
  const metadata = `<b>${bias.icon} ${bias.label} · ${escapedTelegramText(decorateAssetEntry(intelligence.bias?.asset))}</b>  |  ${escapedTelegramText(intelligence.confidence)} confidence  |  Importance ${importance}/5`;
  const title = escapedTelegramText(record.title);
  const updated = escapedTelegramText(formattedUpdatedUtc(intelligence.updatedAt));
  const blocks = [
    `<b>${productTitle}</b>`,
    intelligence.previewLabel ? `<b>${escapedTelegramText(intelligence.previewLabel)}</b>` : "",
  ...
  if (record.product === "daily-market-brief") {
    for (const [index, item] of (intelligence.catalysts ?? []).entries()) {
      ...
      blocks.push(`<i>${escapedTelegramText(decorateAssetEntries(item.affectedAssets).join(" · "))}  |  ${escapedTelegramText(item.confidence)} confidence</i>`);
    }
  } else if (record.product === "weekly-catalyst-calendar") {
    for (const item of selectedWeeklyEvents(intelligence.events)) {
      const eventRail = [...decorateAssetEntries(item.markets), `${item.importance}/5`].filter(nonEmpty).join("  |  ");
    ...
    blocks.push(...wrappedTelegramBlocks(decorateAssetEntries(release.initialReaction).join(" · "), { prefix: "<b>02  ·  FIRST REACTION</b>\n" }));
    ...
    const followUp = intelligence.followUp ?? {};
    blocks.push(...wrappedTelegramBlocks(decorateAssetEntries(followUp.marketMoves).join(" · "), { prefix: "<b>01  ·  MEASURED MOVE</b>\n" }));
  ...
  blocks.push(
    ...wrappedTelegramBlocks(record.risk, { prefix: "<b>RISK / INVALIDATION</b>\n<b>Risk</b>  " }),
    ...wrappedTelegramBlocks(record.invalidation, { prefix: "<b>Invalidates</b>  " }),
  );
...
function discordEditorialChunks(record) {
  const kicker = TELEGRAM_PRODUCT_KICKERS[record.product] ?? "MARKET INTELLIGENCE";
  const accent = DISCORD_PRODUCT_ACCENTS[record.product] ?? "◆";
  const intelligence = record.intelligence ?? {};
  const importance = Number(intelligence.importance) || 0;
  const blocks = [
    `**${[accent, kicker].filter(Boolean).join(" ")}**`,
    ...
    `**${escapedDiscordText(decorateAssetEntry(intelligence.bias?.asset))} · ${escapedDiscordText(String(intelligence.bias?.direction ?? "").toUpperCase())}**  \\|  ${escapedDiscordText(String(intelligence.confidence ?? "").toUpperCase())} CONF.  \\|  ${"●".repeat(importance)}${"○".repeat(Math.max(0, 5 - importance))}`,
  ];
  if (record.product === "daily-market-brief") {
    for (const [index, item] of (intelligence.catalysts ?? []).entries()) {
      ...
      blocks.push(`*${escapedDiscordText(decorateAssetEntries(item.affectedAssets).join(" · "))}  \\|  ${escapedDiscordText(item.confidence)} confidence*`);
    }
  } else if (record.product === "weekly-catalyst-calendar") {
    for (const item of selectedWeeklyEvents(intelligence.events)) {
      const eventRail = [...decorateAssetEntries(item.markets), `${item.importance}/5`].filter(nonEmpty).join("  |  ");
    ...
    blocks.push(...wrappedDiscordBlocks(decorateAssetEntries(release.initialReaction).join(" · "), { prefix: "**02  ·  FIRST REACTION**\n" }));
    ...
    const followUp = intelligence.followUp ?? {};
    blocks.push(...wrappedDiscordBlocks(decorateAssetEntries(followUp.marketMoves).join(" · "), { prefix: "**01  ·  MEASURED MOVE**\n" }));
