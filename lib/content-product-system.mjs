...
function telegramEditorialChunks(record) {
  ...
  const metadata = `<b>BIAS</b>  ${escapedTelegramText(intelligence.bias?.asset)} · ${escapedTelegramText(intelligence.bias?.direction)}\n<b>HORIZON</b>  ${escapedTelegramText(intelligence.horizon)}\n<b>CONFIDENCE</b>  ${escapedTelegramText(intelligence.confidence)}\n<b>IMPORTANCE</b>  ${"●".repeat(Number(intelligence.importance) || 0)}${"○".repeat(Math.max(0, 5 - (Number(intelligence.importance) || 0)))}`;
  const blocks = [
    `<b>YUBIT ACADEMY · ${kicker}</b>\n<i>Market intelligence · ${escapedTelegramText(intelligence.updatedAt)}</i>`,
    ...wrappedTelegramBlocks(record.title, { prefix: "<b>", suffix: "</b>" }),
    metadata,
  ];
  if (record.product === "daily-market-brief") {
    for (const [index, item] of (intelligence.catalysts ?? []).entries()) {
      blocks.push(`<b>${String(index + 1).padStart(2, "0")} · ${escapedTelegramText(item.headline)}</b>`);
      ...
      blocks.push(`<b>IMPACT</b>  ${escapedTelegramText((item.affectedAssets ?? []).join(" · "))}\n<b>WINDOW</b>  ${escapedTelegramText(item.horizon)} · ${escapedTelegramText(item.confidence)}`);
      blocks.push(...wrappedTelegramBlocks(item.whatToWatch, { prefix: "<b>WATCH</b>\n" }));
    }
  } else if (record.product === "weekly-catalyst-calendar") {
    for (const item of intelligence.events ?? []) {
      blocks.push(`<b>${escapedTelegramText(item.timeUtc ?? item.timeUtc8)} · ${escapedTelegramText(item.title)}</b>\n<b>IMPORTANCE</b>  ${escapedTelegramText(`${item.importance}/5`)}\n<b>PREV / CONS / ACTUAL</b>  ${escapedTelegramText(`${item.previous} / ${item.forecast} / ${item.actual}`)}\n<b>MARKETS</b>  ${escapedTelegramText((item.markets ?? []).join(" · "))}`);
      blocks.push(...wrappedTelegramBlocks(item.whyItMatters, { prefix: "<b>WHY IT MATTERS</b>\n" }));
      blocks.push(...wrappedTelegramBlocks(item.scenarioMap, { prefix: "<b>SCENARIO MAP</b>\n" }));
    }
  } else if (record.product === "data-flash") {
    const release = intelligence.release ?? {};
    blocks.push(`<b>ACTUAL</b>  ${escapedTelegramText(release.actual)}\n<b>CONSENSUS</b>  ${escapedTelegramText(release.forecast)}\n<b>PREVIOUS</b>  ${escapedTelegramText(release.previous)}\n<b>SURPRISE</b>  ${escapedTelegramText(release.surprise)}`);
    blocks.push(...wrappedTelegramBlocks((release.initialReaction ?? []).join(" · "), { prefix: "<b>INITIAL REACTION</b>\n" }));
    blocks.push(...wrappedTelegramBlocks(release.deskView, { prefix: "<b>DESK VIEW</b>\n" }));
    blocks.push("<b>VERIFIED FACTS</b>");
    for (const fact of record.facts ?? []) blocks.push(...wrappedTelegramBlocks(fact.text, { prefix: "• " }));
  } else if (record.product === "market-follow-up") {
    const followUp = intelligence.followUp ?? {};
    blocks.push(...wrappedTelegramBlocks((followUp.marketMoves ?? []).join(" · "), { prefix: "<b>MEASURED MOVE</b>\n" }));
    blocks.push(...wrappedTelegramBlocks(followUp.interpretation, { prefix: "<b>INTERPRETATION</b>\n" }));
    blocks.push(...wrappedTelegramBlocks(followUp.confirmation, { prefix: "<b>CONFIRMS IF</b>\n" }));
    blocks.push(...wrappedTelegramBlocks(followUp.invalidation, { prefix: "<b>INVALIDATES IF</b>\n" }));
  }
  blocks.push(
    ...wrappedTelegramBlocks((intelligence.whatToWatch ?? []).map((item) => `• ${item}`).join("\n"), { prefix: "<b>WHAT TO WATCH</b>\n" }),
    ...wrappedTelegramBlocks(record.risk, { prefix: "<b>RISK BOUNDARY</b>\n" }),
