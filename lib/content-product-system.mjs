...
function telegramEditorialChunks(record) {
  ...
  const brandedKicker = [accent, kicker].filter(Boolean).join(" ");
  const intelligence = record.intelligence ?? {};
  const importance = Number(intelligence.importance) || 0;
  const bias = telegramBias(intelligence.bias?.direction);
  const metadata = `<b>${bias.icon} ${bias.label} · ${escapedTelegramText(intelligence.bias?.asset)}</b>  |  ${escapedTelegramText(intelligence.horizon)}  |  ${escapedTelegramText(intelligence.confidence)} confidence  |  Importance ${importance}/5`;
  const title = escapedTelegramText(record.title);
  const updated = escapedTelegramText(formattedUpdatedUtc(intelligence.updatedAt));
  const blocks = [
    `<b>YUBIT ACADEMY  ·  ${brandedKicker}</b>`,
