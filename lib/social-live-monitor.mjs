...
export function socialLiveCheckIntervalMs(source, sources) {
  if (source.platform !== 'YouTube') return LIVE_CHECK_INTERVAL_MS;
...
export async function runSocialLiveMonitor(options = {}) {
  ...
  for (const source of sources) {
    ...
    const row = { id: source.id, sourceName: source.name, platform: source.platform, checkedAt: now.toISOString(), nextCheckAt: new Date(now.getTime() + intervalMs).toISOString(), state: 'unknown', broadcasts: [], notifications: [] };
    try {
...
export async function getSocialLiveStatus(sources, options = {}) {
  const repository = options.repository ?? await (await import('./distribution-repository.mjs')).getDistributionRepository();
  const env = options.env ?? process.env;
  return Promise.all(sources.map(async source => ({ id: source.id, enabled: active(source), intervalMinutes: socialLiveCheckIntervalMs(source, sources) / 60_000, ...liveProviderStatus(source, env), lastCheck: await repository.getMeta(sourceKey(source)) })));
}
