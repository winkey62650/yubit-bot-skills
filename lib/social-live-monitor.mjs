...
import { renderSocialLiveNotice } from './social-live-notice.mjs';
import { findSimulcastReceipt, preferMainBroadcasts, simulcastIdentity } from './social-live-simulcast.mjs';
export { renderSocialLiveNotice } from './social-live-notice.mjs';
...
// Default YouTube projects allow 100 search.list calls per day. Spread normal
// single-page discovery across sources (~72/day total), leaving preview headroom.
export function socialLiveCheckIntervalMs() { return LIVE_CHECK_INTERVAL_MS; }
export function youtubeSearchIntervalMs(sources) {
  const count = sources.filter(item => item.platform === 'YouTube' && active(item)).length;
  return 20 * 60_000 * Math.max(1, count);
}
...
export async function runSocialLiveMonitor(options = {}) {
  ...
  const loadSources = options.loadSources ?? (async () => {
    ...
    return normalizeSocialPackages(saved.packages || saved).filter(source => source.platform === 'YouTube');
  });
  ...
  for (const source of sources) {
    ...
    const intervalMs = socialLiveCheckIntervalMs(source, allSources);
    if (!dryRun && Date.parse(previous?.checkedAt) + intervalMs > now.getTime()) continue;
    ...
    const row = { id: source.id, sourceName: source.name, platform: source.platform, checkedAt: now.toISOString(), nextCheckAt: new Date(now.getTime() + intervalMs).toISOString(), state: 'unknown', broadcasts: [], notifications: [], knownVideoIds: previous?.knownVideoIds || previous?.broadcasts?.map(item => item.id) || [] };
    try {
      const signal = AbortSignal.timeout(Math.max(1, deadline - Date.now()));
      let discovery;
      if (source.platform === 'YouTube' && !dryRun) {
        const searchKey = `${stateKey}:search`;
        const previousSearch = await repository.getMeta(searchKey);
        const search = !(Date.parse(previousSearch?.attemptedAt) + youtubeSearchIntervalMs(allSources) > now.getTime());
        // Reserve an attempt durably before calling the API. Failures/restarts
        // must not turn the five-minute feed loop into a search quota drain.
        if (search) await repository.setMeta(searchKey, { attemptedAt: now.toISOString() });
        discovery = { search, knownVideoIds: row.knownVideoIds };
      }
      const observation = await detect(source, { env, fetchImpl: options.fetchImpl ?? fetch, signal, discovery });
      ...
      row.knownVideoIds = observation.trackedVideoIds || observation.broadcasts.map(item => item.id);
      if (observation.strategy) row.strategy = observation.strategy;
      if (observation.warning) row.warning = observation.warning;
      if (!dryRun) {
        ...
        for (const live of preferMainBroadcasts(observation.broadcasts)) {
          for (const target of source.targets || []) {
            signal.throwIfAborted();
            // Different configured aliases have different source leases. This
            // shared owner/destination lease serializes their simulcast check.
            const simulcastLeaseKey = live.platform === 'YouTube' ? `${PREFIX}simulcast-lease:${keyHash([live.ownerId, liveTargetKey(target)])}` : null;
            if (simulcastLeaseKey && !await repository.acquireMetaLease(simulcastLeaseKey, lease, new Date())) continue;
            try {
              ...
              const duplicate = await findSimulcastReceipt(repository, live, liveTargetKey(target));
              if (duplicate) {
                const suppressed = { sourceId: source.id, targetKey: liveTargetKey(target), broadcastKey: broadcastKey(live), status: 'suppressed', duplicateOf: duplicate.key, simulcast: simulcastIdentity(live), updatedAt: now.toISOString() };
                signal.throwIfAborted();
                await repository.compareAndSetMeta(receiptKey, { absent: true }, suppressed);
                row.notifications.push({ target: liveTargetKey(target), status: 'suppressed', reason: 'simulcast', deliveryId: duplicate.value.deliveryId || null });
                continue;
              }
              const attemptId = randomUUID();
              const fenced = { sourceId: source.id, targetKey: liveTargetKey(target), broadcastKey: broadcastKey(live), status: 'dispatching', attemptId, simulcast: simulcastIdentity(live), updatedAt: now.toISOString() };
              signal.throwIfAborted();
              ...
              if (simulcastLeaseKey) await repository.releaseMetaLease(simulcastLeaseKey, lease.leaseId);
            }
          }
        }
      }
...
export async function getSocialLiveStatus(sources, options = {}) {
  const repository = options.repository ?? await (await import('./distribution-repository.mjs')).getDistributionRepository();
  const env = options.env ?? process.env;
  return Promise.all(sources.map(async source => ({ id: source.id, enabled: active(source), intervalMinutes: socialLiveCheckIntervalMs(source, sources) / 60_000, ...(source.platform === 'YouTube' ? { searchIntervalMinutes: youtubeSearchIntervalMs(sources) / 60_000 } : {}), ...liveProviderStatus(source, env), lastCheck: await repository.getMeta(sourceKey(source)) })));
}
