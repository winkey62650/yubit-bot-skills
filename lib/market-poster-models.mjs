...
function posterHeadline(value, fallback = "Verified market development") {
  const normalized = posterCopy(value, { words: 20, chars: 96, fallback });
  let match = normalized.match(/^(.+?)\s+(?:kept|left)\s+(?:its\s+|the\s+)?(.+?)\s+at\s+(.+)$/iu);
  if (match) return posterCopy(`${match[1]} ${match[2]}: ${match[3]}`, { words: 7, chars: 38, fallback });
  match = normalized.match(/^(.+?)\s+held\s+at\s+(.+)$/iu);
  if (match) return posterCopy(`${match[1]}: ${match[2]}`, { words: 7, chars: 38, fallback });
  match = normalized.match(/^(.+?)\s+remained\s+(?:a\s+|an\s+)?(.+)$/iu);
  if (match) return posterCopy(`${match[1]}: ${match[2]}`, { words: 7, chars: 38, fallback });
  let compacted = posterCopy(normalized, { words: 6, chars: 38, fallback });
  const weakTail = /\s+\b(?:at|a|an|the|as|to|of|for|with|and|or|in|on)\b[,:;.!?]*$/iu;
  while (weakTail.test(compacted)) compacted = compacted.replace(weakTail, "").trim();
...
function posterSentence(value, { kind = "generic", chars = 64, fallback = "Verified market development." } = {}) {
  ...
  const rules = [
    [/inflation.*above.*(?:federal reserve|fed).*target|above.*target.*inflation/iu, "Inflation keeps rates and USD sensitivity high."],
    [/restrictive policy|policy setting.*restrictive/iu, "Restrictive policy limits near-term liquidity."],
    [/regulated access|institutional market-structure/iu, "Regulated access improves structure, not a signal."],
...
export function buildWeeklyCalendarPosterModel(document = {}) {
  ...
  const displayEvents = keyedWeeklyEvents(sourceDays.flatMap((day, dayIndex) => {
    const ranked = (Array.isArray(day.events) ? day.events : [])
      ...
      // A major event may remain visible while its source is being refreshed, but
      // lower-priority density is only allowed when an explicit, current source
      // accompanies the item. This keeps the calendar full without inventing filler.
      .filter((event) => event.importance >= 3
        || (event.importance >= 1 && event.sourceStatus === "CURRENT"))
      .sort((left, right) => right.importance - left.importance || compareWeeklyEvent(left, right));
    return ranked.slice(0, 3).map((event, index) => ({
      ...event,
      lane: index === 0 ? "primary" : "secondary",
    }));
  }));
  ...
  const columns = sourceDays.map((day, dayIndex) => ({
    ...
    events: displayEvents
      ...
      .sort((left, right) => (left.lane === "primary" ? -1 : 1) - (right.lane === "primary" ? -1 : 1)
...
function reactionModel(reaction) {
  ...
  const symbol = rawSymbol.match(/^(BTC|ETH|SOL|XRP)(?:[-/]?USD(?:T|C)?)$/u)?.[1] ?? rawSymbol;
  ...
  if (!symbol || !label || value === null) return null;
  const status = clean(reaction?.status);
  return { symbol, value, label, ...(status ? { status } : {}) };
}
...
function marketReactions(input) {
  ...
  const priority = ["BTC", "ETH", "SOL", "XRP", "DXY", "NASDAQ", "US2Y"];
  const ordered = [
    ...priority.flatMap((symbol) => bySymbol.has(symbol) ? [bySymbol.get(symbol)] : []),
    ...[...bySymbol.entries()].filter(([symbol]) => !priority.includes(symbol)).map(([, reaction]) => reaction),
  ];
...
export function buildDataUpdatePosterModel(input = {}) {
  ...
  const posterVerdict = posterSentence(input.posterVerdict ?? input.verdict, { chars: 62, fallback: "" });
  const posterConfirmation = input.posterConfirmation ?? (input.confirmation ? posterSentence(input.confirmation, { chars: 44, fallback: "" }) : "");
  const posterInvalidation = input.posterInvalidation ?? (input.invalidation ? posterSentence(input.invalidation, { chars: 44, fallback: "" }) : "");
  const frame = editorialFrame({
  ...
  return {
    ...
    posterTitle: posterCopy(input.posterTitle ?? input.title ?? input.event?.title, { words: 7, chars: 42, fallback: "Verified release snapshot" }),
    posterSources: canonicalUnique(frame.footer.sources.map(posterSourceLabel)),
...
export function buildCryptoDailyPosterModel(document = {}) {
  ...
  const historicalReplay = document.mode === "historical-replay" || /HISTORICAL REPLAY/iu.test(clean(document.title));
  const replayReferenceAt = historicalReplay
    ? selected.map((story) => canonicalInstant(story?.publishedAt ?? story?.source?.observedAt ?? story?.source?.retrievedAt)).find(Boolean) || ""
    : "";
  const stories = selected.slice(0, 3).map((story, index) => {
    ...
    return {
      ...
      title,
      posterTitle: posterHeadline(story.posterTitle ?? title),
      ...
      thesis: thesis.text,
      posterThesis: posterSentence(story.posterThesis ?? story.rationale ?? story.summary, { chars: 52, fallback: "Verified market development." }),
  ...
  const frame = editorialFrame({
      ...
      updatedAt: replayReferenceAt || document.updatedAt || document.generatedAt,
    });
  return {
    ...frame,
    kind: "crypto-daily",
    date: clean(replayReferenceAt || document.generatedAt).slice(0, 10),
    historicalReplay,
