...
export async function sendTelegramPreservingClosedTopic(call, method, payload = {}, options = {}) {
  if (typeof call !== "function") throw new TypeError("call is required");
  options.signal?.throwIfAborted?.();
  // A closed Topic is an intentional moderation state. Telegram administrators
  // can still publish to it, so delivery must never mutate the Topic or retry in
  // General. If Telegram rejects the request, preserve that exact error.
  return call(method, payload, options);
}
...
export function createTelegramDelivery(options = {}) {
  ...
  return async function deliverTelegram(botToken, method, payload = {}, deliveryOptions = {}) {
    deliveryOptions.signal?.throwIfAborted?.();
    ...
    if (!isOutbound || !publisherRequired) {
      return sendTelegramPreservingClosedTopic(
        (nextMethod, nextPayload, nextOptions) => nextOptions.signal
          ? botApiCall(botToken, nextMethod, nextPayload, nextOptions)
          : botApiCall(botToken, nextMethod, nextPayload),
        method,
        payload,
        deliveryOptions
      );
    ...
    return sendTelegramPreservingClosedTopic(
      (nextMethod, nextPayload, nextOptions) => nextOptions.signal
        ? userPublisherCall(botToken, nextMethod, nextPayload, nextOptions)
        : userPublisherCall(botToken, nextMethod, nextPayload),
      method,
      payload,
      deliveryOptions
    );
