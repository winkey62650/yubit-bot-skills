...
export function parseTraderMessage(text) {
  ...
  if (type === "submit") {
    ...
    for (const line of lines) {
      const takeProfit = line.match(/^TP(?:[1-4])?\s*(?::|•|-)\s*(.+)$/i);
      if (takeProfit && takeProfits.length < 4) {
        takeProfits.push(takeProfit[1].trim().slice(0, 100));
        continue;
      }
      const match = line.match(/^(SL|Rationale)\s*:\s*(.+)$/i);
      if (!match) continue;
    ...
    if (takeProfits.length > 0) annotations.takeProfit = takeProfits.join(" | ");
  }
...
export function formatVerifiedSignal(signal, trader) {
  ...
  const direction = String(signal?.direction || "N/A");
  const directionIcon = direction.toLowerCase() === "short" ? "📉" : "📈";
  const takeProfits = takeProfitLevels(annotations.takeProfit);

  return [
    `<b>Symbol : ${escapeTelegramHtml(displaySymbol(signal?.symbol))}</b>`,
    "",
    `<b>${directionIcon} Positon: ${escapeTelegramHtml(direction)}</b>`,
    "",
    `<b>LV: ${leverage === null ? "N/A" : `${formatNumber(leverage, 2)}x`}</b>`,
    "",
    `<b>🎯 Entry: ${escapeTelegramHtml(formatNumber(signal?.entryPrice))}</b>`,
    "",
    "<b>✅ TP :</b>",
    ...takeProfits.flatMap((level, index) => ["", `<b>TP${index + 1}• ${escapeTelegramHtml(level)}</b>`]),
    "",
    "<b>⚠️ SL</b>",
    "",
    `<b>• ${escapeTelegramHtml(annotations.stopLoss || "N/A")}</b>`,
  ].join("\n");
}
