...
function hasConflictingStoryDirection(leftEvents, rightEvents) {
  return (
    (leftEvents.has("net-inflow") && rightEvents.has("net-outflow"))
    || (leftEvents.has("net-outflow") && rightEvents.has("net-inflow"))
    || (leftEvents.has("approval") && rightEvents.has("rejection"))
    || (leftEvents.has("rejection") && rightEvents.has("approval"))
  );
}
...
function sameStoryFact(left, right) {
  ...
  if (hasConflictingStoryDirection(leftFact.events, rightFact.events)) return false;
...
function directionalImpactFromEvidence(value) {
  ...
  const bullish = /\b(?:net\s+inflows?|approv(?:al|e|ed|es)|adoption|launch(?:ed)?\s+successfully|allocations?\s+increased|stronger\s+institutional\s+demand)\b/.test(evidence);
  const bearish = /\b(?:net\s+outflows?|hack(?:ed)?|exploit(?:ed)?|ban(?:ned)?|enforcement\s+action|shutdown|allocations?\s+decreased|weaken(?:ing|ed)?\s+institutional\s+demand)\b/.test(evidence);
