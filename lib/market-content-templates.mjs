...
const DIRECTIONAL_EVIDENCE_PATTERNS = Object.freeze({
  Bullish: Object.freeze([
    /\bnet\s+inflows?\b/gu,
    /\bapprov(?:al|e|ed|es)\b/gu,
    /\badoption\b/gu,
    /\b(?:successful(?:ly)?\s+launch(?:ed|es|ing)?|launch(?:ed|es|ing)?\s+(?:successful(?:ly)?|succeeds?|succeeded))\b/gu,
    /\ballocations?\s+increased\b/gu,
    /\bstronger\s+institutional\s+demand\b/gu,
  ]),
  Bearish: Object.freeze([
    /\bnet\s+outflows?\b/gu,
    /\b(?:hack(?:ed|ing|s)?|exploit(?:ed|ing|s)?|breach(?:ed|ing|es)?)\b/gu,
    /\bban(?:ned|s)?\b/gu,
    /\b(?:reject(?:ion|ed|s)?|den(?:y|ial|ied|ies))\b/gu,
    /\benforcement\s+action\b/gu,
    /\b(?:shutdown|shut(?:s|ting)?\s+down)\b/gu,
    /\bfail(?:ed|s|ure)?(?:\s+[a-z]+){0,3}\s+launch(?:ed|es|ing)?\b/gu,
    /\blaunch(?:ed|es|ing)?(?:\s+[a-z]+){0,3}\s+(?<direction>fail(?:ed|s|ure)?)\b/gu,
    /\ballocations?\s+decreased\b/gu,
    /\bweaken(?:ing|ed)?\s+institutional\s+demand\b/gu,
  ]),
});
...
function directionalPolarities(value) {
  ...
  for (const [polarity, patterns] of Object.entries(DIRECTIONAL_EVIDENCE_PATTERNS)) {
    const supported = patterns.some((pattern) => (
      [...evidence.matchAll(pattern)].some((match) => {
        ...
        return !directionalMatchIsNegated(evidence, directionIndex);
      })
    ));
    if (supported) polarities.add(polarity);
  }
  return polarities;
}
...
function hasConflictingStoryDirection(leftFact, rightFact) {
  return [...leftFact.polarities].some((leftPolarity) => (
...
function storyDirectionalEvidence(story) {
  return text(story?.rationale) || text(story?.evidence);
}

function directionalImpactFromEvidence(value) {
  const polarities = directionalPolarities(value);
  return polarities.size === 1 ? [...polarities][0] : "Neutral";
}

function inferredStoryImpact(story) {
  const supplied = safeImpact(story?.impact);
  if (text(story?.impact).toLowerCase() === "neutral") return "Neutral";
  const directions = [
    story?.rationale,
    story?.evidence,
    `${text(story?.title)} ${text(story?.summary)}`,
  ].map(directionalImpactFromEvidence).filter((impact) => impact !== "Neutral");
  const distinctDirections = new Set(directions);
  const supported = distinctDirections.size === 1 ? directions[0] : "Neutral";
  if (supplied !== "Neutral") return supported === supplied ? supplied : "Neutral";
  return supported;
}
...
function cryptoSection(id, selected) {
  ...
  const rationale = storyDirectionalEvidence(selected) || text(selected.summary) || "Available evidence does not establish a directional market impact.";
  return {
    id,
    title: details.title,
    impact,
