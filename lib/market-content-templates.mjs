...
export function deduplicateCryptoStories(stories) {
  ...
  for (const candidate of Array.isArray(stories) ? stories : []) {
    if (!candidate || !text(candidate.title)) continue;
    const key = storyKey(candidate);
    const existing = groups.get(key);
    if (!existing) {
      groups.set(key, { ...candidate, sourceConfirmations: Math.max(1, Number(candidate.sourceConfirmations) || 1) });
      continue;
    }
  ...
  return [...groups.values()].sort((left, right) => storyKey(left).localeCompare(storyKey(right), "en"));
}
...
function eventDate(event) {
  const scheduled = validDate(event?.scheduledAt);
  if (scheduled) return scheduled.toISOString().slice(0, 10);
  return dateOnly(event?.rawScheduledAt ?? event?.date);
}
...
function importantCalendarEvent(event) {
  if (slug(event?.kind ?? event?.type) === "crypto") {
    return officialStory(event) && Boolean(validDate(event?.scheduledAt)) && Number(event?.importance) >= 2;
  }
...
export function evaluateReleaseImpact(release) {
  ...
  const signals = components.map(directionalSignal).filter((signal) => signal !== null);
  const impact = !signals.length || signals.includes(0) || new Set(signals).size > 1
    ? "Neutral"
...
export function buildDataReleaseDocument({ event, reaction } = {}) {
  ...
  const impact = evaluateReleaseImpact({ ...event, components: records });
