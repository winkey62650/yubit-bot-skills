...
const STORY_TOPIC_PATTERNS = Object.freeze([
  ["etf", /\b(?:etf|exchange traded fund|ibit|bitcoin trust)\b/],
  ["custody", /\bcustod(?:y|ian)\b/],
  ["license", /\blicen[cs](?:e|ed|ing)\b/],
]);

const STORY_ENTITY_STOPWORDS = new Set([
  "after", "august", "bitcoin", "crypto", "daily", "dollar", "dollars", "ethereum", "fund", "funds",
  "into", "million", "net", "publishes", "recorded", "records", "report", "reports", "saw", "spot", "their",
...
function storyFact(story) {
  ...
  const topics = new Set(STORY_TOPIC_PATTERNS.filter(([, pattern]) => pattern.test(corpus)).map(([topic]) => topic));
  ...
  return { events, topics, numbers: [...new Set(numbers)].sort(), entities: new Set(entities) };
}
...
function sameStoryFact(left, right) {
  ...
  if ((leftFact.topics.size || rightFact.topics.size) && ![...leftFact.topics].some((topic) => rightFact.topics.has(topic))) return false;
  ...
  return sharedEntities.length >= (leftFact.numbers.length ? 1 : 2);
}
