...
function tierOneRelease(event) {
  const value = [event?.indicator, event?.id, event?.title, event?.name]
    ...
    .replace(/[-_/]+/g, " ")
    .replace(/\s+/g, " ");
  const fomcDecision = /\bfomc\s+(?:decision|(?:interest\s+)?rate\s+decision|(?:monetary\s+)?policy\s+decision)\b/.test(value);
  const fomcStatement = /\bfomc\s+(?:monetary\s+policy\s+)?statement\b/.test(value);
  const fomcRelease = !/\b(?:preview|expectations?|watch|ahead of|what to expect|analysis)\b/.test(value)
    && (fomcDecision || fomcStatement);
