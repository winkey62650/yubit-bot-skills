...
export class JsonDistributionRepository {
  async read() {
    ...
    state.rules = (Array.isArray(state.rules) ? state.rules : []).map((stored) => {
      ...
      if (stored.kind === "automation" && stored.contentType === "whale-signals" && stored.schedulePreset !== "daily-0800-utc") {
        return { ...rule, nextRunAt: computeNextRunAt("daily-0800-utc", new Date()).toISOString() };
      }
      return rule;
    });
    return state;
  ...
  async claimDueAutomationRules(now = new Date()) {
    return this.mutate((state) => {
      const due = state.rules.filter((rule) => rule.kind === "automation" && rule.enabled && (!rule.nextRunAt || Date.parse(rule.nextRunAt) <= now.getTime()));
      for (const rule of due) {
        const next = computeNextRunAt(rule.schedulePreset, now).toISOString();
        rule.nextRunAt = next;
...
export class PostgresDistributionRepository {
  ...
  async initialize() {
    const statements = [
      `CREATE TABLE IF NOT EXISTS distribution_rules (
        ...
        enabled boolean NOT NULL DEFAULT true, status text NOT NULL DEFAULT 'ready',
        next_run_at timestamptz, imported_from text, created_at timestamptz NOT NULL DEFAULT now(),
      ...
      `CREATE TABLE IF NOT EXISTS distribution_targets (
        ...
        chat_id text NOT NULL, thread_id bigint, group_name text, topic_name text,
      ...
      `CREATE TABLE IF NOT EXISTS distribution_deliveries (
        ...
        target_message_id bigint, error text, delivered_at timestamptz,
    ...
    await this.sql.query(`UPDATE distribution_rules
      SET schedule_preset='daily-0800-utc',
        next_run_at=CASE WHEN date_trunc('day',now()) + interval '8 hours' > now()
          THEN date_trunc('day',now()) + interval '8 hours'
          ELSE date_trunc('day',now()) + interval '1 day 8 hours' END,
        updated_at=now()
      WHERE kind='automation' AND content_type='whale-signals'
        AND schedule_preset IS DISTINCT FROM 'daily-0800-utc'`);
    return this;
  }
  ...
  async getRule(id) {
    ...
    return rowRule(rows[0], targets.map((target) => ({ id: target.id, chatId: target.chat_id, threadId: target.thread_id == null ? null : Number(target.thread_id), groupName: target.group_name, topicName: target.topic_name, enabled: target.enabled, order: target.sort_order })));
  }

  async saveRule(input) {
    const rule = ensureAutomationNextRunAt(normalizeDistributionRule(input));
    await this.sql.query(`INSERT INTO distribution_rules
      (id, kind, name, content_type, schedule_preset, mode, source, enabled, status, next_run_at, imported_from)
      ...
      next_run_at=EXCLUDED.next_run_at,imported_from=EXCLUDED.imported_from,updated_at=now()`,
    [rule.id, rule.kind, rule.name, rule.contentType, rule.schedulePreset, rule.mode, JSON.stringify(rule.source), rule.enabled, rule.status, rule.nextRunAt, rule.importedFrom]);
    ...
    for (const [index, target] of rule.targets.entries()) {
      await this.sql.query(`INSERT INTO distribution_targets
        (id,rule_id,chat_id,thread_id,group_name,topic_name,enabled,sort_order)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
      [target.id, rule.id, target.chatId, target.threadId, target.groupName, target.topicName, target.enabled, index]);
    }
  ...
  async claimDueAutomationRules(now = new Date()) {
    const rows = await this.sql.query(`UPDATE distribution_rules SET next_run_at = CASE schedule_preset
      WHEN 'daily-0800-utc' THEN CASE WHEN date_trunc('day',$1::timestamptz) + interval '8 hours' > $1::timestamptz
        THEN date_trunc('day',$1::timestamptz) + interval '8 hours' ELSE date_trunc('day',$1::timestamptz) + interval '1 day 8 hours' END
      WHEN 'hourly' THEN date_trunc('hour',$1::timestamptz) + interval '1 hour'
      WHEN 'every-4-hours' THEN date_trunc('day',$1::timestamptz) + ((floor(extract(hour from $1::timestamptz) / 4) + 1) * interval '4 hours')
      WHEN 'every-5-minutes' THEN date_trunc('hour',$1::timestamptz) + ((floor(extract(minute from $1::timestamptz) / 5) + 1) * interval '5 minutes')
      ELSE date_trunc('hour',$1::timestamptz) + ((floor(extract(minute from $1::timestamptz) / 15) + 1) * interval '15 minutes') END,
      updated_at=now() WHERE kind='automation' AND enabled=true AND (next_run_at IS NULL OR next_run_at <= $1::timestamptz)
      RETURNING *`, [now.toISOString()]);
    if (!rows.length) return [];
    const targets = await this.sql.query("SELECT * FROM distribution_targets WHERE rule_id = ANY($1::text[]) ORDER BY sort_order", [rows.map((row) => row.id)]);
    return rows.map((row) => rowRule(row, targets.filter((target) => target.rule_id === row.id).map((target) => ({ id: target.id, chatId: target.chat_id, threadId: target.thread_id == null ? null : Number(target.thread_id), groupName: target.group_name, topicName: target.topic_name, enabled: target.enabled, order: target.sort_order }))));
  }
  ...
  async updateDelivery(id, patch) {
    ...
    const rows = await this.sql.query(`UPDATE distribution_deliveries SET status=$2,attempts=$3,
      target_message_id=$4,error=$5,delivered_at=$6,updated_at=now() WHERE id=$1 RETURNING *`,
      [id, merged.status, merged.attempts, merged.targetMessageId, merged.error, merged.deliveredAt]);
    return rowDelivery(rows[0]);
  }
...
export async function getDistributionRepository() {
  if (!repositoryPromise) {
    repositoryPromise = (async () => {
      ...
      if (databaseUrl) return new PostgresDistributionRepository(databaseUrl).initialize();
      ...
      if (process.env.VERCEL || process.env.NODE_ENV === "production") {
        throw new Error("内容分发数据库未配置：请设置 DATABASE_URL");
      }
