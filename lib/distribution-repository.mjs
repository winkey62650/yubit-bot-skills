...
export class JsonDistributionRepository {
  ...
  async claimDueAutomationRules(now = new Date()) {
    return this.mutate((state) => {
      ...
      const claimed = due.map((rule) => clone(rule));
      for (const rule of due) {
        if (rule.runOnce) {
          ...
          rule.nextRunAt = computeNextRunAt(rule.schedulePreset, now).toISOString();
        }
...
export class PostgresDistributionRepository {
  ...
  async saveRule(input) {
    ...
    await this.sql.query(`INSERT INTO distribution_rules
      (id, kind, name, content_type, schedule_preset, mode, source, enabled, run_once, status, next_run_at, imported_from)
    ...
    [rule.id, rule.kind, rule.name, rule.contentType, rule.schedulePreset, rule.mode, JSON.stringify(rule.source), rule.enabled, rule.runOnce, rule.status, rule.nextRunAt, rule.importedFrom]);
  ...
  async claimDueAutomationRules(now = new Date()) {
    const rows = await this.sql.query(`UPDATE distribution_rules
      SET enabled = CASE WHEN run_once THEN false ELSE enabled END,
      status = CASE WHEN run_once THEN 'running' ELSE status END,
      next_run_at = CASE WHEN run_once THEN NULL ELSE CASE schedule_preset
        ...
        ELSE date_trunc('hour',$1::timestamptz) + ((floor(extract(minute from $1::timestamptz) / 15) + 1) * interval '15 minutes') END END,
