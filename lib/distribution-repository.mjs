...
function jsonAutomationCircuitNamespace(env = process.env) {
  const backend = String(env.JSON_STORE_BACKEND || "").trim().toLowerCase();
  const dedicatedLocal = Boolean(env.APP_RELEASE_SHA && !env.VERCEL);
  const blob = !dedicatedLocal && (backend === "blob"
    || (backend !== "local" && Boolean(env.VERCEL || env.VERCEL_OIDC_TOKEN || env.BLOB_READ_WRITE_TOKEN)));
  const storageKind = blob ? "blob" : "local";
  const identity = blob
    ? localStatePath
    : resolve(String(env.JSON_STORE_DIRECTORY || "").trim() || resolve(process.cwd(), ".runtime"), localStatePath);
  const digest = createHash("sha256").update(identity).digest("hex");
  return `distribution-json-${storageKind}-v1:${digest}`;
}

export class JsonDistributionRepository {
  constructor() {
    this.automationCircuitNamespace = jsonAutomationCircuitNamespace();
  }
...
export class PostgresDistributionRepository {
  constructor(databaseUrl) {
    this.automationCircuitNamespace = "distribution-postgres-production-v1";
