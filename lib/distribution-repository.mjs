...
function rowDelivery(row) {
  const parsedTargetMessageId = row.target_message_id == null ? null : Number(row.target_message_id);
  const targetMessageId = Number.isInteger(parsedTargetMessageId) && parsedTargetMessageId > 0
    ? parsedTargetMessageId
    : null;
  const targetMessageIds = Array.isArray(row.target_message_ids)
    ? row.target_message_ids.map(Number).filter((messageId) => Number.isInteger(messageId) && messageId > 0)
...
export class PostgresDistributionRepository {
  ...
  async updateDelivery(id, patch) {
    ...
    const targetMessageIds = suppliedMessageIds
      .map(Number)
    ...
    const rows = await this.sql.query(`UPDATE distribution_deliveries SET status=$2,attempts=$3,
      target_message_id=$4,target_message_ids=$5::jsonb,publisher_progress=$6::jsonb,error=$7,delivered_at=$8,updated_at=now() WHERE id=$1 RETURNING *`,
      [id, merged.status, merged.attempts, targetMessageId, JSON.stringify(targetMessageIds), JSON.stringify(publisherProgress), merged.error, merged.deliveredAt]);
    return rowDelivery(rows[0]);
  }
