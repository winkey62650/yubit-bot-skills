...
const DEMO_PUBLIC_TITLES = Object.freeze({
...
const DEMO_POSTER_TEMPLATES = Object.freeze({
...
const DEMO_POSTER_CONTRACTS = Object.freeze({
  ...
  "weekly-catalyst-calendar": Object.freeze({
...
const PUBLIC_BRAND_PATTERN = /\bYUBIT(?:\s+ACADEMY)?\b/i;
const PUBLIC_GENERIC_HORIZON_PATTERN = /\b(?:0[–-]4H|1[–-]7D|1[–-]3D)\b|Intraday\s*\/\s*1[–-]3D/i;

function visibleTelegramText(text) {
  return String(text || "")
...
function buildDemoShowcaseTemporaryRule(showcaseCase, {
  ...
  const resolvedThreadId = Number(threadId);
  const defaultThreadId = Number(showcaseCase.threadId);
  const isDailyReviewSupplement = showcaseCase.key === "daily" && resolvedThreadId === DEMO_THREAD_ID;
  if (resolvedThreadId !== defaultThreadId && !isDailyReviewSupplement) {
    throw new Error("Academy DEMO showcase target override is not approved");
  }
  return {
    id: ruleId,
    kind: "automation",
    ...
    targets: [{
      ...
      groupName: "DEMO Academy",
      topicName: String(topicName || showcaseCase.topicName),
    }],
  };
}
...
function decodeDemoPosterIdentity(url) {
  let parsedUrl;
  let poster;
  try {
    parsedUrl = new URL(String(url || ""));
    const encoded = parsedUrl.searchParams.get("data");
    if (!encoded) throw new Error("missing data");
    poster = JSON.parse(Buffer.from(encoded, "base64url").toString("utf8"));
  } catch {
    throw new Error("Academy DEMO poster URL is missing a valid governed poster payload");
  }
  const template = poster?.visualTemplate;
  if (!template?.id || !template?.product) {
    throw new Error("Academy DEMO poster payload is missing its visual template identity");
  }
  return {
    ...
    assetPath: template.assetPath || null,
    sha256: template.sha256 || null,
    composition: template.composition || null,
    version: Number(template.version || 0),
    canvas: {
      width: Number(template.canvas?.width || 0),
      height: Number(template.canvas?.height || 0),
    },
  };
}

function assertDemoShowcasePosterUrls(urls = [], showcaseCase) {
  if (!DEMO_SHOWCASE_CASES.includes(showcaseCase)) throw new Error("Unknown Academy DEMO showcase case");
  if (urls.length !== showcaseCase.productTypes.length) {
    throw new Error(`${showcaseCase.key} poster gate requires one poster per product`);
  }
  if (new Set(urls.map(String)).size !== urls.length) {
    throw new Error(`${showcaseCase.key} poster gate rejected duplicate poster URLs`);
  }
  return urls.map((url, index) => {
    const product = showcaseCase.productTypes[index];
    const expected = DEMO_POSTER_CONTRACTS[product];
    const identity = decodeDemoPosterIdentity(url);
    if (!expected
      || identity.product !== product
      || identity.templateId !== expected.id
      || identity.file !== expected.file
      || !identity.assetPath?.endsWith(`/${expected.file}`)
      || identity.sha256 !== expected.sha256
      || identity.composition !== "locked-master-fixed-field-overlay"
      || identity.version !== (expected.version || 4)
      || identity.canvas.width !== 1200
      || identity.canvas.height !== 675) {
      throw new Error(`${showcaseCase.key} poster gate rejected a non-canonical locked master for ${product}`);
    }
    return identity;
  });
}
...
function assertDemoShowcasePreview(preview = {}, showcaseCase, {
  ...
  if (!DEMO_SHOWCASE_CASES.includes(showcaseCase)) throw new Error("Unknown Academy DEMO showcase case");
  if (preview.demoShowcase !== true || preview.textOnly === true || !preview.imageUrl) {
    throw new Error("Academy DEMO showcase preview must be explicit and include approved poster media");
  }
  ...
  if (plans.length !== 1 || !targetMatches(plans[0]?.target, { threadId })) {
  ...
  if (steps.length !== showcaseCase.productTypes.length * 2
    || showcaseCase.productTypes.some((_, index) => {
      const poster = steps[index * 2];
      const copy = steps[index * 2 + 1];
      return poster?.method !== "sendPhoto"
        || !/^https:\/\//i.test(String(poster?.payload?.photo || ""))
        || poster?.payload?.caption
        || copy?.method !== "sendMessage"
        || !copy?.payload?.text;
    })) {
    throw new Error("Academy DEMO showcase must send one caption-free poster immediately before each text product");
  }
  const posterIdentities = assertDemoShowcasePosterUrls(
    steps.filter((step) => step?.method === "sendPhoto").map((step) => step.payload.photo),
    showcaseCase,
  );
  const expectedTemplateIds = showcaseCase.productTypes.map((product) => DEMO_POSTER_TEMPLATES[product]);
  const availableTemplateIds = Object.keys(preview.mediaDelivery?.byTemplateId ?? {});
  if (expectedTemplateIds.some((templateId) => !availableTemplateIds.includes(templateId))) {
    throw new Error("Academy DEMO showcase is missing an approved product poster template");
  }
  const texts = steps.filter((step) => step?.method === "sendMessage").map((step) => String(step?.payload?.text || ""));
  if (texts.some((text) => PUBLIC_BRAND_PATTERN.test(visibleTelegramText(text)))) {
    throw new Error("Academy DEMO public copy must remain publisher-neutral");
  }
  if (texts.some((text) => PUBLIC_GENERIC_HORIZON_PATTERN.test(text))) {
    throw new Error("Academy DEMO public copy must not expose generic editorial horizons");
  }
  const joined = texts.join("\n");
  for (const productType of showcaseCase.productTypes) {
    const title = DEMO_PUBLIC_TITLES[productType];
    if (!joined.includes(`<b>${title}</b>`)) {
      throw new Error(`Academy DEMO public copy is missing its neutral content title: ${title}`);
    }
  }
  if (!joined.includes("₿ BTC")) {
    throw new Error("Academy DEMO public copy requires a native BTC recognition glyph");
  }
  return {
    productIds: products.map((product) => product.id),
    stepCount: steps.length,
    visualTemplateIds: expectedTemplateIds,
...
function assertDemoShowcaseExecution({
  ruleId,
  execution = {},
  deliveries = [],
  showcaseCase,
  ...
  if (preview.demoShowcase !== true || preview.textOnly === true || !preview.imageUrl) {
    throw new Error("Academy DEMO showcase execution lost its poster safety flags");
  }
  const results = Array.isArray(preview.targetResults) ? preview.targetResults : [];
  if (results.length !== 1 || !targetMatches(results[0]?.target, { threadId })) {
    throw new Error("Academy DEMO showcase execution left its approved Topic");
  }
  const messageIds = messageIdsOf(results[0]);
  if (results[0]?.status !== "success" || messageIds.length !== showcaseCase.productTypes.length * 2) {
    throw new Error("Academy DEMO showcase execution did not return every poster and text message ID");
  }
  const governance = preview.contentGovernance || {};
  const products = Array.isArray(governance.products) ? governance.products : [];
  if (governance.approved !== true
  ...
  if (plans.length !== 1
    ...
    || plan.steps?.length !== showcaseCase.productTypes.length * 2
    || showcaseCase.productTypes.some((_, index) => plan.steps?.[index * 2]?.method !== "sendPhoto"
      || plan.steps?.[index * 2 + 1]?.method !== "sendMessage")) {
    throw new Error("Academy DEMO showcase requires one canonical poster-and-text Obsidian delivery plan");
  }
  assertDemoShowcasePosterUrls(
    plan.steps.filter((step) => step?.method === "sendPhoto").map((step) => step.payload.photo),
    showcaseCase,
  );
  const matching = deliveries.filter((delivery) => delivery?.ruleId === ruleId
    && delivery?.status === "success"
    && targetMatches(delivery.target, { threadId })
    && messageIds.every((id) => messageIdsOf(delivery).includes(id)));
  ...
  return {
    ...
    target: { chatId: DEMO_CHAT_ID, threadId: Number(threadId) },
    messageIds,
    deliveryId: matching[0].id,
    feedbackReceiptId: feedback.receiptId,
    deliveredAt: matching[0].deliveredAt || null,
