...
function buildDemoShowcaseTemporaryRule(showcaseCase, { chatId = DEMO_CHAT_ID } = {}) {
  if (!DEMO_SHOWCASE_CASES.includes(showcaseCase)) throw new Error("Unknown Academy DEMO showcase case");
  return {
    id: `academy-demo-showcase-${showcaseCase.key}-temporary`,
    kind: "automation",
...
function assertDemoShowcaseRecoveryState({ receipts = [], rules = [] } = {}) {
  const showcaseRule = /^academy-demo-showcase-(daily|weekly|release)-temporary$/;
  const temporaryRules = rules.filter((rule) => showcaseRule.test(String(rule?.id || "")));
  if (temporaryRules.length !== 0) {
    throw new Error("Academy DEMO recovery requires no temporary showcase rules");
  }

  const byKey = (key) => receipts.filter((receipt) => (
    receipt?.ruleId === `academy-demo-showcase-${key}-temporary`
  ));
