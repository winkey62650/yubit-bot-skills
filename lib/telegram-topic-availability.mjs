...
export async function hydrateTelegramTopicAvailability(
  ...
  return Promise.all(dialogs.map(async (dialog) => {
    ...
    if (dialog?.isForum !== true && threadIds.length === 0) return dialog;
    ...
    try {
      const liveTopics = await call(null, "getForumTopics", { chat_id: chatId }, { userId });
      const liveById = new Map(
        (Array.isArray(liveTopics) ? liveTopics : []).map((topic) => [Number(topic.threadId), topic])
      );
      // Discovery may omit older Topics. Verify saved/selected IDs explicitly
      // before treating them as missing; a failed check stays unavailable.
      const unresolvedIds = threadIds.filter((id) => !liveById.has(id));
      if (unresolvedIds.length > 0) {
        const exactTopics = await call(null, "getForumTopicsById", {
          chat_id: chatId, thread_ids: unresolvedIds
        }, { userId });
        for (const topic of Array.isArray(exactTopics) ? exactTopics : []) {
          const id = normalizedThreadId(topic?.threadId);
          if (id !== null && unresolvedIds.includes(id)) liveById.set(id, topic);
        }
      }
      const orderedIds = [
        ...threadIds,
        ...[...liveById.keys()].filter((threadId) => !threadIds.includes(threadId))
      ];
      return {
        ...dialog,
        topics: orderedIds.map((threadId) => {
          ...
          const availabilityStatus = topic.deleted
            ? "deleted"
            : topic.closed
              ? dialog?.canManageTopics === true ? "managed-closed" : "closed"
          ...
          return {
            ...
            threadId,
            availabilityStatus,
            canSendMessages: availabilityStatus === "available" || availabilityStatus === "managed-closed"
          };
        }),
        topicStatusCheckedAt: checkedAt,
        topicStatusError: ""
