...
export function createTelegramMtprotoTransport(options = {}) {
  ...
  return async function telegramMtprotoCall(_botToken, method, payload = {}, options = {}) {
    ...
    // Use explicitly passed userId, or fall back to the env-configured account
    const resolvedUserId = options.userId || String(process.env.TELEGRAM_USER_PUBLISHER_ID || "").trim() || undefined;
    const client = await getClient(resolvedUserId);
