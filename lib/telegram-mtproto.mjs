...
async function canPublishWithOfficialIdentity(client, dialog) {
  ...
  try {
    ...
    await requireOfficialGroupIdentity(client, peer, telegramDialogChatId(dialog));
    return true;
  } catch {
    return false;
  }
      ...
      return Promise.all(dialogs
        ...
        .map(async d => {
          ...
          const canSendAsOfficialIdentity = hasMessagePermission
            ? await canPublishWithOfficialIdentity(client, d)
            : false;
          return {
            id: chatId,
