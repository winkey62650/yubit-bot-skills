...
function getSendAsUnsupported(error) {
  const code = String(error?.code || error?.errorMessage || "").toUpperCase();
  const name = String(error?.name || "");
  const message = String(error?.message || error || "");
  return code === "PEER_ID_INVALID"
    || name === "PeerIdInvalidError"
    || (/peer id is invalid/i.test(message) && /channels\.GetSendAs/i.test(message));
}
...
export function createTelegramMtprotoTransport(options = {}) {
  ...
  return async function telegramMtprotoCall(_botToken, method, payload = {}, options = {}) {
    ...
    if (method === "reopenForumTopic" || method === "closeForumTopic") {
      ...
      return client.invoke(new Api.messages.EditForumTopic({
        peer,
        topicId,
        closed: method === "closeForumTopic"
      }));
    }
    ...
    if (newContentMethods.has(method)) {
      try {
        ...
        // Telegram rejects explicit sendAs for ordinary supergroups. Sending without it
        // still honors Telegram's anonymous-admin group identity when that is enabled.
        if (
          err.code !== "TELEGRAM_GROUP_TARGET_REQUIRED"
          && err.code !== "TELEGRAM_GROUP_IDENTITY_NOT_AVAILABLE"
          && !getSendAsUnsupported(err)
        ) {
          throw err;
        }
        sendAs = undefined;
