...
async function resolveInputEntity(client, targetChatId, signal) {
  try {
    return await awaitAbortable(() => client.getInputEntity(targetChatId), signal);
  } catch (originalError) {
    if (isAbortError(originalError, signal)) throw abortError(signal);
    const dialogs = await awaitAbortable(() => client.getDialogs({ limit: 500 }), signal);
    const dialog = dialogs.find((candidate) => telegramDialogChatId(candidate) === targetChatId);
    if (!dialog?.entity) throw originalError;
    return awaitAbortable(() => client.getInputEntity(dialog.entity), signal);
  }
}

async function requireOfficialGroupIdentity(client, peer, targetChatId, signal) {
  let available;
  try {
    available = await awaitAbortable(
      () => client.invoke(new Api.channels.GetSendAs({ peer })),
      signal
    );
  } catch (cause) {
    if (isAbortError(cause, signal)) throw abortError(signal);
...
async function canPublishWithOfficialIdentity(client, dialog, signal) {
  const entity = dialog?.entity;
  if (entity?.broadcast === true) return true;
  if (entity?.megagroup !== true) return false;

  try {
    const peer = await awaitAbortable(() => client.getInputEntity(entity), signal);
    await requireOfficialGroupIdentity(client, peer, telegramDialogChatId(dialog), signal);
    return true;
  } catch (error) {
...
export function createTelegramMtprotoTransport(options = {}) {
  ...
  async function getClient(userId, signal) {
    throwIfAborted(signal);
    const stored = await awaitAbortable(() => loadSession(userId), signal);
    ...
    try {
      return await awaitAbortable(() => clientCache.get(cacheKey), signal);
    } catch (error) {
      // A caller abort must not disconnect or evict a shared client which may
      // still be serving another publisher invocation.
      if (!isAbortError(error, signal)) clientCache.delete(cacheKey);
      throw error;
    }
  }
  ...
  return async function telegramMtprotoCall(_botToken, method, payload = {}, options = {}) {
    ...
    if (method === "getDialogs") {
      ...
      return awaitAbortable(() => Promise.all(dialogs
        ...
        .map(async d => {
          ...
          const hasMessagePermission = telegramEntityCanSendMessages(d.entity, d);
          const canSendAsOfficialIdentity = hasMessagePermission
            ? await canPublishWithOfficialIdentity(client, d, signal)
            : false;
          return {
            id: chatId,
            ...
            canSendMessages: hasMessagePermission && canSendAsOfficialIdentity,
            canSendAsOfficialIdentity,
            publishUnavailableReason: !hasMessagePermission
              ? "message_permission_unavailable"
              : !canSendAsOfficialIdentity
                ? (d.entity?.megagroup === true
                  ? "official_identity_unavailable"
                  : "official_identity_unsupported")
                : "",
    ...
    const peer = await resolveInputEntity(client, targetChatId, signal);
    ...
    if (method === "getForumTopics") {
      ...
      for (let page = 0; page < 100; page += 1) {
        const response = await awaitAbortable(() => client.invoke(new Api.messages.GetForumTopics({
          peer, q: "", offsetDate, offsetId, offsetTopic, limit: 100
        })), signal);
        const batch = Array.isArray(response?.topics) ? response.topics : [];
        for (const topic of batch) topics.set(Number(topic.id), topic);
        const total = Number(response?.count);
        if (batch.length === 0 || (Number.isFinite(total) && topics.size >= total)
          || (!Number.isFinite(total) && batch.length < 100)) return mapForumTopics([...topics.values()]);

        // Telegram may return fewer than limit; count/cursor, not page size,
        // determines completion. See core.telegram.org/method/messages.getForumTopics.
        const last = batch.at(-1);
        offsetTopic = Number(last.id);
        offsetId = Number(last.topMessage || 0);
        offsetDate = Number(response.orderByCreateDate ? last.date
          : response.messages?.find((message) => Number(message.id) === offsetId)?.date);
        const cursor = `${offsetTopic}:${offsetId}:${offsetDate}`;
        if (!Number.isFinite(offsetDate) || cursors.has(cursor)) {
          throw new Error("Telegram Topic pagination did not advance; refresh the directory");
        }
        cursors.add(cursor);
      }
      throw new Error("Telegram Topic directory exceeded the pagination limit");
    }
    if (method === "getForumTopicsById") {
      ...
      const response = await awaitAbortable(() => client.invoke(new Api.messages.GetForumTopicsByID({
        peer,
        topics: topicIds
      })), signal);
    ...
    if (newContentMethods.has(method)) {
      const entity = await awaitAbortable(() => client.getEntity(peer), signal);
      if (entity?.broadcast === true) {
        // Telegram Channels publish with their native Channel identity.
        sendAs = undefined;
      } else if (entity?.megagroup === true) {
        sendAs = await requireOfficialGroupIdentity(client, peer, targetChatId, signal);
      } else {
    ...
    if (method === "sendMessage") {
      // teleproto does not expose physical cancellation for every send. Abort
      // rejects promptly and ignores any late receipt; the automation service
      // keeps its durable `sending` fence for manual reconciliation, preventing
      // an uncertain side effect from being retried automatically.
      return messageResult(await awaitAbortable(() => client.sendMessage(peer, {
        ...shared,
    ...
    if (["sendPhoto", "sendVideo", "sendDocument", "sendAnimation", "sendAudio", "sendVoice"].includes(method)) {
      ...
      return messageResult(await awaitAbortable(() => client.sendFile(peer, {
        ...shared,
        file: payload[field],
    ...
    if (method === "sendMediaGroup") {
      ...
      const messages = await awaitAbortable(() => client.sendFile(peer, {
        ...shared,
    ...
    if (method === "copyMessage" || method === "copyMessages") {
      const source = await resolveInputEntity(client, String(payload.from_chat_id), signal);
      const ids = method === "copyMessages"
        ? (Array.isArray(payload.message_ids) ? payload.message_ids : [])
        : [payload.message_id];
      const messages = await awaitAbortable(() => client.forwardMessages(peer, {
    ...
    if (method === "editMessageText" || method === "editMessageCaption") {
      return messageResult(await awaitAbortable(() => client.editMessage(peer, {
