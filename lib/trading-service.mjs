...
function recordSymbol(record) {
  return String(record?.symbol ?? record?.contract ?? record?.contractCode ?? record?.instrumentId ?? "").toUpperCase();
}
...
function matchesSubmittedRecord(record, parsed, { allowMissingOrderId = false } = {}) {
  ...
  return (allowMissingOrderId ? !orderId || orderId === parsed.orderId : orderId === parsed.orderId)
    && (!recordSymbol(record) || recordSymbol(record) === parsed.symbol);
}
  ...
  try {
    ...
    const result = sanitizeTradingResponse({
      ...
      symbolMatchingKeys: candidateOrderEntries
        .filter(([, value]) => String(value ?? "").toUpperCase() === parsed.symbol)
        .map(([key]) => key)
        .sort()
        .slice(0, 10),
