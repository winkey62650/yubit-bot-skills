...
import { cronSecretConfig, deploymentEnvironment } from "./deployment-config.mjs";
...
import { getDiscordStatus, sendDiscordMessage } from "./discord-service.mjs";
...
function tradingDemoOnly(env = process.env) {
  const configured = String(env?.TRADING_DEMO_ONLY ?? "").trim().toLowerCase();
  return configured === "true";
}

function demoChatId(env = process.env) {
  return String(env?.DEMO_TELEGRAM_CHAT_ID || DEFAULT_DEMO_CHAT_ID).trim();
}

function isDemoDestination(destination, env = process.env) {
  if (String(destination?.platform || "telegram") === "discord") {
    return Boolean(env?.DEMO_DISCORD_GUILD_ID)
      && String(destination?.guildId ?? "") === String(env.DEMO_DISCORD_GUILD_ID);
  }
  return String(destination?.chatId ?? "") === demoChatId(env);
}

function allowedTradingDestinations(destinations, env = process.env) {
  const unique = uniqueDestinations(destinations);
  return tradingDemoOnly(env) ? unique.filter((destination) => isDemoDestination(destination, env)) : unique;
}
...
async function resolveDependencies(options = {}) {
  ...
  return {
    ...
    discord: options.discord ?? ((channelId, payload) => sendDiscordMessage(channelId, payload, options.discordOptions ?? {})),
    discordStatus: options.discordStatus ?? (() => getDiscordStatus(options.discordOptions ?? {})),
...
export function tradingErrorStatus(error) {
  ...
  if (/未配置|TOKEN_REQUIRED|NOT_CONFIGURED|GATEWAY_OFFLINE|DATABASE|NETWORK_ERROR|TIMEOUT|HTTPS_REQUIRED/.test(message)) return 503;
  return 400;
}

function safeErrorCode(error, fallback) {
  const message = String(error?.message || "");
  const known = message.match(/(?:YUBIT|TELEGRAM|DISCORD|TRADING|PNL|CREDENTIAL)_[A-Z0-9_]+(?::\d+)?/);
...
function recordOrderId(record) {
  return String(record?.orderId ?? record?.orderID ?? record?.order_id ?? record?.id ?? "");
}
...
function recordSymbol(record) {
  return comparableSymbol(record?.symbol ?? record?.contract ?? record?.contractCode ?? record?.instrumentId ?? "");
}

function matchesSubmittedSymbol(value, submittedSymbol) {
  const actual = comparableSymbol(value);
  const submitted = comparableSymbol(submittedSymbol);
  if (!actual) return true;
  if (actual === submitted) return true;
  // YUBIT can append a contract-family suffix (for example PERP/LINEAR)
  // even when the request and web UI use the base pair. The exact order ID
  // remains mandatory, so this only relaxes the symbol representation.
  return submitted.length >= 6
    && actual.startsWith(submitted)
    && actual.length <= submitted.length + 20;
}

function matchesSubmittedRecord(record, parsed, { allowMissingOrderId = false } = {}) {
  const orderId = recordOrderId(record);
  const orderIdMatches = allowMissingOrderId
    ? !orderId || orderId === parsed.orderId
    : orderId === parsed.orderId;
  if (!orderIdMatches) return false;
  // The exchange order id is the authoritative account-scoped identifier.
  // Some YUBIT contract APIs return a venue-internal symbol that cannot be
  // derived from the public BTCUSDT label. Never relax the order-id match;
  // only use the symbol as a fallback for an execution that omits orderId.
  return orderId ? true : matchesSubmittedSymbol(recordSymbol(record), parsed.symbol);
}

function canonicalOrderRecord(record, parsed) {
  if (!record || typeof record !== "object") return record;
  return {
    ...record,
    orderId: recordOrderId(record) || parsed.orderId,
    symbol: recordSymbol(record) || parsed.symbol,
  };
}

async function lookupSubmittedOrder(client, parsed) {
  const errors = [];
  let exactRecords = [];
  try {
    exactRecords = listFromYubitResponse(await client.getOrderHistory({
      symbol: parsed.symbol,
      orderId: parsed.orderId,
      limit: 20,
    }));
  } catch (error) {
    errors.push(safeErrorCode(error, "YUBIT_EXACT_ORDER_LOOKUP_FAILED"));
  }

  let order = exactRecords.find((candidate) => matchesSubmittedRecord(candidate, parsed));
  let fallbackRecords = [];
  if (!order) {
    try {
      fallbackRecords = listFromYubitResponse(await client.getOrderHistory({
        symbol: parsed.symbol,
        limit: 100,
      }));
      order = fallbackRecords.find((candidate) => matchesSubmittedRecord(candidate, parsed));
    } catch (error) {
      errors.push(safeErrorCode(error, "YUBIT_ORDER_LIST_LOOKUP_FAILED"));
    }
  }

  return {
    order: canonicalOrderRecord(order, parsed),
    exactRecords,
    fallbackRecords,
    errors,
  };
...
async function lookupSubmittedExecutions(client, parsed) {
  const errors = [];
  let exactRecords = [];
  try {
    exactRecords = listFromYubitResponse(await client.getExecutions({
      symbol: parsed.symbol,
      orderId: parsed.orderId,
      limit: 100,
    }));
  } catch (error) {
    errors.push(safeErrorCode(error, "YUBIT_EXACT_EXECUTION_LOOKUP_FAILED"));
  }
  let executions = exactRecords.filter((candidate) => (
    matchesSubmittedRecord(candidate, parsed, { allowMissingOrderId: true })
  ));
  let fallbackRecords = [];
  if (executions.length === 0) {
    try {
      fallbackRecords = listFromYubitResponse(await client.getExecutions({
        symbol: parsed.symbol,
        limit: 100,
      }));
      executions = fallbackRecords.filter((candidate) => matchesSubmittedRecord(candidate, parsed));
    } catch (error) {
      errors.push(safeErrorCode(error, "YUBIT_EXECUTION_LIST_LOOKUP_FAILED"));
    }
  }
  return { executions, exactRecords, fallbackRecords, errors };
}
...
function destinationPlatform(destination) {
  return String(destination?.platform || "telegram").toLowerCase() === "discord" ? "discord" : "telegram";
}

function telegramHtmlToDiscord(value) {
  return String(value ?? "")
    .replace(/<br\s*\/?>/gi, "\n")
    .replace(/<\/?b>/gi, "**")
    .replace(/<\/?strong>/gi, "**")
...
async function sendTradingDestination(dependencies, token, destination, { text, imageUrl = null }) {
  if (destinationPlatform(destination) === "discord") {
    const message = await dependencies.discord(destination.channelId, {
      content: telegramHtmlToDiscord(text),
      ...(imageUrl ? { imageUrl } : {}),
    });
    return {
      attempts: 1,
      telegramMessageId: null,
      platformMessageId: message?.id ? String(message.id) : null,
    };
  }
  if (!token) throw new Error("TELEGRAM_TOKEN_REQUIRED");
  const sent = imageUrl
    ? await sendTelegramWithRetry(dependencies, token, photoPayload(destination, imageUrl, text), 2, "sendPhoto")
    : await sendTelegramWithRetry(dependencies, token, topicPayload(destination, text));
  const messageId = Number(sent.message?.message_id) || null;
  return {
    attempts: sent.attempts,
    telegramMessageId: messageId,
    platformMessageId: messageId == null ? null : String(messageId),
  };
}
...
async function verifySubmittedOrder(parsed, accounts, dependencies) {
  ...
  for (const account of accounts.filter((candidate) => candidate.status === "verified")) {
    ...
    try {
      ...
      const orderLookup = await lookupSubmittedOrder(client, parsed);
      errors.push(...orderLookup.errors.map((errorCode) => ({ accountId: account.id, errorCode })));
      const order = orderLookup.order;
      if (!order) {
        if (orderLookup.errors.length === 0) {
          errors.push({ accountId: account.id, errorCode: "YUBIT_ORDER_NOT_FOUND" });
        }
        continue;
      }
      const executionLookup = await lookupSubmittedExecutions(client, parsed);
      errors.push(...executionLookup.errors.map((errorCode) => ({ accountId: account.id, errorCode })));
      const executions = executionLookup.executions;
      matches.push({
        account,
        order,
...
async function finishUpdate(repository, updateId, status, result = {}) {
  await repository.completeUpdate(updateId, {
    processingStatus: status,
    safeErrorCode: result.errorCode ?? result.errorCodes?.[0] ?? null,
  });
...
export async function processSpeakerTelegramUpdate(update, options = {}) {
  ...
  const processClaimedUpdate = async () => {
    ...
    const destinations = allowedTradingDestinations(
      await repository.resolveDestinations(trader.id),
      dependencies.env,
    );
    ...
    for (const destination of destinations) {
      ...
      if (!claimed) continue;
      try {
        const sent = await sendTradingDestination(dependencies, token, destination, { text: publicText });
        ...
        const platform = destinationPlatform(destination).toUpperCase();
        const code = safeErrorCode(error, `${platform}_SIGNAL_DELIVERY_FAILED`);
        await repository.updateDelivery(claimed.id, {
          status: "failed",
          attempts: claimed.attempts + Math.max(0, Number(error?.telegramAttempts || 1) - 1),
          errorCode: code,
          errorMessageSafe: `${platform === "DISCORD" ? "Discord" : "Telegram"} delivery failed. Retry from the trading log.`,
        });
        failed += 1;
      }
    }
...
export async function recoverTraderOrder(input, options = {}) {
  const dependencies = await resolveDependencies(options);
  const trader = await dependencies.repository.getTrader(String(input?.traderId ?? ""));
  if (!trader) throw notFound("Trader 不存在");

  const symbol = normalizedSymbol(input?.symbol);
  const orderId = String(input?.orderId ?? "").trim();
  const parsed = parseTraderMessage(`${symbol} ${orderId}`);
  if (!parsed.ok) throw new Error("订单号格式无效");

  const now = nowDate(dependencies.now);
  const generatedUpdateId = -(now.getTime() * 1000 + Math.floor(Math.random() * 1000));
  const updateId = Number(options.recoveryUpdateId ?? generatedUpdateId);
  if (!Number.isSafeInteger(updateId)) throw new Error("恢复任务编号无效");

  const result = await processSpeakerTelegramUpdate({
    update_id: updateId,
    message: {
      message_id: Math.abs(updateId),
      date: Math.floor(now.getTime() / 1000),
      chat: { id: Number(trader.telegramUserId), type: "private" },
      from: {
        id: Number(trader.telegramUserId),
        username: trader.telegramUsername || undefined,
        is_bot: false,
      },
      text: `${symbol} ${parsed.orderId}`,
    },
  }, dependencies);

  await appendAdminAudit(dependencies.repository, dependencies.now, "recover-order", trader.id, {
    symbol,
    orderId: parsed.orderId,
    status: result.status,
    signalId: result.signalId ?? null,
  });
  return sanitizeTradingResponse(result);
}
...
function encryptedCredentials(account, key) {
  ...
  return { apiKey: String(parsed.apiKey).trim(), apiSecret: String(parsed.apiSecret).trim() };
}
...
export async function saveExchangeAccount(input, options = {}) {
  ...
  const account = await repository.saveAccount({
    ...
    lastErrorCode: ["pending", "disabled"].includes(nextStatus) ? null : current?.lastErrorCode,
  });
...
export async function diagnoseExchangeOrder(input, options = {}) {
  const dependencies = await resolveDependencies(options);
  const repository = dependencies.repository;
  const account = await repository.getAccountWithCredentials(String(input?.accountId ?? ""));
  if (!account) throw new Error("YUBIT 账户不存在");
  const parsed = parseTraderMessage(`${normalizedSymbol(input?.symbol)} ${String(input?.orderId ?? "").trim()}`);
  if (!parsed.ok) throw new Error("订单号格式无效");
  ...
  try {
    ...
    const orderLookup = await lookupSubmittedOrder(client, parsed);
    const executionLookup = await lookupSubmittedExecutions(client, parsed);
    const candidateOrder = orderLookup.order
      ?? orderLookup.exactRecords[0]
      ?? orderLookup.fallbackRecords[0]
      ?? null;
    const candidateOrderEntries = candidateOrder && typeof candidateOrder === "object"
      ? Object.entries(candidateOrder)
      : [];
    const errorCodes = [...new Set([...orderLookup.errors, ...executionLookup.errors])];
    let verificationStatus = orderLookup.order ? "order_found" : "order_not_found";
    if (orderLookup.order) {
      try {
        deriveVerifiedOrder(orderLookup.order, executionLookup.executions);
        verificationStatus = "filled_verified";
      } catch (error) {
        const code = safeErrorCode(error, String(error?.message || "YUBIT_ORDER_VERIFICATION_FAILED"));
    ...
    const result = sanitizeTradingResponse({
      accountId: account.id,
      symbol: parsed.symbol,
      orderMatched: Boolean(orderLookup.order),
      executionCount: executionLookup.executions.length,
      verificationStatus,
      errorCodes: [...new Set(errorCodes)],
      orderLookup: {
        exactCount: orderLookup.exactRecords.length,
        fallbackCount: orderLookup.fallbackRecords.length,
      },
      executionLookup: {
        exactCount: executionLookup.exactRecords.length,
        fallbackCount: executionLookup.fallbackRecords.length,
      },
      orderFieldKeys: orderLookup.order ? Object.keys(orderLookup.order).sort().slice(0, 40) : [],
      candidateOrderFieldKeys: candidateOrderEntries.map(([key]) => key).sort().slice(0, 40),
      orderIdMatchingKeys: candidateOrderEntries
        .filter(([, value]) => String(value ?? "") === parsed.orderId)
        .map(([key]) => key)
        .sort()
        .slice(0, 10),
      symbolMatchingKeys: candidateOrderEntries
        .filter(([, value]) => matchesSubmittedSymbol(value, parsed.symbol))
        .map(([key]) => key)
        .sort()
        .slice(0, 10),
      executionFieldKeys: executionLookup.executions[0]
        ? Object.keys(executionLookup.executions[0]).sort().slice(0, 40)
        : [],
      checkedAt: nowDate(dependencies.now).toISOString(),
    }, [credentials.apiKey, credentials.apiSecret]);
    await appendAdminAudit(repository, dependencies.now, "diagnose-order", account.id, {
      symbol: parsed.symbol,
      orderMatched: result.orderMatched,
...
export async function verifyTradingDestination(destinationId, options = {}) {
  ...
  if (destinationPlatform(destination) === "discord") {
    try {
      const status = await dependencies.discordStatus();
      if (!status?.configured) throw new Error("DISCORD_NOT_CONFIGURED");
      if (!status?.connected) throw new Error("DISCORD_GATEWAY_OFFLINE");
      const guild = (status.guilds || []).find((item) => String(item.id) === String(destination.guildId));
      const configuredGuild = status.config?.guilds?.[String(destination.guildId)];
      const channel = configuredGuild?.channels?.find((item) => String(item.channelId) === String(destination.channelId));
      if (!guild || !channel) throw new Error("DISCORD_TARGET_NOT_INITIALIZED");
      const saved = await repository.saveDestination({
        ...destination,
        guildTitle: guild.name || destination.guildTitle,
        channelTitle: channel.name || destination.channelTitle,
        lastVerifiedAt: nowDate(dependencies.now).toISOString(),
        lastErrorCode: null,
      });
      await appendAdminAudit(repository, dependencies.now, "verify-destination", saved.id, { ok: true, platform: "discord" });
      return sanitizeTradingResponse({ ok: true, destination: saved, botUsername: status.bot?.username || null });
    } catch (error) {
      const code = safeErrorCode(error, "DISCORD_TARGET_VALIDATION_FAILED");
      const saved = await repository.saveDestination({ ...destination, lastVerifiedAt: null, lastErrorCode: code });
      await appendAdminAudit(repository, dependencies.now, "verify-destination", destination.id, { ok: false, platform: "discord", errorCode: code });
      return sanitizeTradingResponse({ ok: false, destination: saved, errorCode: code });
    }
  }
  const token = speakerToken(dependencies.env);
...
export async function testTradingDestination(destinationId, options = {}) {
  ...
  if (!destination) throw new Error("发送目标不存在");
  if (tradingDemoOnly(dependencies.env) && !isDemoDestination(destination, dependencies.env)) {
    const error = new Error("DEMO_ONLY_TEST_POLICY");
    error.statusCode = 403;
    throw error;
  }
  const token = speakerToken(dependencies.env);
  const text = formatVerifiedSignal({
    symbol: "BTCUSDT",
    direction: "Long",
    leverage: 20,
    entryPrice: 64525,
  ...
  try {
    const sent = await sendTradingDestination(dependencies, token, destination, { text });
    await appendAdminAudit(dependencies.repository, dependencies.now, "test-destination", destination.id, { ok: true });
    return { ok: true, telegramMessageId: sent.telegramMessageId, platformMessageId: sent.platformMessageId };
  } catch (error) {
    const code = safeErrorCode(error, `${destinationPlatform(destination).toUpperCase()}_TEST_MESSAGE_FAILED`);
...
function uniqueDestinations(destinations) {
  return [...new Map((destinations ?? []).map((destination) => [
    destinationPlatform(destination) === "discord"
      ? `discord:${destination.guildId}:${destination.channelId}`
      : `telegram:${destination.chatId}:${destination.threadId ?? 0}`,
    destination,
  ])).values()];
}
...
async function publishVerifiedProfit(signal, trader, realizedPnl, roi, exitPrice, closedAt, dependencies) {
  ...
  const destinations = allowedTradingDestinations(
    await repository.resolveDestinations(signal.traderId),
    dependencies.env,
  );
  const token = speakerToken(dependencies.env);
  ...
  for (const destination of destinations) {
    ...
    if (!claimed) continue;
    try {
      const sent = await sendTradingDestination(dependencies, token, destination, { text: caption, imageUrl: cardAssetUrl });
      await repository.updateDelivery(claimed.id, {
        status: "delivered",
        attempts: claimed.attempts + sent.attempts - 1,
        telegramMessageId: sent.telegramMessageId,
      ...
      await repository.updateDelivery(claimed.id, {
        status: "failed",
        attempts: claimed.attempts + Math.max(0, Number(error?.telegramAttempts || 1) - 1),
        errorCode: safeErrorCode(error, `${destinationPlatform(destination).toUpperCase()}_PNL_DELIVERY_FAILED`),
...
export async function getTradingManagementOverview(options = {}) {
  ...
  const enrichedTraders = traders.map((trader) => {
    const linkedAccounts = accounts.filter((account) => account.traderIds.includes(trader.id));
    const verifiedAccounts = linkedAccounts.filter((account) => account.status === "verified");
    const traderDestinations = enabledDestinations.filter((destination) => (
      destination.scopeType === "trader" && destination.scopeId === trader.id
    ));
    const effectiveDestinations = traderDestinations.length
      ? traderDestinations
      : enabledDestinations.filter((destination) => destination.scopeType === "workspace");
    const telegramReady = /^\d{1,20}$/.test(String(trader.telegramUserId ?? ""));
    const canVerifyOrders = trader.status === "enabled" && telegramReady && verifiedAccounts.length > 0;
    return {
      ...trader,
      telegramReady,
      linkedAccountCount: linkedAccounts.length,
      verifiedAccountCount: verifiedAccounts.length,
      verifiedAccountIds: verifiedAccounts.map((account) => account.id),
      enabledDestinationCount: effectiveDestinations.length,
      canVerifyOrders,
      canPublish: canVerifyOrders && effectiveDestinations.length > 0,
    };
  });
  const metrics = {
    ...
    orderReadyTraders: enrichedTraders.filter((trader) => trader.canVerifyOrders).length,
    publishReadyTraders: enrichedTraders.filter((trader) => trader.canPublish).length,
  };
  return sanitizeTradingResponse({
    metrics,
...
export async function retryTradingDelivery(id, options = {}) {
  ...
  if (claimed.publicationType === "signal") {
    ...
    imageUrl = publication.cardAssetUrl;
    text = formatPnlCaption({
      ...signal,
      direction: signal.side,
  ...
  try {
    const sent = await sendTradingDestination(dependencies, token, destination, { text, imageUrl });
    ...
    const code = safeErrorCode(error, `${destinationPlatform(destination).toUpperCase()}_DELIVERY_RETRY_FAILED`);
    const delivery = await repository.updateDelivery(claimed.id, {
      status: "failed",
      attempts: claimed.attempts + Math.max(0, Number(error?.telegramAttempts || 1) - 1),
      errorCode: code,
      errorMessageSafe: `${destinationPlatform(destination) === "discord" ? "Discord" : "Telegram"} delivery failed. Retry from the trading log.`,
    });
