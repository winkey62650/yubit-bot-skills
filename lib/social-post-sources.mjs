...
export function renderPostNotice(source, post) {
  ...
  return {
    content: [`**${title} · ${date}**`, summary, `Click here to ${youtube ? 'watch the video' : 'read the update'} 👉 ${url}`, '@everyone'].join('\n\n'),
    allowedMentions: { parse: [] },
    embeds: [{
      title,
      url,
      description: `[${youtube ? 'Watch the video' : 'Read the update'}](${url})`,
