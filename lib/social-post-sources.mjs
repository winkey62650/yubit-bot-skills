...
export async function fetchSocialPosts(source, { env = process.env, fetchImpl = fetch, signal } = {}) {
  ...
  if (source.platform === 'YouTube') {
    ...
    const posts = videos.items.flatMap(item => {
      ...
      return [{ platform: 'YouTube', ownerId: channel.id, id: item.id, title: item.snippet.localized?.title || item.snippet.title, description: item.snippet.localized?.description || item.snippet.description || '', publishedAt: item.snippet.publishedAt, url: `https://www.youtube.com/watch?v=${item.id}` }];
    });
...
export function renderPostNotice(source, post) {
  ...
  const creator = label(source.agent, 'Creator update', 100);
  const title = label(post.title, youtube ? 'New YouTube video' : 'New post on X', 250);
  const summary = label(post.description, youtube ? 'A new video is ready to watch.' : 'A new creator update is available.', 500);
  const published = Number.isFinite(Date.parse(post.publishedAt)) ? new Date(post.publishedAt) : null;
  const date = published ? formatDate(published) : 'New update';
  return {
    content: [`**${creator} | ${youtube ? 'YouTube' : 'X'}**`, `**${title} · ${date}**`, summary, `Click here to ${youtube ? 'watch the video' : 'read the update'} 👉 ${url}`, '@everyone'].join('\n\n'),
    allowedMentions: { parse: [] },
    embeds: [{
      author: { name: youtube ? 'YouTube' : 'X' },
      title,
      url,
      description: `${creator}\n[${youtube ? 'Watch on YouTube' : 'Read on X'}](${url})`,
      color: youtube ? 0xFF0033 : 0x315b49,
      ...(youtube ? { image: { url: `https://i.ytimg.com/vi/${post.id}/hqdefault.jpg` } } : {}),
      ...(published ? { timestamp: published.toISOString() } : {}),
      footer: { text: `${youtube ? 'YouTube video' : 'X post'} · Creator update` }
    }]
  };
}
