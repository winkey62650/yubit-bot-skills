...
export function renderWhaleSignalText({
  ...
  return [
    `${html ? "<b>" : ""}🚨 LIQUIDITY ALERT${html ? "</b>" : ""}`,
    `${bold("FACT", html)}\nAt ${value(timestamp, html)} UTC, the ${value(pair, html)} perpetual order book ${value(concentrationRead, html)}:`,
    [
      `${bold("Visible size:", html)} ${value(quantity, html)} ${value(asset, html)} · approx. $${value(notional, html)}`,
      `${bold("Observed action:", html)} ${value(action, html)}`,
      `${bold("Key level:", html)} $${value(price, html)}`,
      `${bold("Depth context:", html)} ${value(state, html)} · imbalance ${value(imbalance, html)}%`
    ].join("\n"),
    `${bold("INTERPRETATION", html)}\n${value(directionRead, html)}`,
    `${bold("WATCH NEXT", html)}\n${value(watchNext, html)}`,
