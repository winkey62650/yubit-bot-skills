...
function reconcileSchedule(event, tier) {
  ...
  const ambiguous = tier === "tier-one"
    ? authoritative.find((candidate) => !timezoneUnambiguous(candidate.value))
    : null;
  const preferred = blocking?.candidate ?? ambiguous ?? clear[0] ?? authoritative[0];
  const status = blocking?.status
    ?? (ambiguous || !clear.length ? "timezone-conflict" : new Set(clear.map((candidate) => iso(candidate.value))).size > 1 ? "conflicting" : "verified");
  return {
