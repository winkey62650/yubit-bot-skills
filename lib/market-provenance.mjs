...
function reconcileSchedule(event, tier) {
  ...
  const preferred = authoritative[0];
  if (tier === "tier-one" && !timezoneUnambiguous(preferred.value)) {
    return {
      value: null,
  ...
  if (!clear.length) return { value: null, status: "timezone-conflict", comparisons: [] };
  const authoritativeTimes = new Set(clear.map((candidate) => iso(candidate.value)));
  const selectedStatus = statusOf(preferred);
  const status = BLOCKING_FIELD_STATUSES.has(selectedStatus)
    ? selectedStatus
    : authoritativeTimes.size > 1 ? "conflicting" : "verified";
  return {
