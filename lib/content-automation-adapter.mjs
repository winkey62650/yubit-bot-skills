...
function normalizedSources(generated, now) {
  ...
  for (const source of raw) {
    ...
    const title = text(source?.title ?? source?.label ?? source?.name ?? id) || id;
    const tier = evidenceTier(source);
    const snapshot = createHash("sha256")
      .update(JSON.stringify({ id, title, url, tier, observedAt }))
      .digest("hex")
      .slice(0, 16);
    const candidate = {
      id: `${id}-${snapshot}`,
...
const POSTER_PRODUCT = Object.freeze({
  ...
  "weekly-catalysts-v5": "weekly-catalyst-calendar",
...
function baseInput(jobId, generated, { now }) {
