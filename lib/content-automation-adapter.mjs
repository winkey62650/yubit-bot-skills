...
function intelligencePayload(product, generated, { timestamp, occurredAt, title, facts }) {
  ...
  const common = {
    ...
    horizon: product === "weekly-catalyst-calendar" ? "1–7D" : product === "daily-market-brief" ? "Intraday / 1–3D" : "0–4H",
    confidence: "Medium",
  ...
  if (product === "daily-market-brief") {
    const selected = Array.isArray(document.selectedStories) && document.selectedStories.length
      ? document.selectedStories.slice(0, 5)
    ...
    const catalysts = selected.map((story) => ({
      ...
      horizon: text(story.horizon) || "Intraday / 1–3D",
  ...
  const moves = reactionMoves(generated?.reaction);
  const surprise = nodeMetric(document.nodes, "Surprise vs Forecast") || (releaseValue(values.actual) === releaseValue(values.forecast) ? "In line with consensus" : `${releaseValue(values.actual)} vs ${releaseValue(values.forecast)}`);
  if (product === "data-flash") return {
    ...common,
    release: {
      actual: releaseValue(values.actual ?? generated?.event?.actual) || "Not available",
      forecast: releaseValue(values.forecast) || "No consensus",
...
function baseInput(jobId, generated, { now, publicBaseUrl }) {
  ...
  const dailyFacts = product === "daily-market-brief" ? (document.selectedStories ?? []).slice(0, 5).map((story) => {
