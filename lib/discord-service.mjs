...
export async function refreshDiscordDemoTemplate(options = {}) {
  ...
  const demoGuildSummary = (Array.isArray(guilds) ? guilds : []).find(
    (guild) => String(guild?.name || "") === DISCORD_DEMO_GUILD_NAME,
  );
  if (!demoGuildSummary?.id) {
    throw new Error(`Discord Bot is not installed in ${DISCORD_DEMO_GUILD_NAME}.`);
  }
  ...
  for (const channel of Array.isArray(channels) ? channels : []) {
    ...
    messagesByChannel[String(channel.id)] = await discordRequest(
      `/channels/${encodeURIComponent(String(channel.id))}/messages?limit=100`,
      runtime,
    );
  }
  ...
  for (const configuredGuild of Object.values(config.guilds || {})) {
    ...
    try {
      ...
      const channels = (configuredGuild.channels || []).map((configuredChannel) => {
        ...
        const identity = {
          templateKey: String(configuredChannel.templateKey || (Number.isInteger(templateId) ? `legacy:${templateId}` : "")),
