...
// Formatting repair for a recorded live notice. This never creates a message.
export async function editDiscordLiveNotice(channelId, messageId, expectedUrl, payload, options = {}) {
  if (![channelId, messageId].every(id => /^[1-9][0-9]*$/.test(String(id)))) throw new Error('Invalid Discord message destination.');
  const runtime = await getRuntimeOptions(options);
  const path = `/channels/${channelId}/messages/${messageId}`;
  const [bot, current] = await Promise.all([discordRequest('/users/@me', runtime), discordRequest(path, runtime)]);
  if (String(current?.channel_id) !== String(channelId) || String(current?.author?.id) !== String(bot?.id) || !bot?.id) throw new Error('The recorded live notice is not owned by this Bot.');
  if (!String(current.content || '').includes(expectedUrl) && !current.embeds?.some(embed => embed.url === expectedUrl)) throw new Error('The recorded message does not match this livestream.');
  const body = { content: payload.content, embeds: payload.embeds, allowed_mentions: { parse: [] } };
  const updated = await discordRequest(path, { ...runtime, method: 'PATCH', body });
  if (String(updated?.id) !== String(messageId)) throw new Error('Discord did not confirm the message edit.');
  const verified = await discordRequest(path, runtime);
  const expected = payload.embeds[0];
  if (verified.content !== payload.content || verified.embeds?.[0]?.title !== expected.title || verified.embeds?.[0]?.url !== expected.url || verified.embeds?.[0]?.description !== expected.description) throw new Error('The English livestream card could not be verified.');
  return { messageId: String(messageId), channelId: String(channelId), content: verified.content, embeds: verified.embeds };
}
