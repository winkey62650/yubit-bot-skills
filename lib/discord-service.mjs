...
export async function sendDiscordManualPublish({
  ...
  try {
    ...
    for (const channel of (health.guilds || []).flatMap((guild) => guild.channels || [])) {
      if (channel.canSend) allowedChannelIds.add(String(channel.channelId || ""));
    }
  } catch {
    ...
    try {
      ...
      const channels = liveTextChannels.map((liveChannel) => {
        ...
        return {
          ...
          // Publishing only requires visibility and the three delivery permissions.
          // History and channel management remain visible in the health audit, but
          // must not hide an otherwise valid manual/automation destination.
          permissionsOk: canView
