...
const DISCORD_EMBED_URL_MAX_LENGTH = 2_048;
...
export async function sendDiscordMessage(channelId, payload = {}, options = {}) {
  ...
  const repository = await resolveRepository(options);
  const runtime = await getRuntimeOptions({ ...options, repository });
  if (imageUrl.length > DISCORD_EMBED_URL_MAX_LENGTH && !attachment?.data) {
    runtime.signal?.throwIfAborted?.();
    const response = await runtime.fetchImpl(imageUrl, { signal: runtime.signal });
    if (!response.ok) {
      throw new Error(`Discord poster download failed with HTTP ${response.status}.`);
    }
  ...
  if (imageUrl && imageUrl.length <= DISCORD_EMBED_URL_MAX_LENGTH && !embeds.some((embed) => embed?.image?.url === imageUrl)) {
