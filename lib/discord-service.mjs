...
export async function refreshDiscordDemoTemplate(options = {}) {
  ...
  for (const channel of Array.isArray(channels) ? channels : []) {
    if (![0, 5].includes(Number(channel?.type))) continue;
...
export async function sendDiscordMessage(channelId, payload = {}, options = {}) {
  ...
  const attachment = normalizedPayload.attachment && typeof normalizedPayload.attachment === "object"
...
export async function sendDiscordManualPublish({
  ...
  if (!normalizedContent && !normalizedImageUrl && !normalizedEmbeds.length && !attachment?.data) {
  ...
  for (const channelId of targets) {
    ...
    if (attachment?.data && hasVerifiedLivePermissions && liveChannel.canAttach === false) {
      results.push({ ok: false, channelId, error: "Discord channel is missing Attach Files permission." });
      continue;
    }
    if (normalizedImageUrl && hasVerifiedLivePermissions && liveChannel.canEmbed === false) {
      results.push({ ok: false, channelId, error: "Discord channel is missing Embed Links permission." });
      continue;
    }
    try {
...
export async function checkDiscordHealth(options = {}) {
  ...
  for (const guildSummary of guildSummaries) {
