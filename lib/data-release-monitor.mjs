...
function monitoredRecord(event, previous, now) {
  const actual = normalizedActual(event);
  return {
    eventKey: eventMonitorKey(event),
    id: eventSourceId(event),
    scheduledAt: eventSchedule(event),
    lastActual: actual ?? previous?.lastActual ?? null,
    observedAt: iso(event?.actualObservedAt ?? event?.values?.actualObservedAt ?? event?.observedAt ?? now),
  };
}
...
export function selectReleasableEvents(events, state, now) {
  ...
  for (const event of stableEvents(collapseMatchingEvents(Array.isArray(events) ? events : []))) {
    ...
    const scheduledAt = instant(event?.scheduledAt);
    const checkedAt = instant(now);
    if (status !== "monitoring" || !actual || !scheduledAt || !checkedAt
      || checkedAt < scheduledAt || actualIsStale(event)) continue;
    const deduplicationKey = buildReleaseDeduplicationKey(event);
    if (published.has(deduplicationKey)) continue;
    if (prior?.lastActual === actual) continue;
    releasableEvents.push(event);
  }
...
export async function pollDataReleaseUpdates(options = {}) {
  ...
  if (cacheStale) {
    ...
    warnings.push(...(Array.isArray(fetchResult?.warnings) ? fetchResult.warnings.map(String) : []));
  ...
  if (!fetchResult && typeof fetchCalendar === "function") {
    fetchResult = await fetchCalendar({ from: week.start.toISOString(), to: week.end.toISOString(), now: checkedAtIso });
    warnings.push(...(Array.isArray(fetchResult?.warnings) ? fetchResult.warnings.map(String) : []));
  }
  const liveEvents = fetchResult ? adapterEvents(fetchResult) : adapterEvents(storedCalendar);
  if (!fetchResult) warnings.push("Live calendar adapter was unavailable; cached schedule was used.");
  ...
  const duplicate = stableEvents(liveEvents).find((event) => {
    if (releaseWindowStatus(event, checkedAt) !== "monitoring") return false;
    const key = buildReleaseDeduplicationKey(event);
