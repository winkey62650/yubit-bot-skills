...
export function selectReleasableEvents(events, state, now) {
  ...
  const monitoredKeys = new Set(monitoredEvents.map(({ eventKey }) => eventKey));
  for (const previous of priorMonitored.values()) {
    if (!previous?.eventKey || monitoredKeys.has(previous.eventKey)) continue;
    if (releaseWindowStatus(previous, now) === "monitoring") {
...
export function pollDataReleaseUpdates(options = {}) {
  ...
  if ((typeof repository !== "object" && typeof repository !== "function") || repository === null) {
    return pollDataReleaseUpdatesUnlocked(options);
  }

  const previous = repositoryPollLocks.get(repository) ?? Promise.resolve();
  const current = previous
    .catch(() => undefined)
    .then(() => pollDataReleaseUpdatesUnlocked(options));
