...
// This lock serializes one Node scheduler process. Cross-process exclusion requires
// repository-level compare-and-swap or transactional support when that is available.
const repositoryPollLocks = new WeakMap();
const DELIVERY_RECEIPT_MAX_ENTRIES = 100;
const DELIVERY_RECEIPT_TTL_MS = 14 * 24 * 60 * 60_000;

function withRepositoryPollLock(repository, operation) {
  if ((typeof repository !== "object" && typeof repository !== "function") || repository === null) {
    return operation();
  }
  const previous = repositoryPollLocks.get(repository) ?? Promise.resolve();
  const current = previous.catch(() => undefined).then(operation);
  repositoryPollLocks.set(repository, current);
  return current.finally(() => {
    if (repositoryPollLocks.get(repository) === current) repositoryPollLocks.delete(repository);
  });
}
...
export function prepareDataReleaseDelivery({ repository, deduplicationKey, event, targetKeys = [], now = new Date() } = {}) {
  return withRepositoryPollLock(repository, () => mutateDeliveryReceipt(repository, deduplicationKey, event, now, (entry, stamp) => {
...
export function pollDataReleaseUpdates(options = {}) {
  ...
  return withRepositoryPollLock(repository, () => pollDataReleaseUpdatesUnlocked(options));
}
...
export function acknowledgeDataReleasePublished(options = {}) {
  const repository = options?.repository;
  return withRepositoryPollLock(repository, () => acknowledgeDataReleasePublishedUnlocked(options));
}
