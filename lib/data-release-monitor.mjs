...
function sourceManifestFor(calendarSources, candidates) {
  const manifest = [...(Array.isArray(calendarSources) ? calendarSources : []), ...candidates]
    .map(manifestRecord).filter(Boolean);
  const deduplicated = new Map();
  for (const record of manifest) deduplicated.set(`${record.id ?? ""}|${record.url ?? ""}`, record);
...
async function enforceOfficialEligibility(events, checkedAt, fetchOfficialActual, calendarSources, warnings) {
  ...
  for (const candidates of groupedEvents(events)) {
    ...
    if (releaseWindowStatus(event, checkedAt) !== "monitoring") {
      assessed.push({ event, selectionEvent: event, eligibility: null, sourceManifest: sourceManifestFor(calendarSources, []) });
      continue;
    }
    ...
    const actual = reconcileSourcedField(actualSources);
    let eligibility = validateDataReleasePublication({ actualSources });
    ...
    const sourceManifest = sourceManifestFor(calendarSources, actualSources);
...
async function pollDataReleaseUpdatesUnlocked(options = {}) {
  ...
  const blocked = assessments.find(({ eligibility, event }) => (
    eligibility && releaseWindowStatus(event, checkedAt) === "monitoring" && !eligibility.publishable
  ));
  if (blocked) {
