...
const META_LEASE_ACQUIRE_ATTEMPTS = 5_000;
...
async function withMetaLease(repository, key, locks, nowValue, operation, options = {}) {
  ...
  let acquired = null;
  for (let attempt = 0; attempt < META_LEASE_ACQUIRE_ATTEMPTS && !acquired; attempt += 1) {
...
function eventWithOfficialActual(event, actual, eligibility, sourceManifest) {
  const components = Array.isArray(actual?.components) ? clone(actual.components) : event?.components;
  return {
    ...event,
    ...(components ? { components } : {}),
...
function officialEvidenceIsFuture(records, checkedAt) {
  const queue = [...records];
  while (queue.length) {
    const record = queue.shift();
    if (!record) continue;
    if ([record.publishedAt, record.retrievedAt]
      .some((value) => instant(value)?.getTime() > checkedAt.getTime())) return true;
    if (Array.isArray(record.components)) queue.push(...record.components);
  }
  return false;
}

function canonicalOfficialComposite(event, record) {
  const expected = Array.isArray(event?.components) ? event.components : [];
  const supplied = Array.isArray(record?.components) ? record.components : [];
  if (!expected.length || expected.length !== supplied.length) return null;

  const expectedById = new Map();
  for (const component of expected) {
    const id = componentIdentity(component);
    if (!id || expectedById.has(id)) return null;
    expectedById.set(id, component);
  }
  const suppliedById = new Map();
  for (const component of supplied) {
    const id = componentIdentity(component);
    if (!id || suppliedById.has(id) || !expectedById.has(id)) return null;
    suppliedById.set(id, component);
  }
  if (suppliedById.size !== expectedById.size) return null;

  const components = [...expectedById.keys()].sort((left, right) => left.localeCompare(right, "en"))
    .map((id) => {
      const calendarComponent = expectedById.get(id);
      const suppliedComponent = suppliedById.get(id);
      const eligibility = validateDataReleasePublication({ actualSources: [suppliedComponent] });
      const actual = eligibility.publishable ? eligibility.actual : null;
      if (!actual || actual.authority !== "official" || actual.status !== "verified") return null;
      return {
        ...clone(calendarComponent),
        id,
        values: { ...(calendarComponent?.values ?? {}), actual: actual.value },
        rawValues: {
          ...(calendarComponent?.rawValues ?? {}),
  ...
  const value = normalizedActual({ components });
  if (!value) return null;
  return {
    ...clone(record),
    value,
...
function officialCompositeIsStale(event, records) {
  const parentSchedule = instant(event?.scheduledAt);
  return records.some((record) => Array.isArray(record?.components) && record.components.some((component) => {
    const componentSchedule = instant(component?.scheduledAt) ?? parentSchedule;
    const published = instant(component?.actualObservedAt ?? component?.actualSources?.[0]?.publishedAt);
    return Boolean(componentSchedule && published && published < componentSchedule);
  }));
}

function officialRecordSelectedBy(actual, records) {
  if (!actual) return null;
  const matches = records.filter((record) => (
    text(record?.sourceId) === text(actual.sourceId)
      && text(record?.sourceUrl) === text(actual.sourceUrl)
      && iso(record?.publishedAt) === iso(actual.publishedAt)
      && iso(record?.retrievedAt) === iso(actual.retrievedAt)
  ));
  if (matches.length <= 1) return matches[0] ?? null;
  return matches.find((record) => text(record?.value) === text(actual.value)) ?? matches[0];
}

function hydrateOfficialCompositeEvidence(actual, records) {
  const selected = officialRecordSelectedBy(actual, records);
  if (!selected || !Array.isArray(selected.components)) return actual;
  return {
    ...actual,
    value: selected.value,
    rawValue: selected.rawValue,
    unit: selected.unit,
    components: clone(selected.components),
    comparisons: (actual.comparisons ?? []).map((comparison) => {
      const record = officialRecordSelectedBy(comparison, records);
      return record && Array.isArray(record.components)
        ? { ...comparison, value: record.value, rawValue: record.rawValue, unit: record.unit }
        : comparison;
    }),
  };
}
...
async function enforceOfficialEligibility(events, checkedAt, fetchOfficialActual, calendarSources, warnings) {
  ...
  for (const candidates of groupedEvents(events)) {
    ...
    let officialEvidenceFailure = official.length && officialEvidenceIsFuture(official, checkedAt)
      ? "future-official-evidence"
      : null;
    if (!officialEvidenceFailure && Array.isArray(event?.components) && event.components.length && official.length) {
      const canonical = official.map((record) => canonicalOfficialComposite(event, record));
      if (canonical.some((record) => record === null)) {
        officialEvidenceFailure = "official-component-evidence-unavailable";
      } else {
        official = canonical;
        if (officialCompositeIsStale(event, official)) officialEvidenceFailure = "stale-actual";
      }
    }
    ...
    actual = hydrateOfficialCompositeEvidence(actual, official);
    ...
    if (officialEvidenceFailure) {
      eligibility = { ...eligibility, publishable: false, reason: officialEvidenceFailure };
    } else if (scheduleReason) eligibility = { ...eligibility, publishable: false, reason: scheduleReason };
    else if (stale) eligibility = { ...eligibility, publishable: false, reason: "stale-actual" };
    const componentSources = official.flatMap((record) => (
      Array.isArray(record?.components)
        ? record.components.flatMap((component) => component?.actualSources ?? [])
        : []
    ));
    const sourceManifest = sourceManifestFor(
      calendarSources,
      [...actualSources, ...componentSources],
      eligibility.actual ?? actual,
    );
...
async function pollDataReleaseUpdatesUnlocked(options = {}) {
  ...
  const eventFilter = typeof options.eventFilter === "function" ? options.eventFilter : () => true;
  const assessments = (await enforceOfficialEligibility(liveEvents, checkedAt, fetchOfficialActual, sources, warnings))
    .filter(({ event, selectionEvent }) => eventFilter(selectionEvent ?? event));
