...
function normalizeSocialPlatform(source = {}) {
  const platform = String(source.platform || "").trim().toLowerCase();
  if (platform === "x" || platform === "twitter") return "x";
  if (platform === "youtube") return "youtube";

  const url = String(source.accountUrl || source.feedUrl || "").trim().toLowerCase();
  if (/^https?:\/\/(?:www\.)?(?:x\.com|twitter\.com)(?:\/|$)/.test(url)) return "x";
  if (/^https?:\/\/(?:www\.)?(?:youtube\.com|youtu\.be)(?:\/|$)/.test(url)) return "youtube";
  return platform;
}

function summarizeReleaseSocialSource(item = {}) {
  const nestedSources = Array.isArray(item.sources) ? item.sources : [];
  const sources = nestedSources.length > 0
    ? nestedSources
    : (String(item.accountUrl || item.feedUrl || "").trim() ? [item] : []);
  return {
    id: item.id,
    name: item.name,
    enabled: item.enabled === true || item.status === "已启用",
    sourceCount: sources.length,
    xCount: sources.filter((source) => normalizeSocialPlatform(source) === "x").length,
    youtubeCount: sources.filter((source) => normalizeSocialPlatform(source) === "youtube").length,
  };
}
...
function evaluateTradingRelease(trading, {
  ...
  if (Number(metrics.enabledDestinations ?? 0) < 1) failures.push("尚未配置交易信号发布目标");
  return failures;
}
