...
export function collectTelegramChatCandidates(updates = [], savedGroups = []) {
  ...
  for (const group of savedGroups) {
    ...
    chats.set(chatId, {
      ...
      username: String(previous?.username || group?.username || ""),
...
export async function discoverTelegramChats({ token, botId, updates = [], savedGroups = [], telegram }) {
  ...
  for (const candidate of candidates.active) {
    try {
      ...
      const resolvedType = chat.type || candidate.type;
      const username = chat.username || candidate.username || "";
      const isForum = resolvedType === "supergroup" && chat.is_forum === true;
      groups.push({
        ...candidate,
        ...
        username,
        type: resolvedType,
        isPrivateChannel: resolvedType === "channel" && !username,
        ...
        permissions: {
          ...
          canPostMessages: member.status === "creator" || member.can_post_messages === true,
          canEditMessages: member.status === "creator" || member.can_edit_messages === true
        }
      });
    } catch (error) {
      groups.push({
        ...candidate,
        isPrivateChannel: candidate.type === "channel" && !candidate.username,
        membership: "unknown",
        isForum: false,
        canUseTopics: false,
...
export function mergeBotGroupDiscoveries(bots = [], options = {}) {
  ...
  for (const bot of bots) {
    for (const group of bot.groups || []) {
      ...
      previous.username = group.username || previous.username || "";
      previous.type = group.type || previous.type || "supergroup";
      previous.isPrivateChannel = group.isPrivateChannel === true || previous.isPrivateChannel === true;
  ...
  return [...chats.values()].map((chat) => {
    ...
    const adminBot = memberships.find((bot) => bot.name === "AdminBot");
    const speakerBot = memberships.find((bot) => bot.name === "SpeakerBot");
    const adminBotIdentityVerified = adminBot?.identityVerified === true;
    const adminBotIsAdmin = adminBot?.isAdmin === true;
    ...
    const adminBotReady = adminBotIdentityVerified
      && adminBotIsAdmin
      && forumReady
      && adminBotCanManageTopics
      && adminBotCanPinMessages
      && adminBotCanChangeInfo;
    const channelPublishingReady = isChannel
      && speakerBot?.identityVerified === true
      && speakerBot?.isAdmin === true
      && (speakerBot.membership === "creator" || speakerBot.permissions?.canPostMessages === true);
    const readyForInitialization = !isChannel && adminBotReady;
    const distributionReady = isChannel ? channelPublishingReady : readyForInitialization;
    const initializationBlockReason = isChannel
      ? !speakerBot?.isAdmin
        ? "SpeakerBot 必须是 Channel 管理员"
        : speakerBot?.membership !== "creator" && speakerBot?.permissions?.canPostMessages !== true
          ? "SpeakerBot 缺少 Post Messages 权限"
          : "Channel 可用于内容分发，无需 Topic 初始化"
      : !adminBotIdentityVerified
      ? "AdminBot Token 身份与配置不一致"
      : !adminBotIsAdmin
        ? "AdminBot 必须是目标群管理员"
        : !forumReady
        ...
        : !adminBotCanManageTopics
          ...
          : !adminBotCanPinMessages
            ...
            : !adminBotCanChangeInfo
              ...
              : "";
    const systemThreadIds = telegramSystemTopicThreadIds(chat.topics);
    const topics = chat.isForum === true
      ? mergeExpectedForumTopics(chat.topics, expectedForumTopics)
      : normalizeTopics(chat.topics);
    const detectedTopicThreadIds = normalizeThreadIds([
      ...(chat.detectedTopicThreadIds || []).filter((threadId) => !systemThreadIds.has(Number(threadId))),
    ...
    return {
      ...
      type: chat.type,
      isPrivateChannel: chat.type === "channel" && (chat.isPrivateChannel === true || !chat.username),
...
export function filterActiveTelegramGroups(groups = []) {
  return (Array.isArray(groups) ? groups : []).filter((group) => Number(group?.botCount || 0) > 0);
}
...
/**
 * Telegram reserves thread id 1 for a forum's General topic. It is a system
 * entry rather than one of the seven managed Academy topics, so it must never
 * affect operator counts or routing selectors.
 */
export function isTelegramSystemTopic(topic = {}) {
  const name = typeof topic === "string" ? topic : String(topic?.name || topic?.title || "");
  const explicitThreadId = typeof topic === "object"
    ? Number(topic?.threadId || topic?.message_thread_id || 0)
    : 0;
  return canonicalTopicName(name) === "general chat" || explicitThreadId === 1;
}

export function excludeTelegramSystemTopics(topics = []) {
  return (Array.isArray(topics) ? topics : []).filter((topic) => !isTelegramSystemTopic(topic));
}

export function telegramSystemTopicThreadIds(topics = []) {
  return new Set([1, ...excludeInvalidThreadIds(
    (Array.isArray(topics) ? topics : [])
      .filter((topic) => isTelegramSystemTopic(topic))
      .map((topic) => topic?.threadId || topic?.message_thread_id || 0)
  )]);
}
...
export function orderTopicsByTemplate(topics = [], expectedTopics = []) {
  const normalized = normalizeTopics(excludeTelegramSystemTopics(topics));
  // `expectedTopics` is already the editorial template order.  Do not pass it
  // through `normalizeTopics`: that helper sorts by thread id, while template
  // ids are semantic slot ids and intentionally differ from the forum thread
  // ids saved by Telegram.
  const expected = normalizeTopicsInInputOrder(expectedTopics);
...
export function mergeExpectedForumTopics(actualTopics, expectedTopics) {
  const actual = normalizeTopics(excludeTelegramSystemTopics(actualTopics));
  const expected = normalizeTopicsInInputOrder(expectedTopics).map((topic) => ({
    ...topic,
    id: topic.name,
    threadId: null,
    source: "template",
  ...
  const merged = expected.map((expectedTopic) => {
    ...
    return {
      ...actual[index],
      // Keep the verified Telegram thread id, but expose the current canonical
      // label in the operator UI.  Old groups may still contain labels such as
      // `2. Market Events` or `7. xxx's Trading Zone`; retaining those labels
      // makes the backend appear out of sync even though routing is correct.
      name: mergedManagedTopicName(actual[index].name, expectedTopic.name)
    };
  });
...
function mergedManagedTopicName(actualName, expectedName) {
  const actualCanonical = canonicalTopicName(actualName);
  const expectedSemantic = legacyTopicSequence(expectedName);
  const isUnnumberedAgentTradingZone = !topicSequence(actualName)
    && expectedSemantic === "2"
    && actualCanonical.endsWith(" trading zone")
    && actualCanonical !== "trading zone";

  return isUnnumberedAgentTradingZone ? actualName : expectedName;
}

function normalizeTopicsInInputOrder(topics = []) {
  const unique = new Map();
  for (const topic of Array.isArray(topics) ? topics : []) {
    const name = String(topic?.name || topic?.title || "").trim();
    if (!name) continue;
    const threadId = Number(topic?.threadId || topic?.message_thread_id || 0);
...
export function reconcileGroupWithSetupState(group = {}, setupState = {}, expectedTopics = []) {
  ...
  const systemThreadIds = telegramSystemTopicThreadIds(group?.topics || []);
  const actual = normalizeTopics(excludeTelegramSystemTopics(group?.topics || []));
  const managed = [];
  ...
  const detectedTopicThreadIds = normalizeThreadIds([
    ...(setupStateIsComplete
      ? []
      : (group?.detectedTopicThreadIds || []).filter((threadId) => (
        !suppressedThreadIds.has(Number(threadId)) && !systemThreadIds.has(Number(threadId))
      ))),
    ...topics.map((topic) => topic.threadId)
  ]);
  const resolvedCount = topics.filter((topic) => topic.threadId).length;
...
function legacyTopicSequence(value) {
  ...
  if (canonical === "trading zone" || canonical.endsWith(" trading zone")) return "2";
  if (canonical === "read first - disclaimer" || canonical === "community disclaimer") return "1";
  return "";
}

function normalizeThreadIds(values = []) {
  return [...new Set(excludeInvalidThreadIds(values))]
    .sort((a, b) => a - b);
}

function excludeInvalidThreadIds(values = []) {
  return values.map((value) => Number(value || 0)).filter((value) => Number.isInteger(value) && value > 0);
}
