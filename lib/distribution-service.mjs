...
function isApprovedDistributionTarget(target, env = process.env) {
  ...
  if (!distributionDemoOnly(env) || String(target?.chatId ?? "") === demoChatId) return true;
...
function isDesktopDemoTarget(target, env = process.env) {
  const demoChatId = String(env?.DEMO_TELEGRAM_CHAT_ID || DEFAULT_DEMO_CHAT_ID).trim();
  const threadId = Number(target?.threadId);
  return String(target?.chatId ?? "").trim() === demoChatId
...
function desktopStepsFromTelegramPlan(steps) {
  return (Array.isArray(steps) ? steps : []).flatMap((step) => {
    ...
    if (step?.method === "sendMessage" && step.payload?.text) {
      return [{ kind: "text", text: decodeTelegramHtml(step.payload.text) }];
    }
    return [];
  });
}
...
export async function desktopPublisherHealth(options = {}) {
  ...
  return {
    ...
    routingReady,
    ready: credentialsReady && routingReady && active,
    configured: credentialsReady,
    authorized: active,
    username: routing.username || "@Serenity_Crypto",
    userId: null,
    ...
    lastDeliveryAt: heartbeat.lastDeliveryAt || null,
    lastDeliveryStatus: heartbeat.lastDeliveryStatus || null,
...
export async function distributionOverview() {
  ...
  for (const storedRule of storedRules) {
    const labeled = repairAutomationTargetLabels(storedRule);
    const reconciled = reconcileDistributionRouting(labeled, groupConfig.groups);
...
export async function retryDistributionDelivery(deliveryId, options = {}) {
  ...
  if (desktopPublisherRequired(env)) {
    if (!isDesktopDemoTarget(delivery.target, env)) throw new Error("DESKTOP_PUBLISHER_DEMO_ONLY");
...
export async function claimDesktopPublisherDelivery(options = {}) {
  ...
  const pending = await repository.listDeliveries({ status: "pending", limit: Math.max(1, Number(options.limit) || 200) });
  const eligible = pending
    .filter((delivery) => isDesktopDemoTarget(delivery.target, env))
    .sort((left, right) => new Date(left.createdAt || 0).getTime() - new Date(right.createdAt || 0).getTime());

  for (const delivery of eligible) {
    const claimed = await repository.claimDelivery(delivery.id);
    if (!claimed) continue;
    const event = await repository.getEvent(claimed.eventId);
    const plan = (event?.payload?.deliveryPlans ?? []).find((entry) => endpointMatches(entry?.target, claimed.target));
    const steps = desktopStepsFromTelegramPlan(plan?.steps);
...
export async function completeDesktopPublisherDelivery(deliveryId, input = {}, options = {}) {
  ...
  if (!status) throw new Error("回写状态必须是 success 或 failed");
  const targetMessageIds = (Array.isArray(input.targetMessageIds) ? input.targetMessageIds : [])
    .map(Number)
  ...
  const updated = await repository.updateDelivery(delivery.id, {
    ...
    targetMessageIds,
    error: status === "success" ? null : String(input.error || "本机管理员发布失败"),
    deliveredAt: status === "success" ? now.toISOString() : null
  });
  const heartbeatPatch = {
    lastDeliveryAt: now.toISOString(),
    lastDeliveryStatus: status,
    lastError: status === "success" ? null : String(input.error || "本机管理员发布失败")
  };
  if (status === "success") heartbeatPatch.lastVerifiedAt = now.toISOString();
