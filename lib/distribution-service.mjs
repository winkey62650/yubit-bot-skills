...
export async function createDistributionEngine() {
...
export async function ensureAutomationSchedules(repository, now = new Date()) {
  ...
  const missing = rules.filter((rule) => rule.enabled && !rule.nextRunAt);
...
export async function backfillRule(ruleId, references, { preview = true } = {}) {
  const repository = await getDistributionRepository();
  const rule = await repository.getRule(ruleId);
  ...
  if (preview) return { preview: true, items, targets: rule.targets };
  const engine = new DistributionEngine({
    repository,
    forwardBotId: token("forward").split(":")[0],
    telegram: (method, payload) => telegramCall(token("forward"), method, payload)
  });
  const results = [];
  for (const [index, messageId] of messageIds.entries()) {
    ...
    const deliveries = await engine.deliverEvent(rule, event);
...
export async function retryDistributionDelivery(deliveryId) {
  const repository = await getDistributionRepository();
  ...
  try {
    const run = await runAutomationJob(jobId, {
      dryRun: false,
      force: true,
...
async function executeAutomationRule(rule, {
  ...
  try {
    ...
    await repository.updateEvent(event.id, { payload: { ...event.payload, preview: run.preview ?? null } });
...
export async function runDueDistributionJobs(now = new Date()) {
  const repository = await getDistributionRepository();
  await repository.cleanupExpired(now);
  await ensureLegacyDistributionMigration(repository);
  const rules = await repository.claimDueAutomationRules(now);
  const results = [];
  for (const rule of rules) {
    try {
      results.push(await executeAutomationRule(rule, { repository, now, trigger: "scheduled" }));
    } catch (error) {
