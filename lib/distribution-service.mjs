...
function automationPersistenceCircuitKey(repository, ruleId) {
  ...
  const scope = namespace
    ...
    : "global";
  return JSON.stringify([scope, String(ruleId)]);
}

function openAutomationPersistenceCircuit(repository, ruleId, diagnostic) {
  AUTOMATION_PERSISTENCE_CIRCUITS.set(
    automationPersistenceCircuitKey(repository, ruleId),
    { ...diagnostic, circuitBreakerOpen: true }
  );
}

function clearAutomationPersistenceCircuit(repository, ruleId) {
  AUTOMATION_PERSISTENCE_CIRCUITS.delete(automationPersistenceCircuitKey(repository, ruleId));
}

function getAutomationPersistenceCircuit(repository, ruleId) {
  return AUTOMATION_PERSISTENCE_CIRCUITS.get(automationPersistenceCircuitKey(repository, ruleId)) ?? null;
}
