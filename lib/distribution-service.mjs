...
async function executeAutomationRule(rule, {
  ...
  try {
    ...
    const nonPublishable = run.status === "duplicate" || run.status === "skipped";
    targetResults = Array.isArray(run.preview?.targetResults) ? run.preview.targetResults : [];
    const missingTargetResults = run.status === "success"
      ? targets.filter((target) => !targetResults.some((row) => row.target?.id === target.id
        || distributionTargetKey(row.target) === distributionTargetKey(target)))
      : [];
    const resultStatus = missingTargetResults.length
      ? (missingTargetResults.length === targets.length ? "failed" : "partial")
      : run.status;
    const resultError = missingTargetResults.length ? "AUTOMATION_TARGET_RESULT_MISSING" : null;
    try {
    ...
    if (nonPublishable) {
      return { ruleId: rule.id, status: run.status, run, telemetryPersisted: true };
    }
    for (const target of targets) {
      const targetResult = targetResults.find((row) => row.target?.id === target.id
        || distributionTargetKey(row.target) === distributionTargetKey(target));
      const missingTargetResult = run.status === "success" && !targetResult;
      const resolvedStatus = missingTargetResult
        ? "failed"
        : targetResult?.status ?? (run.status === "queued" ? "pending" : run.status);
      const externallySent = targetResult?.receiptFinalizationPending === true;
      const pending = resolvedStatus === "pending" && !externallySent;
      ...
      await repository.updateDelivery(delivery.id, {
        ...
        error: success ? null : (missingTargetResult ? "AUTOMATION_TARGET_RESULT_MISSING" : targetResult?.error ?? run.message ?? "自动发布失败"),
        deliveredAt: success ? new Date().toISOString() : null
      });
    }
    await saveAutomationBroadcastMappings(repository, targetResults);
    return { ruleId: rule.id, status: resultStatus, run, ...(resultError ? { error: resultError } : {}) };
  } catch (error) {
