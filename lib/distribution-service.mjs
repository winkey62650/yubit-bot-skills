...
function automationExecutionFingerprint(rule, targets) {
  return createHash("sha256").update(JSON.stringify({
    ...
    targets: targets.map(distributionTargetKey).sort()
  })).digest("hex");
}
...
async function resolveAutomationExecutionScope(repository, rule, targets, now, trigger) {
  ...
  const storedCurrent = stored?.generation
    && stored?.fingerprint === fingerprint
    && (!stored.expiresAt || Date.parse(stored.expiresAt) > now.getTime());
  if (storedCurrent && ["running", "partial", "failed", "manual-reconciliation"].includes(stored.status)) {
    return { generation: stored.generation, fingerprint, stateKey, state: stored };
  }
  const anchor = automationExecutionAnchor(rule, now, trigger);
  const generation = createHash("sha256")
    .update(JSON.stringify({ ruleId: rule.id, fingerprint, anchor }))
    .digest("hex")
    .slice(0, 24);
  return {
    generation,
    ...
    anchor,
    stateKey,
    state: storedCurrent && stored.generation === generation ? stored : null
  };
}
...
async function loadAutomationTargetReceipts(repository, ruleId, scope, now) {
  ...
  if (typeof repository?.getMeta === "function") {
    const stored = await repository.getMeta(`${AUTOMATION_TARGET_RECEIPTS_META_PREFIX}${ruleId}:${scope.generation}`);
    if (stored?.generation !== scope.generation
      || stored?.fingerprint !== scope.fingerprint
      || (stored?.expiresAt && Date.parse(stored.expiresAt) <= now.getTime())) return receipts;
...
async function executeAutomationRule(rule, {
  ...
  try {
    ...
    await releaseAutomationExecutionLease(repository, leaseState);
  }
}
