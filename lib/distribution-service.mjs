...
async function executeAutomationRule(rule, {
  ...
  try {
    ...
    targetResults = run.preview?.targetResults ?? [];
    for (const target of targets) {
      ...
      const resolvedStatus = targetResult?.status ?? (run.status === "queued" ? "pending" : run.status);
      const externallySent = targetResult?.receiptFinalizationPending === true;
      const pending = resolvedStatus === "pending" && !externallySent;
      const success = externallySent || (targetResult ? targetResult.status === "success" : run.status === "success");
...
async function settleDesktopReleaseReceipt(repository, delivery, now, succeeded) {
  ...
  if (!succeeded) {
    return releaseDataReleaseTargetClaim({ repository, deduplicationKey, targetKey, event, now });
  }
  const aggregate = await acknowledgeDataReleaseTarget({ repository, deduplicationKey, targetKey, event, now });
