...
const DESKTOP_PUBLISHER_LEASE_TTL_MS = 30 * 60_000;
...
export async function claimDesktopPublisherDelivery(options = {}) {
  ...
  try {
    ...
    const eligible = [...sending, ...pending]
...
export async function completeDesktopPublisherDelivery(deliveryId, input = {}, options = {}) {
  ...
  if (!await ownsDesktopPublisherLease(repository, leaseId, now)) throw new Error("DESKTOP_PUBLISHER_LEASE_INVALID");
  const status = ["progress", "success", "failed"].includes(input.status) ? input.status : "";
  if (!status) throw new Error("回写状态必须是 progress、success 或 failed");
  const { steps } = await desktopDeliveryPlan(repository, delivery);
  ...
  if (status === "progress") {
    ...
    return repository.updateDelivery(delivery.id, { status: "sending", publisherProgress: progress });
  }
  if (status === "success") {
  ...
  const targetMessageIds = progress.map((entry) => Number(entry.targetMessageId)).filter(Number.isFinite);
