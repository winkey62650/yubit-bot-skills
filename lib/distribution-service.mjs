...
// succeeded but every durable receipt/reconciliation write failed. This cannot
// provide cross-process exactly-once guarantees, so callers receive an explicit
// hard diagnostic until the rule can be durably disabled.
const AUTOMATION_PERSISTENCE_CIRCUITS = new WeakMap();

function automationPersistenceCircuitMap(repository) {
  if (!repository || (typeof repository !== "object" && typeof repository !== "function")) return null;
  let circuits = AUTOMATION_PERSISTENCE_CIRCUITS.get(repository);
  if (!circuits) {
    circuits = new Map();
    AUTOMATION_PERSISTENCE_CIRCUITS.set(repository, circuits);
  }
  return circuits;
}

function openAutomationPersistenceCircuit(repository, ruleId, diagnostic) {
  automationPersistenceCircuitMap(repository)?.set(String(ruleId), { ...diagnostic, circuitBreakerOpen: true });
}

function clearAutomationPersistenceCircuit(repository, ruleId) {
  const circuits = AUTOMATION_PERSISTENCE_CIRCUITS.get(repository);
  circuits?.delete(String(ruleId));
  if (circuits?.size === 0) AUTOMATION_PERSISTENCE_CIRCUITS.delete(repository);
}

function getAutomationPersistenceCircuit(repository, ruleId) {
  return AUTOMATION_PERSISTENCE_CIRCUITS.get(repository)?.get(String(ruleId)) ?? null;
}
