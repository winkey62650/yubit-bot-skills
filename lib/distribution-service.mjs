...
async function executeAutomationRule(rule, {
  ...
  let targetResults = [];
  const persistedReleaseTargets = new Set();
  ...
  try {
    ...
    for (const target of targets) {
      ...
      if (releaseTargetKey) persistedReleaseTargets.add(releaseTargetKey);
      if (pending) continue;
    ...
    for (const targetResult of targetResults) {
      ...
      if (targetResult?.status !== "pending" || !releaseTargetKey || persistedReleaseTargets.has(releaseTargetKey)) continue;
      try {
