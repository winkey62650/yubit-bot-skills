...
const AUTOMATION_TARGET_RECEIPTS_META_PREFIX = "automation-target-receipts-v1:";
...
function automationTargetReceiptKey(target) {
  return distributionTargetKey(target) || (target?.id ? `id:${String(target.id)}` : "");
}

function validAutomationMessageId(value) {
...
function automationReceiptMessageIds(receipt) {
  ...
  return [...new Set(candidates.filter(validAutomationMessageId))];
}

async function loadAutomationTargetReceipts(repository, ruleId) {
  const receipts = {};
  if (typeof repository?.getMeta === "function") {
    const stored = await repository.getMeta(`${AUTOMATION_TARGET_RECEIPTS_META_PREFIX}${ruleId}`);
    for (const [key, receipt] of Object.entries(stored?.targets ?? {})) {
      const messageIds = automationReceiptMessageIds(receipt);
      if (!key || !messageIds.length) continue;
      receipts[key] = { ...receipt, messageId: messageIds[0], messageIds };
    }
  }
  if (typeof repository?.listDeliveries === "function") {
    const deliveries = await repository.listDeliveries({ status: "success", limit: 1000 });
    for (const delivery of deliveries ?? []) {
      if (delivery?.ruleId !== ruleId || delivery?.status !== "success") continue;
      const key = automationTargetReceiptKey(delivery.target);
      const messageIds = automationReceiptMessageIds({
        messageId: delivery.targetMessageId,
        messageIds: delivery.targetMessageIds
      });
      if (!key || !messageIds.length) continue;
      receipts[key] = {
        targetKey: key,
...
async function persistAutomationTargetReceipts(repository, ruleId, receipts, now) {
  if (typeof repository?.setMeta !== "function") return false;
  await repository.setMeta(`${AUTOMATION_TARGET_RECEIPTS_META_PREFIX}${ruleId}`, {
    ruleId,
...
async function executeAutomationRule(rule, {
  ...
  const previousTargetReceipts = rule.runOnce
    ? await loadAutomationTargetReceipts(repository, rule.id)
    : {};
  ...
  try {
    ...
    const targetOutcomes = targets.map((target) => {
      const targetResult = targetResults.find((row) => row.target?.id === target.id
        || distributionTargetKey(row.target) === distributionTargetKey(target));
    ...
    if (rule.runOnce && targetOutcomes.some((outcome) => outcome.reliableSuccess)) {
      try {
        receiptStatePersisted = await persistAutomationTargetReceipts(repository, rule.id, currentTargetReceipts, stamp);
      } catch (error) {
    ...
    const persistTelemetry = () => repository.updateEvent(event.id, telemetryPatch);
    if (nonPublishable) {
      try {
        await persistTelemetry();
        return { ruleId: rule.id, status: resultStatus, run, telemetryPersisted: true };
      } catch (error) {
        return {
          ruleId: rule.id,
          status: resultStatus,
    ...
    try {
      for (const { target, targetResult, success, pending, messageIds, invalidSuccessReceipt } of targetOutcomes) {
        ...
        const delivery = await repository.createDelivery({
          ...
          ...(releaseReceipt && releaseTargetKey ? { payload: {
            releaseDeduplicationKey: releaseReceipt.deduplicationKey,
            releaseExpectedTargetKeys: releaseReceipt.expectedTargetKeys,
            releaseTargetKey,
