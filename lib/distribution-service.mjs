...
function automationExecutionFingerprint(rule, targets) {
  return automationHash({
    ...
    revision: {
      ...
      content: automationContentRevision(rule)
    },
...
async function persistAutomationExecutionState(repository, scope, status, now, patch = {}) {
  if (typeof repository?.setMeta !== "function") return false;
  await repository.setMeta(scope.stateKey, {
...
async function acquireAutomationExecutionLease(repository, ruleId, generation, now) {
  ...
  const key = `${AUTOMATION_EXECUTION_LEASE_META_PREFIX}${ruleId}:${generation}`;
  const lease = {
    leaseId: randomUUID(),
    generation,
...
async function renewAutomationExecutionLease(repository, leaseState, now = new Date()) {
  ...
  try {
    ...
    const renewed = await repository.renewMetaLease(leaseState.key, leaseState.lease.leaseId, leaseUntil);
...
function startAutomationExecutionHeartbeat(repository, leaseState, env) {
  ...
  const timer = setInterval(() => { void renewAutomationExecutionLease(repository, leaseState); }, automationExecutionHeartbeatMs(env));
  timer.unref?.();
  return timer;
}
...
async function persistAutomationTargetReceipts(repository, ruleId, scope, receipts, now) {
  if (typeof repository?.setMeta !== "function") return false;
...
async function queueAutomationTelemetry(repository, ruleId, eventId, patch, generation, error, now) {
  if (typeof repository?.setMeta !== "function") return false;
  const key = `${AUTOMATION_TELEMETRY_META_PREFIX}${ruleId}`;
  const current = typeof repository.getMeta === "function" ? await repository.getMeta(key) : null;
  const items = Array.isArray(current?.items) ? current.items : [];
  await repository.setMeta(key, {
    kind: "automation-telemetry-pending", ruleId, updatedAt: new Date(now).toISOString(),
    items: [...items.filter((item) => item.eventId !== eventId), { eventId, patch, generation, error }]
  });
  return true;
}

async function flushAutomationTelemetry(repository, ruleId) {
  if (typeof repository?.getMeta !== "function" || typeof repository?.updateEvent !== "function") return false;
  const key = `${AUTOMATION_TELEMETRY_META_PREFIX}${ruleId}`;
  const queued = await repository.getMeta(key);
  if (queued?.kind !== "automation-telemetry-pending" || !Array.isArray(queued.items)) return false;
  for (const item of queued.items) await repository.updateEvent(item.eventId, item.patch);
  if (typeof repository?.setMeta === "function") await repository.setMeta(key, null);
  return true;
}
...
async function saveAutomationBroadcastMappings(repository, targetResults) {
  const successful = (targetResults ?? []).filter((result) => result?.target?.platform !== "discord" && result?.status === "success" && Number(result.messageId));
...
async function executeAutomationRule(rule, {
  ...
  const expandedTargets = await expandAutomaticBroadcastTargets(repository, rule.targets);
  ...
  try {
    ...
    if (!(await renewAutomationExecutionLease(repository, leaseState, stamp))) {
    ...
    if (typeof repository?.setMeta === "function") {
      await persistAutomationExecutionState(repository, executionScope, "running", stamp);
    }
  ...
  try {
    const rawRun = await runner(jobId, {
      dryRun: false,
    ...
    if (!(await renewAutomationExecutionLease(repository, leaseState))) {
      try {
        await persistAutomationExecutionState(repository, executionScope, "manual-reconciliation", stamp, {
          reconciliationError: "AUTOMATION_EXECUTION_LEASE_LOST_AFTER_SEND"
        });
    ...
    if (nonPublishable) {
      return { ruleId: rule.id, status: resultStatus, run, ...(await persistTelemetry()) };
    }
    if (!(await renewAutomationExecutionLease(repository, leaseState))) {
      try {
        await persistAutomationExecutionState(repository, executionScope, "manual-reconciliation", stamp, {
          reconciliationError: "AUTOMATION_EXECUTION_LEASE_LOST_BEFORE_RECEIPT_PERSISTENCE"
        });
      } catch { /* The returned state remains fail-closed. */ }
      try { await blockAutomationRuleForReconciliation(repository, rule); } catch { /* Best effort secondary durability. */ }
      return {
...
export async function resetAutomationManualReconciliation(ruleId, options = {}) {
  ...
  const now = new Date(options.now ?? Date.now());
  const stateKey = `${AUTOMATION_EXECUTION_STATE_META_PREFIX}${rule.id}`;
  const state = typeof repository?.getMeta === "function" ? await repository.getMeta(stateKey) : null;
