...
async function executeAutomationRule(rule, {
  ...
  const targets = await hydrateDestinationCtas(repository, allowedDistributionTargets(expandedTargets, env));
  if (!targets.length) throw new Error("DEMO_ONLY_TEST_POLICY");
  ...
  try {
    ...
    const targetOutcomes = targets.map((target) => {
      ...
      const externallySent = targetResult?.receiptFinalizationPending === true;
      const success = externallySent || targetResult?.status === "success";
      const pending = !externallySent && ["pending", "queued"].includes(targetResult?.status);
      const messageIds = targetResult?.messageIds ?? (targetResult?.messageId != null ? [targetResult.messageId] : []);
      return {
        target,
        targetResult,
        success,
        pending,
        reliableSuccess: success && (externallySent || messageIds.length > 0)
      };
    });
    ...
    if (!validRunStatus) {
      ...
      if (missingTargetResults.length) resultError = "AUTOMATION_TARGET_RESULT_MISSING";
    ...
    try {
      for (const { target, targetResult, success, pending } of targetOutcomes) {
        ...
        if (pending) continue;
        const deliveryError = resultError === "AUTOMATION_RUN_STATUS_INVALID"
          ? resultError
          : missingTargetResult
        ...
        await repository.updateDelivery(delivery.id, {
          ...
          targetMessageIds: targetResult?.messageIds ?? (targetResult?.messageId != null ? [targetResult.messageId] : []),
      ...
      const reliableExternalSuccess = resultStatus === "success"
        && targetOutcomes.length > 0
        && targetOutcomes.every((outcome) => outcome.reliableSuccess);
