...
import { hydrateDestinationCtas, requireSavedMarketEventCtas } from "./destination-cta.mjs";
...
function shouldArchiveExpiredDesktopDelivery(delivery, now, env = process.env) {
  if (delivery?.payload?.socialLiveExpiresAt && delivery.status === "pending") {
    return Date.parse(delivery.payload.socialLiveExpiresAt) <= now.getTime();
  }
...
export async function validateRuleRuntime(ruleId, options = {}) {
  ...
  for (const target of rule.targets) {
    ...
    if (!isApprovedDistributionTarget(target, env)) {
      checks.push({
        key: `target:${target.id}:policy`,
        ok: false,
        message: `${target.topicName || "目标 Topic"} 未纳入生产允许列表，定时任务不会发送`
      });
      continue;
    }
...
export async function retryDistributionDelivery(deliveryId, options = {}) {
  ...
  if (delivery.payload?.socialLiveSourceId) throw new Error("直播提醒具有时效性，请在社媒直播检查中核对，不可重放历史通知");
  if (delivery.payload?.socialPostSourceId) throw new Error("社媒更新已保存逐条回执，请先核对平台记录，不可重放历史通知");
...
async function executeAutomationRule(rule, {
  ...
  const hydratedTargets = await hydrateDestinationCtas(
    repository,
    allowedDistributionTargets(expandedTargets, env),
  );
  const enforceSavedCtas = env.NODE_ENV === "production"
    || hydratedTargets.some((target) => target?.ctaSource === "destination-registry");
  const configuredTargets = enforceSavedCtas
    ? requireSavedMarketEventCtas(jobId, hydratedTargets)
    : hydratedTargets;
...
export async function runDistributionAutomationRule(ruleId, options = {}) {
  ...
  return executeAutomationRule(rule, {
    ...
    demoAcceptanceBatchId: options.demoAcceptanceBatchId,
