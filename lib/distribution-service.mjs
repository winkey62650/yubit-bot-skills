  ...
  try {
    ...
    try {
      ...
      if (!receiptStatePersisted) {
        let telemetryRecoveryError = null;
        if (resultStatus === "success") {
          try {
            await repository.updateEvent(event.id, telemetryPatch);
            deliveryFailureTelemetry = { telemetryPersisted: true, telemetryQueued: false };
          } catch (telemetryError) {
            telemetryRecoveryError = telemetryError;
          }
        }
        if (!deliveryFailureTelemetry) {
          ...
          try {
            await persistAutomationExecutionState(repository, executionScope, resultStatus, stamp, {
              phase: "uncertain",
              attemptToken,
              reconciliationError: resultError,
              expiresAt: null
            }, { generation: executionScope.generation, attemptToken, status: priorResultStatus });
          } catch { /* The confirmed execution state still prevents generation reuse. */ }
          try { await blockAutomationRuleForReconciliation(repository, rule); } catch { /* Execution state is the primary fence. */ }
          deliveryFailureTelemetry = await persistTelemetry();
          if (telemetryRecoveryError && deliveryFailureTelemetry.telemetryPersisted) {
            deliveryFailureTelemetry.telemetryRecoveredFrom = telemetryRecoveryError.message;
          }
        }
      }
      return {
