...
function daily(e, model, masterArtwork) {
  ...
  stories.forEach((story, index) => {
    const x = xs[index];
    items.push(
      field(e, `daily-${index + 1}-category`, usable(story.title) ? dailyCategory(story, index) : "", { x: x + 58, y: 244, w: 225, h: 25 }, { background: DAILY_SURFACE, size: 15, weight: 650, letterSpacing: .8, max: 28 }),
      field(e, `daily-${index + 1}-title`, (usable(story.posterTitle) ? posterCopy(story.posterTitle) : compactTitle(story.title)).toUpperCase(), { x, y: 279, w: 292, h: 48 }, { background: DAILY_SURFACE, size: 23, lineHeight: 1.02, weight: 900, fontFamily: TYPE.display, letterSpacing: -.25, align: "flex-start", padding: "1px 5px", max: 42 }),
      field(e, `daily-${index + 1}-thesis`, usable(story.posterThesis) ? posterCopy(story.posterThesis) : compact(story.thesis, "", { words: 10, chars: 64 }), { x, y: 329, w: 278, h: 61 }, { background: DAILY_SURFACE, size: 15, lineHeight: 1.24, weight: 550, color: COLORS.ink, align: "flex-start", padding: "3px 5px", max: 64 }),
