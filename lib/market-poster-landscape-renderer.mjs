...
function weekly(e, model, masterArtwork) {
  ...
  columns.forEach((column, index) => {
    ...
    items.push(
      ...
      field(e, `weekly-${index + 1}-event`, hasEvent ? (usable(event.posterTitle) ? posterCopy(event.posterTitle) : compact(event.title, "", { words: 5, chars: 34 })) : "", { x: xs[index] + 7, y: 274, w: 144, h: 54 }, { background: "#FFFFFF", size: 12, lineHeight: 1.17, align: "flex-start", padding: "3px 4px", max: 38 }),
      ...
      field(e, `weekly-${index + 1}-markets`, hasEvent && usable(event.posterMarkets || event.markets || event.affected) ? `MARKETS · ${posterCopy(event.posterMarkets || clean(event.markets || event.affected, "", 23)).toUpperCase()}` : "", { x: xs[index] + 7, y: 391, w: 144, h: 43 }, { background: "#FFFFFF", size: 10, lineHeight: 1.15, align: "flex-start", max: 32 }),
      field(e, `weekly-${index + 1}-sensitivity`, hasEvent ? (usable(event.posterSensitivity) ? posterCopy(event.posterSensitivity) : compact(event.sensitivity, "", { words: 9, chars: 58 })) : "", { x: xs[index] + 7, y: 442, w: 144, h: 48 }, { background: "#FFFFFF", size: 10, lineHeight: 1.16, color: COLORS.muted, align: "flex-start", max: 58 }),
  ...
  items.push(
    field(e, "weekly-key-risk", usable(priority?.posterSensitivity) ? posterCopy(priority.posterSensitivity) : compact(priority?.sensitivity, "", { words: 12, chars: 76 }), { x: 118, y: 570, w: 450, h: 38 }, { size: 14, lineHeight: 1.22, weight: 650, max: 76 }),
