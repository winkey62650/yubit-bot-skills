...
function field(e, key, value, rect, options = {}) {
  const rendered = clean(value, options.fallback || "", options.max || 100);
  return e("div", {
    key,
...
function uniqueAffected(stories) {
  const seen = new Set();
  return stories.map((story) => firstAffected(story?.affected, ""))
...
function daily(e, model, masterArtwork) {
  ...
  stories.forEach((story, index) => {
    const x = xs[index];
    items.push(
      field(e, `daily-${index + 1}-category`, clean(story.source, "", 28).toUpperCase(), { x: x + 58, y: 244, w: 225, h: 25 }, { size: 14, letterSpacing: 1.1, max: 28 }),
      field(e, `daily-${index + 1}-title`, compactTitle(story.title).toUpperCase(), { x, y: 281, w: 272, h: 42 }, { size: 19, lineHeight: 1.04, weight: 900, align: "flex-start", padding: "1px 5px", max: 42 }),
      field(e, `daily-${index + 1}-thesis`, compact(story.thesis, "", { words: 10, chars: 64 }), { x, y: 326, w: 265, h: 58 }, { size: 13, lineHeight: 1.32, weight: 600, color: COLORS.ink, align: "flex-start", padding: "3px 5px", max: 64 }),
...
function weekly(e, model, masterArtwork) {
  ...
  columns.forEach((column, index) => {
    ...
    items.push(
      field(e, `weekly-day-${index + 1}-mask`, "", { x: xs[index], y: 225, w: 158, h: 307 }, { background: "#FFFFFF", border: "1px solid #DCE7F2", radius: 6, size: 1 }),
      field(e, `weekly-${index + 1}-day`, day, { x: xs[index] + 6, y: 233, w: 58, h: 30 }, { background: "#C98A17", color: "#FFFFFF", size: 14, justify: "center", radius: 4, max: 5 }),
      field(e, `weekly-${index + 1}-date`, date, { x: xs[index] + 111, y: 233, w: 38, h: 30 }, { background: "#FFFFFF", size: 20, color: COLORS.gold, justify: "center", max: 2 }),
      field(e, `weekly-${index + 1}-event`, hasEvent ? compact(event.title, "", { words: 5, chars: 34 }) : "", { x: xs[index] + 7, y: 274, w: 144, h: 54 }, { background: "#FFFFFF", size: 12, lineHeight: 1.17, align: "flex-start", padding: "3px 4px", max: 38 }),
      field(e, `weekly-${index + 1}-time`, hasEvent ? eventTime : "", { x: xs[index] + 7, y: 331, w: 144, h: 24 }, { background: "#FFFFFF", color: COLORS.blue, size: 10, max: 18 }),
      field(e, `weekly-${index + 1}-impact`, hasEvent ? `IMPACT · ${impact}` : "", { x: xs[index] + 7, y: 361, w: 144, h: 25 }, { background: "#FFFFFF", color: statusColor(impact), size: 10, max: 20 }),
      field(e, `weekly-${index + 1}-markets`, hasEvent && usable(event.markets || event.affected) ? `MARKETS · ${clean(event.markets || event.affected, "", 23).toUpperCase()}` : "", { x: xs[index] + 7, y: 391, w: 144, h: 43 }, { background: "#FFFFFF", size: 10, lineHeight: 1.15, align: "flex-start", max: 32 }),
      field(e, `weekly-${index + 1}-sensitivity`, hasEvent ? compact(event.sensitivity, "", { words: 9, chars: 58 }) : "", { x: xs[index] + 7, y: 442, w: 144, h: 48 }, { background: "#FFFFFF", size: 10, lineHeight: 1.16, color: COLORS.muted, align: "flex-start", max: 58 }),
      field(e, `weekly-${index + 1}-status`, hasEvent ? "STATUS · FOCUS" : "", { x: xs[index] + 7, y: 498, w: 144, h: 27 }, { background: "#FFFFFF", size: 10, color: COLORS.blue, max: 20 }),
    );
  });
  ...
  items.push(
    field(e, "weekly-key-risk", compact(priority?.sensitivity, "", { words: 12, chars: 76 }), { x: 118, y: 570, w: 450, h: 38 }, { size: 14, lineHeight: 1.22, weight: 650, max: 76 }),
...
function dataFlash(e, model, masterArtwork) {
  ...
  const items = [
    ...
    field(e, "flash-subtitle", compact(model.title, "VERIFIED RELEASE SNAPSHOT", { words: 7, chars: 42 }).toUpperCase(), { x: 165, y: 251, w: 465, h: 25 }, { size: 14, color: COLORS.blue, justify: "center", max: 42 }),
    ...
    field(e, "flash-forecast", clean(model.forecast, "", 16), { x: 232, y: 314, w: 194, h: 70 }, { size: String(model.forecast ?? "").length > 8 ? 32 : 48, justify: "center", max: 16 }),
    field(e, "flash-previous", clean(model.previous, "", 16), { x: 446, y: 314, w: 193, h: 70 }, { size: String(model.previous ?? "").length > 8 ? 32 : 48, justify: "center", max: 16 }),
  ...
  reactions.forEach((reaction, index) => {
    ...
    items.push(
      field(e, `flash-reaction-${index + 1}-symbol`, available ? clean(reaction.symbol, "", 10).toUpperCase() : "", { x: 731, y, w: 72, h: 28 }, { size: 14, max: 10 }),
      field(e, `flash-reaction-${index + 1}-mask`, "", { x: 812, y: y - 2, w: 360, h: 34 }, { size: 1 }),
      field(e, `flash-reaction-${index + 1}-observation`, available ? "VERIFIED WINDOW" : "", { x: 818, y: y - 2, w: 192, h: 34 }, { size: 10, color: COLORS.muted, justify: "center", max: 28 }),
      field(e, `flash-reaction-${index + 1}-value`, available ? reaction.label : "", { x: 1012, y: y - 2, w: 80, h: 34 }, { size: 16, color: available ? statusColor(reaction.label) : COLORS.muted, justify: "center", max: 12 }),
      field(e, `flash-reaction-${index + 1}-status`, available ? clean(reaction.status, "OBSERVED", 12).toUpperCase() : "", { x: 1103, y: y - 2, w: 69, h: 34 }, { size: 12, color: available ? statusColor(reaction.label) : COLORS.muted, justify: "center", max: 12 }),
    );
  });
  ...
  items.push(
    ...
    field(e, "flash-desk-view", compact(model.verdict || model.title, "", { words: 14, chars: 82 }), { x: 698, y: 444, w: 365, h: 58 }, { size: 15, lineHeight: 1.24, weight: 650, align: "flex-start", max: 82 }),
...
function followUp(e, model, masterArtwork) {
  ...
  const status = clean(model.tapeStatus || model.verdictStatus, "", 24).toUpperCase();
  ...
  reactions.forEach((reaction, index) => {
    ...
    const available = usable(reaction?.symbol) && usable(reaction?.label);
    items.push(
      field(e, `follow-reaction-${index + 1}-symbol`, available ? clean(reaction.symbol, "", 10).toUpperCase() : "", { x: 81, y, w: 75, h: 30 }, { size: 15, max: 10 }),
      field(e, `follow-reaction-${index + 1}-value`, available ? reaction.label : "", { x: 169, y, w: 72, h: 30 }, { size: available ? 17 : 11, color: available ? statusColor(reaction.label) : COLORS.muted, justify: "center", max: 12 }),
      field(e, `follow-reaction-${index + 1}-status`, available ? clean(reaction.status, "OBSERVED", 12).toUpperCase() : "", { x: 248, y, w: 88, h: 30 }, { size: 12, color: available ? statusColor(reaction.label) : COLORS.muted, justify: "center", max: 12 }),
      field(e, `follow-reaction-${index + 1}-window`, available ? "START → CURRENT" : "", { x: 356, y: y - 2, w: 306, h: 35 }, { size: 10, color: COLORS.muted, justify: "center", max: 34 }),
    );
  });

  const changes = [
    compact(model.posterVerdict || model.verdict, "", { words: 7, chars: 44 }),
    compact(model.posterConfirmation || model.confirmation, "", { words: 7, chars: 44 }),
    compact(model.posterInvalidation || model.invalidation, "", { words: 7, chars: 44 }),
  ];
  ...
  items.push(
    field(e, "follow-volatility", usable(model.volatility) ? `VOLATILITY ${clean(model.volatility, "", 14).toUpperCase()}` : "", { x: 74, y: 440, w: 135, h: 52 }, { size: 13, color: COLORS.gold, max: 26 }),
    field(e, "follow-volume", usable(model.volume) ? `VOLUME ${clean(model.volume, "", 14).toUpperCase()}` : "", { x: 307, y: 440, w: 110, h: 52 }, { size: 13, color: COLORS.green, max: 26 }),
    field(e, "follow-breadth", usable(model.breadth) ? `BREADTH ${clean(model.breadth, "", 14).toUpperCase()}` : "", { x: 528, y: 440, w: 105, h: 52 }, { size: 13, color: COLORS.gold, max: 26 }),
    field(e, "follow-conclusion", compact(model.posterVerdict || model.verdict, "", { words: 13, chars: 76 }), { x: 725, y: 432, w: 250, h: 69 }, { size: 14, lineHeight: 1.22, weight: 650, align: "flex-start", max: 76 }),
    ...
    field(e, "follow-confidence", reactions.filter((item) => usable(item?.symbol) && usable(item?.label)).length >= 3 ? "CONFIDENCE MEDIUM" : "", { x: 900, y: 497, w: 105, h: 55 }, { size: 11, color: COLORS.gold, justify: "center", textAlign: "center", max: 24 }),
