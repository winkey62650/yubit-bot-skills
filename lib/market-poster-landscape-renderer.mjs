...
const EMPTY_UPDATE = "";
const DAILY_SURFACE = "#ECEFF3";
const DAILY_FADE = "linear-gradient(90deg, rgba(236, 239, 243, 1) 0%, rgba(236, 239, 243, 1) 86%, rgba(236, 239, 243, 0.94) 94%, rgba(236, 239, 243, 0) 100%)";
...
function dailyCategory(story, index) {
  if (usable(story?.posterCategory)) return clean(story.posterCategory, "", 28).toUpperCase();
  const text = `${story?.title || ""} ${story?.thesis || ""} ${story?.affected || ""}`.toUpperCase();
  if (/ETF|INSTITUTION|FUND FLOW|INFLOW|OUTFLOW/.test(text)) return "ETF / INSTITUTIONAL";
  if (/FED|FOMC|CPI|PCE|GDP|RATE|YIELD|DXY|DOLLAR|LIQUIDITY|MACRO/.test(text)) return "MACRO / POLICY";
  if (/SEC|CFTC|REGULAT|LEGISLAT|COURT|COMPLIANCE/.test(text)) return "POLICY / REGULATION";
  if (/TOKEN|PROTOCOL|PROJECT|UPGRADE|MAINNET|UNLOCK/.test(text)) return "MARKET / PROJECT";
  return ["MARKET / STRUCTURE", "MACRO / POLICY", "MARKET / PROJECT"][index] || "MARKET / STRUCTURE";
}

function dailyWatchLabel(story, index) {
  if (usable(story?.posterWatch)) return clean(story.posterWatch, "", 18).toUpperCase();
  const text = `${story?.title || ""} ${story?.thesis || ""} ${story?.affected || ""}`.toUpperCase();
  if (/ETF|FUND FLOW|INFLOW|OUTFLOW/.test(text)) return "ETF FLOW";
  if (/DXY|DOLLAR|RATE|YIELD|FED|FOMC|CPI|PCE|GDP/.test(text)) return "DXY / RATES";
  if (/BTC|BITCOIN/.test(text)) return "BTC STRUCTURE";
  if (/ETH|ETHEREUM/.test(text)) return "ETH STRUCTURE";
  if (/ALTCOIN|BREADTH|SECTOR/.test(text)) return "ALTCOIN BREADTH";
  const asset = firstAffected(story?.affected, "");
  return asset ? `${asset} WATCH` : ["MARKET FLOW", "MACRO TAPE", "BREADTH"][index];
}

function dailyAssetWatchLabel(asset) {
  const symbol = clean(asset, "", 12).toUpperCase();
  if (symbol === "BTC") return "BTC STRUCTURE";
  if (symbol === "ETH") return "ETH STRUCTURE";
  if (/^(?:DXY|US2Y|US10Y|YIELDS?)$/.test(symbol)) return `${symbol} / RATES`;
  if (symbol === "ALTCOINS") return "ALTCOIN BREADTH";
  return symbol ? `${symbol} WATCH` : "";
}
...
function daily(e, model, masterArtwork) {
  ...
  const xs = [35, 424, 813];
  const replay = model.historicalReplay === true;
  const items = [
    field(e, "daily-date", `${replay ? "REPLAY" : "DATE"} · ${clean(model.date, "UTC", 26).replace(/UTC/gi, "").replace(/·/g, "").trim()}`, { x: 56, y: 174, w: 202, h: 31 }, { background: COLORS.dark, color: "#FFFFFF", size: 14, max: 28 }),
    ...
    field(e, "daily-watch", usable(watch.title) ? `${firstAffected(watch.affected, "")} · ${clean(watch.impact, "WATCH", 12).toUpperCase()}` : "TAPE · WATCH", { x: 792, y: 178, w: 146, h: 29 }, { background: "#1F5E9C", color: "#FFFFFF", size: 13, justify: "center", radius: 12, max: 24 }),
    field(e, "daily-market-mask", "", { x: 986, y: 178, w: 157, h: 29 }, { background: "#5090CD", size: 1, radius: 12 }),
    field(e, "daily-market", `MARKET · ${replay ? "REPLAY" : clean(model.primaryBias, "MIXED", 12).toUpperCase()}`, { x: 986, y: 178, w: 157, h: 29 }, { background: "#5090CD", color: "#FFFFFF", size: 13, justify: "center", radius: 12, max: 26 }),
  ];

  stories.forEach((story, index) => {
    const x = xs[index];
    items.push(
      field(e, `daily-${index + 1}-category-mask`, "", { x: x + 58, y: 244, w: 225, h: 25 }, { background: "transparent", backgroundImage: DAILY_FADE, size: 1, radius: 0 }),
      field(e, `daily-${index + 1}-title-mask`, "", { x, y: 279, w: 292, h: 48 }, { background: "transparent", backgroundImage: DAILY_FADE, size: 1, radius: 0 }),
      field(e, `daily-${index + 1}-thesis-mask`, "", { x, y: 329, w: 260, h: 61 }, { background: "transparent", backgroundImage: DAILY_FADE, size: 1, radius: 0 }),
      field(e, `daily-${index + 1}-category`, usable(story.title) ? dailyCategory(story, index) : "", { x: x + 58, y: 244, w: 215, h: 25 }, { background: "transparent", size: 15, weight: 650, letterSpacing: .8, max: 28 }),
      field(e, `daily-${index + 1}-title`, (usable(story.posterTitle) ? posterCopy(story.posterTitle) : compactTitle(story.title)).toUpperCase(), { x, y: 279, w: 278, h: 48 }, { background: "transparent", size: 23, lineHeight: 1.02, weight: 900, fontFamily: TYPE.display, letterSpacing: -.25, align: "flex-start", padding: "1px 5px", max: 42 }),
      field(e, `daily-${index + 1}-thesis`, usable(story.posterThesis) ? posterCopy(story.posterThesis) : compact(story.thesis, "", { words: 8, chars: 52 }), { x, y: 329, w: 248, h: 61 }, { background: "transparent", size: 15, lineHeight: 1.24, weight: 550, color: COLORS.ink, align: "flex-start", padding: "3px 5px", max: 52 }),
      field(e, `daily-${index + 1}-impact`, usable(story.title) ? `IMPACT · ${clean(story.impact, "NEUTRAL", 14).toUpperCase()}` : "", { x: x + 8, y: 479, w: 190, h: 29 }, { background: DAILY_SURFACE, color: statusColor(story.impact), size: 13, letterSpacing: .45, max: 24 }),
      field(e, `daily-${index + 1}-status`, usable(story.title) ? (index === 0 ? `CONFIDENCE · ${Number(story.score) >= 75 ? "HIGH" : "MEDIUM"}` : index === 1 ? "STATUS · MONITOR" : "RISK · MODERATE") : "", { x: x + 181, y: 479, w: 170, h: 29 }, { background: DAILY_SURFACE, color: COLORS.gold, size: 13, letterSpacing: .35, max: 28 }),
    );
  });

  items.push(
    field(e, "daily-read", compact(model.summary || model.read, "Confirmation still matters before conviction rises.", { words: 12, chars: 76 }), { x: 136, y: 559, w: 420, h: 48 }, { background: DAILY_SURFACE, size: 18, lineHeight: 1.18, weight: 600, align: "flex-start", padding: "2px 4px", max: 76 }),
    field(e, "daily-watch-1-mask", "", { x: 661, y: 565, w: 124, h: 30 }, { background: DAILY_SURFACE, size: 1 }),
    field(e, "daily-watch-2-mask", "", { x: 827, y: 565, w: 139, h: 30 }, { background: DAILY_SURFACE, size: 1 }),
    field(e, "daily-watch-3-mask", "", { x: 1007, y: 565, w: 137, h: 30 }, { background: DAILY_SURFACE, size: 1 }),
    ...stories.map((story, index) => field(e, `daily-watch-${index + 1}`, assets[index] ? dailyAssetWatchLabel(assets[index]) : usable(story.title) ? dailyWatchLabel(story, index) : "", { x: [665, 841, 1014][index], y: 565, w: [120, 125, 130][index], h: 30 }, { background: DAILY_SURFACE, size: 14, max: 18 })),
    field(e, "daily-source", `SOURCE · ${sourceLine(model)}`, { x: 74, y: 629, w: 310, h: 26 }, { background: DAILY_SURFACE, size: 13, color: COLORS.muted, max: 44 }),
  );
