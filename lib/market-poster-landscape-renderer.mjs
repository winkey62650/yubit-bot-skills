...
const EMPTY_UPDATE = "NO MAJOR EVENT";
...
function compact(value, fallback = "", { words = 10, chars = 72 } = {}) {
  ...
  let result = firstThought.split(/\s+/).slice(0, words).join(" ");
  if (result.length > chars) {
    const clipped = result.slice(0, chars + 1);
    result = clipped.slice(0, Math.max(clipped.lastIndexOf(" "), chars - 12)).trim();
  }
...
function compactTitle(value, fallback = EMPTY_UPDATE) {
  ...
  return compact(keywords.join(" "), fallback, { words: 5, chars: 34 });
}
...
function daily(e, model, masterArtwork) {
  ...
  stories.forEach((story, index) => {
    ...
    items.push(
      ...
      field(e, `daily-${index + 1}-title`, compactTitle(story.title).toUpperCase(), { x, y: 281, w: 272, h: 42 }, { size: 19, lineHeight: 1.04, weight: 900, align: "flex-start", padding: "1px 5px", max: 34 }),
      field(e, `daily-${index + 1}-thesis`, compact(story.thesis, "Confirmation is still required.", { words: 10, chars: 64 }), { x, y: 326, w: 265, h: 58 }, { size: 13, lineHeight: 1.32, weight: 600, color: COLORS.ink, align: "flex-start", padding: "3px 5px", max: 64 }),
...
function weekly(e, model, masterArtwork) {
  ...
  columns.forEach((column, index) => {
    ...
    items.push(
      ...
      field(e, `weekly-${index + 1}-event`, hasEvent ? `${clean(event.time, "TIME", 8)} · ${compact(event.title, "VERIFIED EVENT", { words: 5, chars: 32 })}` : EMPTY_UPDATE, { x: xs[index] - 5, y: 261, w: 207, h: 34 }, { size: hasEvent ? 12 : 11, lineHeight: 1.12, align: "flex-start", padding: "3px 9px", max: 44 }),
      field(e, `weekly-${index + 1}-impact`, hasEvent ? `IMPACT · ${impact}` : "", { x: xs[index], y: 294, w: 202, h: 38 }, { color: statusColor(impact), size: 12, max: 24 }),
      field(e, `weekly-${index + 1}-markets`, hasEvent && usable(event.markets || event.affected) ? `MARKETS · ${clean(event.markets || event.affected, "", 25).toUpperCase()}` : "", { x: xs[index], y: 327, w: 202, h: 31 }, { size: 11, max: 34 }),
      field(e, `weekly-${index + 1}-status`, hasEvent ? `STATUS · ${Number(event.importance) >= 3 || event.isPriority ? "FOCUS" : "MONITOR"}` : "", { x: xs[index] + 31, y: 505, w: 151, h: 27 }, { size: 13, color: hasEvent ? COLORS.blue : COLORS.muted, max: 24 }),
    );
  });
  ...
  items.push(
    field(e, "weekly-key-risk", compact(priority?.sensitivity, "No major verified catalyst this week.", { words: 12, chars: 76 }), { x: 118, y: 570, w: 450, h: 38 }, { size: 14, lineHeight: 1.22, weight: 650, max: 76 }),
...
function dataFlash(e, model, masterArtwork) {
  ...
  const items = [
    ...
    field(e, "flash-surprise", usable(model.surprise) ? `SURPRISE · ${clean(model.surprise, "", 18).toUpperCase()}` : !usable(model.forecast) && !usable(model.previous) ? "COMPARISON · UNAVAILABLE" : "", { x: 93, y: 400, w: 230, h: 25 }, { size: 13, color: statusColor(impact), justify: "center", max: 30 }),
  ...
  items.push(
    ...
    field(e, "flash-desk-view", compact(model.verdict || model.title, `${impact} read; confirmation remains necessary.`, { words: 14, chars: 82 }), { x: 698, y: 444, w: 365, h: 58 }, { size: 15, lineHeight: 1.24, weight: 650, align: "flex-start", max: 82 }),
...
function followUp(e, model, masterArtwork) {
  ...
  const status = clean(model.tapeStatus || model.verdictStatus, "MONITOR", 24).toUpperCase().replace("AWAITING CONFIRMATION", "CONFIRMATION PENDING");
  ...
  const changes = [
    compact(model.posterVerdict || model.verdict, "Initial move needs confirmation.", { words: 7, chars: 44 }),
    compact(model.posterConfirmation || model.confirmation, "Cross-asset follow-through is required.", { words: 7, chars: 44 }),
    compact(model.posterInvalidation || model.invalidation, "Benchmark reversal invalidates the read.", { words: 7, chars: 44 }),
  ];
  ...
  items.push(
    ...
    field(e, "follow-conclusion", compact(model.posterVerdict || model.verdict, "Evidence remains mixed; confirmation is incomplete.", { words: 13, chars: 76 }), { x: 725, y: 432, w: 250, h: 69 }, { size: 14, lineHeight: 1.22, weight: 650, align: "flex-start", max: 76 }),
