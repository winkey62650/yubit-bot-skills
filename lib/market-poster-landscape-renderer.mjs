...
function canvas(masterArtwork, title) {
...
function topRule(e, left, right, meta, light = false) {
  return e("div", { style: { position: "absolute", left: 0, right: 0, top: 0, height: 58, display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 56px", borderBottom: `1px solid ${light ? "rgba(255,255,255,.28)" : COLORS.line}`, color: light ? "#D8EEF8" : COLORS.muted, background: light ? COLORS.ink : COLORS.paper, fontSize: 11, fontWeight: 900, letterSpacing: 2.8 } },
...
function hero(e, title, kicker, meta) {
  return e("div", { style: { position: "absolute", left: 0, right: 0, top: 150, height: 70, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 56px", color: "#FFFFFF", background: `linear-gradient(90deg, ${COLORS.ink}, ${COLORS.navy})`, borderTop: "1px solid rgba(255,255,255,.18)" } },
    e("span", { style: { display: "flex", color: "#9EDDF3", fontSize: 11, fontWeight: 900, letterSpacing: 2.4 } }, clean(kicker, "VERIFIED MARKET INTELLIGENCE", 58).toUpperCase()),
    usable(meta) ? e("span", { style: { display: "flex", padding: "8px 13px", border: "1px solid rgba(255,255,255,.34)", borderRadius: 9, color: "#FFFFFF", background: "rgba(255,255,255,.08)", fontSize: 10, fontWeight: 900, letterSpacing: 1.35 } }, clean(meta, "", 52).toUpperCase()) : null
  );
}

function assetPills(e, value) {
  const symbols = clean(value, "MARKET", 28).toUpperCase().split(/\s*[·,/]\s*|\s+/).filter(Boolean).slice(0, 4);
  return e("div", { style: { display: "flex", gap: 8, marginTop: 6 } }, ...symbols.map((symbol) => e("span", { key: symbol, style: { display: "flex", alignItems: "center", color: COLORS.blue, fontSize: 12, fontWeight: 900 } },
    e("span", { style: { width: 18, height: 18, marginRight: 4, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 999, color: "#FFFFFF", background: COLORS.blue, fontSize: 9, fontWeight: 900 } }, symbol.slice(0, 1)),
    symbol
  )));
}

function bodySurface(e) {
  return e("div", { style: { position: "absolute", left: 0, right: 0, top: 220, bottom: 0, display: "flex", background: `linear-gradient(135deg, ${COLORS.paper} 0%, #FFFFFF 54%, #DDECF3 100%)`, borderTop: "5px solid rgba(22,184,231,.72)" } });
}
...
function daily(e, model, masterArtwork) {
  ...
  return e("div", { "aria-label": "DAILY MARKET BRIEF poster", style: canvas(masterArtwork, "DAILY MARKET BRIEF") },
    ...
    e("div", { style: { position: "absolute", left: 56, right: 56, top: 244, bottom: 66, display: "flex", gap: 16 } },
      ...stories.map((story, index) => e("div", { key: story.rank || index, style: { flex: 1, minWidth: 0, display: "flex", flexDirection: "column", padding: "22px 20px 18px", border: `1px solid ${COLORS.line}`, borderRadius: 15, background: "rgba(255,255,255,.94)", boxShadow: "0 12px 28px rgba(7,28,50,.09)" } },
        ...
        e("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginTop: "auto", paddingTop: 14, borderTop: `1px solid ${COLORS.line}` } },
          e("div", { style: { display: "flex", flexDirection: "column" } }, label(e, "AFFECTED"), assetPills(e, story.affected)),
...
function weekly(e, model, masterArtwork) {
  ...
  return e("div", { "aria-label": "WEEKLY CATALYSTS poster", style: canvas(masterArtwork, "WEEKLY CATALYSTS") },
    ...
    e("div", { style: { position: "absolute", left: 56, right: 56, top: 280, bottom: 64, display: "flex", gap: 12 } },
      ...
      events.length === 1 ? e("div", { style: { width: 290, display: "flex", flexDirection: "column", padding: "22px 20px", border: `1px solid ${COLORS.line}`, borderTop: `4px solid ${COLORS.cyan}`, borderRadius: 14, background: "rgba(255,255,255,.96)", boxShadow: "0 10px 24px rgba(7,28,50,.08)" } },
        label(e, "WEEK AT A GLANCE", COLORS.blue),
        e("span", { style: { display: "flex", marginTop: 14, color: COLORS.ink, fontSize: 24, lineHeight: 1.14, fontWeight: 900 } }, singleWeekLabel(model.weekStart)),
        e("div", { style: { display: "flex", justifyContent: "space-between", marginTop: 24, padding: "15px 0", borderTop: `1px solid ${COLORS.line}`, borderBottom: `1px solid ${COLORS.line}` } }, label(e, "VERIFIED EVENTS"), e("span", { style: { display: "flex", color: COLORS.blue, fontSize: 28, fontWeight: 900 } }, String(events.length))),
        e("div", { style: { display: "flex", justifyContent: "space-between", marginTop: 18 } }, label(e, "HIGH IMPACT"), e("span", { style: { display: "flex", color: COLORS.red, fontSize: 28, fontWeight: 900 } }, String(events.filter((event) => Number(event.importance) >= 3 || event.isPriority).length))),
        e("span", { style: { display: "flex", marginTop: "auto", color: COLORS.muted, fontSize: 10, lineHeight: 1.42, fontWeight: 800, letterSpacing: 1.05 } }, "ONLY VERIFIED EVENTS ARE SHOWN. EMPTY DAYS ARE INTENTIONALLY OMITTED.")
      ) : null
    ),
...
function dataFlash(e, model, masterArtwork) {
  ...
  return e("div", { "aria-label": "DATA FLASH poster", style: canvas(masterArtwork, "DATA FLASH") },
    ...
    e("div", { style: { position: "absolute", left: 56, right: 56, top: 244, bottom: 64, display: "flex", gap: 26 } },
      e("div", { style: { width: comparisons.length ? 485 : 380, display: "flex", flexDirection: "column", padding: "22px", border: `1px solid ${COLORS.line}`, borderRadius: 15, background: "rgba(255,255,255,.96)", boxShadow: "0 12px 28px rgba(7,28,50,.09)" } },
      ...
      e("span", { style: { display: "flex", marginTop: 12, color: COLORS.ink, fontSize: 19, fontWeight: 900 } }, clean(model.indicator, "OFFICIAL RELEASE", 34).toUpperCase()),
      ...
      e("div", { style: { display: "flex", flexDirection: "column", marginTop: "auto", paddingTop: 16, borderTop: `1px solid ${COLORS.line}` } },
        label(e, "INITIAL IMPACT"),
        e("span", { style: { display: "flex", marginTop: 8, color: tone(impact), fontSize: 24, fontWeight: 900, letterSpacing: 1.2 } }, impact),
        e("span", { style: { display: "flex", marginTop: 8, color: COLORS.muted, fontSize: 10, fontWeight: 800, letterSpacing: 1.05 } }, clean(model.source, "OFFICIAL SOURCE", 36).toUpperCase())
      )
      ),
...
function followUp(e, model, masterArtwork) {
  ...
  return e("div", { "aria-label": "MARKET FOLLOW-UP poster", style: canvas(masterArtwork, "MARKET FOLLOW-UP") },
