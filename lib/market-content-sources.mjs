...
const NASDAQ_API_URL = "https://query1.finance.yahoo.com/v8/finance/chart/NQ=F";
const US2Y_API_URL = "https://query1.finance.yahoo.com/v8/finance/chart/%5EUST2Y";

const YAHOO_REACTION_MARKETS = Object.freeze({
  DXY: { id: "dxy-yahoo-finance", url: DXY_API_URL },
  NASDAQ: { id: "nasdaq-yahoo-finance", url: NASDAQ_API_URL },
  US2Y: { id: "us2y-yahoo-finance", url: US2Y_API_URL },
});
...
function coinbaseNumberValue(value, label, { allowZero = false } = {}) {
  const minimumAccepted = (numeric) => allowZero ? numeric >= 0 : numeric > 0;
  const validNumber = typeof value === "number" && Number.isFinite(value) && minimumAccepted(value);
  const validDecimal = typeof value === "string"
    && /^(?:0|[1-9]\d*)(?:\.\d+)?$/.test(value)
    && Number.isFinite(Number(value))
    && minimumAccepted(Number(value));
  if (!validNumber && !validDecimal) throw new SourceSchemaError(`Invalid ${label}`);
  return Number(value);
}

function identicalCoinbaseCandle(left, right) {
  return left.length === right.length && left.every((value, index) => Object.is(value, right[index]));
}

function coinbaseCandles(payload, symbol) {
  if (!Array.isArray(payload)) throw new SourceSchemaError(`Invalid ${symbol} Coinbase candle schema`);
  const unique = new Map();
  for (const row of payload) {
    if (!Array.isArray(row) || row.length !== 6) {
      throw new SourceSchemaError(`Invalid ${symbol} Coinbase candle schema`);
    }
    const time = coinbaseNumberValue(row[0], `${symbol} Coinbase candle timestamp`);
    coinbaseNumberValue(row[1], `${symbol} Coinbase low`);
    coinbaseNumberValue(row[2], `${symbol} Coinbase high`);
    coinbaseNumberValue(row[3], `${symbol} Coinbase open`);
    const close = coinbaseNumberValue(row[4], `${symbol} Coinbase before price`);
    coinbaseNumberValue(row[5], `${symbol} Coinbase volume`, { allowZero: true });
    const existing = unique.get(time);
    if (existing && !identicalCoinbaseCandle(existing.row, row)) {
      throw new SourceSchemaError(`${symbol} Coinbase duplicate candle conflict`);
    }
    if (!existing) unique.set(time, { row, openTime: time * 1000, close });
  }
...
async function fetchCoinbaseReaction(symbol, beforeMs, observedAt, requestOptions) {
  const product = `${symbol}-USD`;
  const candleUrl = new URL(`${COINBASE_EXCHANGE_API_URL}/products/${product}/candles`);
  candleUrl.searchParams.set("granularity", "60");
  candleUrl.searchParams.set("start", new Date(beforeMs - 100 * 60_000).toISOString());
  candleUrl.searchParams.set("end", new Date(beforeMs).toISOString());
  const candle = await getWithRetry(candleUrl.toString(), { ...requestOptions, type: "json" });
  const selectedCandle = coinbaseCandles(candle.data, symbol)
    .filter((entry) => entry.openTime + 60_000 <= beforeMs)
    .sort((left, right) => right.openTime - left.openTime)[0];
  if (!selectedCandle) throw new Error(`${symbol} Coinbase completed candle unavailable before target`);
  const candleTime = selectedCandle.openTime;
  const expectedCandleTime = Math.floor(beforeMs / 60_000) * 60_000 - 60_000;
  if (candleTime !== expectedCandleTime) throw new Error(`${symbol} Coinbase candle gap before target`);

  const tickerUrl = `${COINBASE_EXCHANGE_API_URL}/products/${product}/ticker`;
  const ticker = await getWithRetry(tickerUrl, { ...requestOptions, type: "json" });
  if (!ticker.data || typeof ticker.data !== "object" || Array.isArray(ticker.data)) {
    throw new SourceSchemaError(`Invalid ${symbol} Coinbase ticker schema`);
  }
  return reactionRecord(
    symbol,
    selectedCandle.close,
    coinbaseNumberValue(ticker.data.price, `${symbol} Coinbase latest price`),
    "Coinbase",
    observedAt,
...
async function fetchYahooReaction(symbol, apiUrl, beforeMs, nowMs, observedAt, requestOptions) {
  ...
  if (!beforePoint || !latestPoint) throw new Error(`${symbol} data unavailable for requested window`);
  ...
  if (symbol === "US2Y") {
    record.changeBasisPoints = Number(((latestPoint.value - beforePoint.value) * 100).toFixed(2));
  }
  return record;
}

function providerState(id, url, checkedAt, fallbackFrom) {
  return { id, url, checkedAt, fallbackFrom, attempts: 0, successes: 0, errors: [], timeout: false };
}
...
function recordProviderObservation(state, observation) {
  state.attempts += observation.calls;
  if (observation.error) noteProviderError(state, observation.error);
  else state.successes += 1;
}

function joinedProviderNames(names) {
...
export async function fetchMarketReaction({
  ...
  now,
  fetchImpl = fetch,
  symbols = ["BTC", "ETH", "NASDAQ", "DXY", "US2Y"],
  timeoutMs,
  deadlineMs,
  retryDelayMs,
  delayImpl,
  ...
  const providers = {
    ...
    coinbase: providerState("coinbase-exchange", COINBASE_EXCHANGE_API_URL, observedAt, "okx"),
    ...Object.fromEntries(Object.entries(YAHOO_REACTION_MARKETS).map(([symbol, market]) => [
      symbol,
      providerState(market.id, market.url, observedAt),
    ])),
  };

  async function attemptProvider(provider, fetchReaction) {
    let calls = 0;
    const trackedOptions = {
      ...requestOptions,
      fetchImpl: (...arguments_) => {
        calls += 1;
        return requestOptions.fetchImpl(...arguments_);
      },
    };
    try {
      const record = await fetchReaction(trackedOptions);
      return {
        record,
        error: null,
        observation: calls > 0 ? { provider, calls, error: null } : null,
      };
    } catch (error) {
      return {
        record: null,
        error,
        observation: calls > 0 ? { provider, calls, error } : null,
      };
    }
  }

  const outcomes = await Promise.all(normalizedSymbols.map(async (symbol) => {
    const yahooMarket = YAHOO_REACTION_MARKETS[symbol];
    if (yahooMarket) {
      const attempt = await attemptProvider(symbol, (options) => fetchYahooReaction(symbol, yahooMarket.url, beforeMs, nowMs, observedAt, options));
      return {
        symbol,
        record: attempt.record,
        observations: attempt.observation ? [attempt.observation] : [],
        warning: attempt.error ? `${symbol} optional data unavailable: ${attempt.error.message}` : null,
      };
    }

    const routes = [
      ["binance", "Binance", (options) => fetchBinanceReaction(symbol, beforeMs, observedAt, options)],
      ["okx", "OKX", (options) => fetchOkxReaction(symbol, beforeMs, observedAt, options)],
      ["coinbase", "Coinbase", (options) => fetchCoinbaseReaction(symbol, beforeMs, observedAt, options)],
    ];
    const observations = [];
    const attemptedNames = [];
    let lastError;
    for (const [provider, label, fetchReaction] of routes) {
      const attempt = await attemptProvider(provider, fetchReaction);
      lastError = attempt.error;
      if (attempt.observation) {
        observations.push(attempt.observation);
        attemptedNames.push(label);
      }
      if (attempt.record) return { symbol, record: attempt.record, observations, warning: null };
    }

    const skippedNames = routes.map(([, label]) => label).filter((label) => !attemptedNames.includes(label));
    const attempted = attemptedNames.length > 0
      ? `attempted ${joinedProviderNames(attemptedNames)}`
      : "no provider request was attempted";
    const skipped = skippedNames.length > 0
      ? `; ${joinedProviderNames(skippedNames)} skipped because the shared deadline expired`
      : "";
    return {
      symbol,
      record: null,
      observations,
      warning: `${symbol} market reaction unavailable; ${attempted}${skipped}: ${lastError?.message ?? "no source response"}`,
    };
  }));
  ...
  for (const outcome of outcomes) {
    for (const observation of outcome.observations) {
      recordProviderObservation(providers[observation.provider], observation);
    }
    if (outcome.record) data[outcome.symbol] = outcome.record;
    if (outcome.warning) warnings.push(outcome.warning);
  }

  const sources = [];
  if (providers.binance.attempts > 0) sources.push(providerHealth(providers.binance));
  if (providers.okx.attempts > 0) sources.push(providerHealth(providers.okx));
  if (providers.coinbase.attempts > 0) sources.push(providerHealth(providers.coinbase));
  for (const symbol of Object.keys(YAHOO_REACTION_MARKETS)) {
    if (providers[symbol].attempts > 0) sources.push(providerHealth(providers[symbol]));
  }
