...
export async function fetchNasdaqCalendar({
  ...
  for (const date of dates) {
    ...
    try {
      ...
      nasdaqRows(response.data).forEach((row, index) => {
        ...
        if (!title) return;
        events.push({
          id: `nasdaq:${date}:${gmt || "tbd"}:${title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || index}`,
      ...
      warnings.push(`Nasdaq economic calendar unavailable for ${date}: ${error.message}`);
    }
  }
  const status = successfulRequests === dates.length && dates.length > 0
    ? "ok"
    : successfulRequests > 0 ? "degraded" : "error";
  return {
    data: events,
    events,
    sources: [sourceHealth({
...
export function parseFederalReserveCalendar(html) {
  const matches = [...String(html ?? "").matchAll(/href=["']([^"']*\/monetarypolicy\/files\/monetary(\d{8})a1\.pdf)["']/gi)];
  const seen = new Set();
  return matches.flatMap((match) => {
    ...
    const scheduled = new Date(Date.UTC(year, month - 1, day, utcHour));
    ...
    return [{
      ...
      timeLabel: scheduled.toISOString(),
      rawScheduledAt: compactDate,
      unit: null,
      ...
      source: {
        ...
        url: new URL(match[1], "https://www.federalreserve.gov").toString(),
        authority: "official",
      },
    }];
  });
...
export async function fetchFederalReserveCalendar({
  ...
  try {
    const response = await getWithRetry(FEDERAL_RESERVE_CALENDAR_URL, {
      ...
      delayImpl,
      validate: (html) => {
        if (!/<(?:html|body|main|a)\b/i.test(String(html ?? ""))) {
...
function parseBeaReleaseSchedule(html, { from, to, checkedAt }) {
  const blocks = [...String(html ?? "").matchAll(/<(?:article|tr|li)\b[^>]*>([\s\S]*?)<\/(?:article|tr|li)>/gi)];
  return blocks.flatMap((match, index) => {
    const datetime = match[1].match(/<time\b[^>]*datetime=["']([^"']+)["']/i)?.[1];
    const scheduledAt = normalizedScheduledAt(datetime);
...
async function fetchBeaReleaseSchedule(options = {}) {
  ...
  try {
    const response = await getWithRetry(BEA_RELEASE_SCHEDULE_URL, {
      fetchImpl, timeoutMs, type: "text", deadlineAt: adapterDeadline(timeoutMs, deadlineMs), retryDelayMs, delayImpl,
      validate: (html) => {
        if (!/<(?:html|body|main|article|table)\b/i.test(String(html ?? ""))) throw new SourceSchemaError("Unrecognized BEA release schedule payload schema");
      },
    });
...
function canonicalCalendarKey(event) {
...
function calendarCandidate(event, value, checkedAt, authority = "auxiliary") {
  return {
    value,
    rawValue: event.rawScheduledAt ?? value,
    status: "verified",
    authority,
...
function mergeCalendarEvents(events, checkedAt) {
  ...
  for (const event of events) {
    const day = String(event.scheduledAt ?? event.rawScheduledAt ?? checkedAt).slice(0, 10);
    const country = event.country ?? "US";
    const key = canonicalCalendarKey(event);
    const groupKey = `${key}:${country}:${day}`;
    if (!groups.has(groupKey)) groups.set(groupKey, { key, country, day, events: [] });
  ...
  return [...groups.values()].map((group) => {
    ...
    return {
      ...preferred,
      id: `${group.key}:${group.country}:${group.day}`,
