...
function mergeCalendarEvents(events, checkedAt) {
  ...
  for (const event of auxiliary) {
    ...
    if (matching) matching.events.push(event);
      ...
      country,
      period,
      occurrence ?? (event.scheduledAt ? null : `event-${String(event.id ?? event.sourceId ?? groups.length + 1)}`),
    );
  }
  ...
  for (const event of official) {
    ...
    if (closestBySeries.size > 0) {
      ...
      addGroup(event, identity, country, period, period ?? `event-${String(event.id ?? event.sourceId ?? groups.length + 1)}`);
    }
  }
