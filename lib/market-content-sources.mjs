  ...
  try {
    const response = await getWithRetry(FEDERAL_RESERVE_CALENDAR_URL, {
      ...
      validate: (html) => {
        ...
        const visibleMeetingCount = yearHeadings.reduce((count, heading, index) => {
          ...
          const candidates = visibleText.match(new RegExp(`\\b(?:${monthName})\\b(?:\\s*\\/\\s*(?:${monthName}))?\\s+\\d{1,2}(?:\\s*[-–]\\s*\\d{1,2})?\\b`, "gi"));
          return count + (candidates?.length ?? 0);
        }, 0);
        ...
        const hasInvalidMeetingRow = meetingRows.some((row, index) => {
          ...
          return !explicitPlaceholder && !(new RegExp(`^(?:${monthName})(?:\\s*\\/\\s*(?:${monthName}))?$`, "i").test(month)
            && /^\d{1,2}(?:\s*[-–]\s*\d{1,2})?\*?$/.test(date));
        });
        if (hasInvalidMeetingRow || visibleMeetingCount > parsedMeetings.length) {
