  ...
  try {
    const response = await getWithRetry(BEA_RELEASE_SCHEDULE_URL, {
      ...
      validate: (html) => {
        ...
        const hasCandidateRows = [...body.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)]
          .some((match) => /\brelease-(?:date|title)\b/i.test(match[1]));
        if (hasCandidateRows && parseBeaReleaseSchedule(document, { checkedAt }).length === 0) {
