...
async function fetchCoinbaseReaction(symbol, beforeMs, observedAt, requestOptions) {
  ...
  const selectedCandle = (Array.isArray(candle.data) ? candle.data : [])
    .filter((row) => {
      ...
      return Array.isArray(row) && Number.isFinite(openTime) && openTime + 60_000 <= beforeMs;
    })
    .sort((left, right) => Number(right[0]) - Number(left[0]))[0];
  if (!selectedCandle) throw new Error(`${symbol} Coinbase completed candle unavailable before target`);
  const candleTime = Number(selectedCandle[0]) * 1000;
  ...
  return reactionRecord(
    symbol,
    numberValue(selectedCandle[4], `${symbol} Coinbase before price`),
    numberValue(ticker.data?.price, `${symbol} Coinbase latest price`),
    "Coinbase",
    observedAt,
...
export async function fetchMarketReaction({
  ...
  await Promise.all(normalizedSymbols.map(async (symbol) => {
    ...
    try {
      data[symbol] = await fetchCoinbaseReaction(symbol, beforeMs, observedAt, requestOptions);
      providers.coinbase.successes += 1;
    } catch (error) {
      noteProviderError(providers.coinbase, error);
      warnings.push(`${symbol} market reaction unavailable from Binance, OKX, and Coinbase: ${error.message}`);
    }
  }));
  ...
  if (providers.coinbase.successes || providers.coinbase.errors.length) sources.push(providerHealth(providers.coinbase));
