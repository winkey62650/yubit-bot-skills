...
export async function fetchMarketReaction({
  ...
  const outcomes = await Promise.all(normalizedSymbols.map(async (symbol) => {
    if (symbol === "DXY") {
      const attempt = await attemptProvider("dxy", (options) => fetchDxyReaction(beforeMs, nowMs, observedAt, options));
      return {
        symbol,
        record: attempt.record,
        observations: attempt.observation ? [attempt.observation] : [],
        warning: attempt.error ? `DXY optional data unavailable: ${attempt.error.message}` : null,
      };
    }
  ...
  if (providers.dxy.attempts > 0) sources.push(providerHealth(providers.dxy));
