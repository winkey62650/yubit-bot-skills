  ...
  try {
    const response = await getWithRetry(FEDERAL_RESERVE_CALENDAR_URL, {
      ...
      validate: (html) => {
        ...
        const hasMeetingRows = /<div\b[^>]*class=["'](?=[^"']*\brow\b)(?=[^"']*\bfomc-meeting\b)[^"']*["'][^>]*>/i.test(document);
        ...
        if (hasMeetingRows && !parseFederalReserveCalendar(document).some((event) => (
          event.rawScheduledAt && typeof event.rawScheduledAt === "object"
        ))) {
          throw new SourceSchemaError("Federal Reserve calendar meeting rows did not match the expected schema");
        }
        const yearHeadings = [...document.matchAll(/<a\b[^>]*>\s*\d{4}\s+FOMC Meetings\s*<\/a>/gi)];
        const hasUnparsedMeetingContent = !hasMeetingRows && yearHeadings.some((heading, index) => {
          const section = document.slice(heading.index + heading[0].length, yearHeadings[index + 1]?.index ?? document.length);
          const visibleText = decodeXmlEntities(section.replace(/<[^>]+>/g, " ")).replace(/\s+/g, " ").trim();
          return /\b(?:January|February|March|April|May|June|July|August|September|October|November|December)\b/i.test(visibleText)
            && /\b\d{1,2}\s*(?:[-–]\s*\d{1,2})?\b/.test(visibleText);
        });
        if (hasUnparsedMeetingContent) {
  ...
  try {
    const response = await getWithRetry(BEA_RELEASE_SCHEDULE_URL, {
      ...
      validate: (html) => {
        ...
        const hasNonemptyRows = [...body.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)]
          .some((match) => decodeXmlEntities(match[1].replace(/<[^>]+>/g, " ")).trim().length > 0);
        if (hasNonemptyRows && parseBeaReleaseSchedule(document, { checkedAt }).length === 0) {
...
function stableCalendarEventToken(event, identity, country) {
  ...
  const rawSchedule = event?.rawScheduledAt && typeof event.rawScheduledAt === "object"
    ? JSON.stringify(event.rawScheduledAt)
    : String(event?.rawScheduledAt ?? "");
  return `fingerprint-${stableCalendarHash(JSON.stringify({
    ...
    rawSchedule,
    actual: event?.rawValues?.actual ?? event?.values?.actual ?? null,
    forecast: event?.rawValues?.forecast ?? event?.values?.forecast ?? null,
    previous: event?.rawValues?.previous ?? event?.values?.previous ?? null,
  }))}`;
}
...
function mergeCalendarEvents(events, checkedAt) {
  ...
  for (const event of auxiliary) {
    ...
    const occurrence = period ?? (!event.scheduledAt && rawDate ? `date-${rawDate}` : null);
    const matching = groups.find((group) => (
      group.seriesKey === identity.seriesKey
      && group.country === country
      && (occurrence ? group.occurrence === occurrence : !group.occurrence)
      && (event.scheduledAt ? calendarDistance(group.events[0], event) <= 48 * 60 * 60 * 1000 : true)
    ));
