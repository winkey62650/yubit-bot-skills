...
export function renderAgentUpdateText(update, now = new Date()) {
  ...
  const description = String(update?.description || "A new video is ready to watch.").replace(/\s+/g, " ").trim().slice(0, 500);
  const url = String(update?.url || "").trim();
  return [
    `${title} · ${formatAgentUpdateDate(resolvedDate)}`,
    description,
...
function formatAgentUpdateDate(date) {
