...
async function acknowledgeReleaseTargetWithRetry(options) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await acknowledgeDataReleaseTarget(options);
    } catch (error) {
      lastError = error;
    }
  }
...
export async function runAutomationJob(jobId, options = {}) {
  ...
  if (jobId === "data-release-updates" && !options[DATA_RELEASE_RUN_LOCKED]) {
    ...
    return withDataReleaseRunLease({
      ...
      now: options.now,
      operation: () => runAutomationJob(jobId, {
        ...options,
        repository: lockRepository,
  ...
  try {
    ...
    for (const item of telegramTargets) {
      if (options.deferDelivery === true) {
        if (jobId === "data-release-updates") await markDataReleaseTargetPending({ repository: options.repository, deduplicationKey: generated.deduplicationKey, targetKey: buildDataReleaseTargetKey(item), event: generated.event, now });
        targetResults.push({ target: item, status: "pending", messageId: null, messageIds: [], releaseTargetKey: buildDataReleaseTargetKey(item) });
        continue;
      }
      ...
      if (jobId === "data-release-updates") await markDataReleaseTargetPending({ repository: options.repository, deduplicationKey: generated.deduplicationKey, targetKey: releaseTargetKey, event: generated.event, now });
      try {
        const plan = deliveryPlans.find((entry) => entry.target === item)?.steps ?? [];
        for (const step of plan) {
          const sender = options.telegramSender || telegramCall;
          const sent = await sender(token, step.method, step.payload);
          if (sent.message_id) messageIds.push(sent.message_id);
        }
        if (jobId === "data-release-updates") await acknowledgeReleaseTargetWithRetry({ repository: options.repository, deduplicationKey: generated.deduplicationKey, targetKey: releaseTargetKey, event: generated.event, now });
        targetResults.push({ target: item, status: "success", messageId: messageIds[0] || null, messageIds, releaseTargetKey });
      } catch (error) {
        if (jobId === "data-release-updates" && messageIds.length === 0) await releaseDataReleaseTargetClaim({ repository: options.repository, deduplicationKey: generated.deduplicationKey, targetKey: releaseTargetKey, event: generated.event, now });
        targetResults.push({
          target: item,
          status: jobId === "data-release-updates" && messageIds.length ? "pending" : "failed",
          messageId: messageIds[0] || null,
          messageIds,
          error: error.message,
          ...(jobId === "data-release-updates" && messageIds.length ? { receiptFinalizationPending: true, releaseTargetKey } : {})
        });
      }
    }
    for (const item of discordTargets) {
      ...
      try {
        ...
        for (const step of plan) {
          ...
          const sent = await sender(item.channelId, step.payload);
    ...
    const status = failed ? (succeeded || pending ? "partial" : "failed") : pending ? "queued" : "success";
    ...
    await writeJson(statePath, state);
    return logRun({ job, slot, dryRun, status, message: `内容已发送至 ${succeeded}/${targetResults.length} 个目标${pending ? `，${pending} 个 Telegram 目标等待本机发布桥` : ""}。`, target: configuredTargets, preview: { ...preview, deliveryPlans, targetResults, ...(releaseReceipt ? { deliveryReceipt: releaseReceipt } : {}) } });
  } catch (error) {
...
async function buildWeeklyCalendarContent(now, { fetchImpl = fetch, repository, persist = false } = {}) {
  const range = utcWeekRange(now);
  const result = await fetchTradingViewCalendar({ from: range.start, to: range.end, fetchImpl });
...
async function buildDataReleaseContent(now, {
  ...
  const result = await pollDataReleaseUpdates({
    ...
    fetchCalendar: ({ from, to }) => fetchTradingViewCalendar({ from, to, fetchImpl }),
    ...
    for (const step of plan.steps) {
      try {
        const message = isDiscordDestination(plan.target)
          ? await sendDiscordMessage(plan.target.channelId, step.payload)
          : await telegramCall(token, step.method, step.payload);
