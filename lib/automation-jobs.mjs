...
function marketDiagnostics(templateId, document, result) {
  ...
  if (templateId === "weekly-calendar") {
    ...
    return {
      ...
      missing: [],
      conflicts,
      nextMonitoredEvent: result.nextMonitoredEvent ?? candidates[0] ?? null,
      sources
    };
  }
...
function marketEnvelope(templateId, document, now, result = {}) {
  const diagnostics = marketDiagnostics(templateId, document, result);
  return {
    templateId,
