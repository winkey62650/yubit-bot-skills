...
import { buildAcademyDemoShowcaseContent } from "./academy-demo-showcase.mjs";
...
import { governAutomationContent, publishGovernedContent } from "./content-automation-adapter.mjs";
...
} from "./market-poster-models.mjs";
import { selectMarketPosterTemplate } from "./market-poster-templates.mjs";
import { fetchOfficialActual as fetchOfficialActualDefault } from "./market-official-releases.mjs";
import { reconcileCalendarEvents, validateWeeklyPublication } from "./market-provenance.mjs";
import {
...
} from "./market-editorial-articles.mjs";
import {
...
import {
  buildLiquidityAlert,
  classifyLiquidityLifecycle,
  multiplyDecimal,
  renderMarketIntelligenceAlertText,
  shouldPublishLiquidityUpdate
} from "./market-intelligence-alerts.mjs";
import {
...
const marketCardTemplateVersion = "market-card-v5";
const MARKET_INTELLIGENCE_ALERT_TEMPLATE_VERSION = "market-intelligence-alert-v1";
const MARKET_DOCUMENT_JOBS = new Set(["crypto-daily", "weekly-calendar", "data-release-updates"]);
const FIXED_EDITORIAL_JOBS = new Set(["daily-analysis"]);
const DURABLE_MARKET_DELIVERY_JOBS = new Set(["weekly-calendar", "data-release-updates"]);
...
function appendTargetCtaWithRange(value, target, options = {}) {
  const content = appendTargetCta(value, target, options);
  const cta = targetCtaBlock(target, options);
  if (!cta) return { content };
  const limit = Number(options.limit);
  const deliveredCta = Number.isFinite(limit) && limit > 0 && cta.length >= limit
    ? cta.slice(0, limit)
    : cta;
  if (!deliveredCta || !content.endsWith(deliveredCta)) return { content };
  return {
    content,
    ctaRange: { start: content.length - deliveredCta.length, end: content.length },
  };
}
...
export const AUTOMATION_JOBS = [
  ...
  { id: "whale-hourly", name: "Market Intelligence Alert", schedule: "每小时检查，重大异动才发布", cron: "0 * * * *", topic: "6. Smart Money Tracker", bot: "SpeakerBot", content: "流动性 / Whale Flow / Position 异动" },
...
export function buildMarketDeliveryIdempotencyKey(jobId, generated, target) {
  const product = generated?.publication?.product ?? jobId;
  const identity = generated?.publication?.slug ?? generated?.deduplicationKey ?? "unknown";
  const language = generated?.language ?? "en";
  const platform = isDiscordDestination(target) ? "discord" : "telegram";
  const container = platform === "discord" ? String(target?.guildId ?? "") : String(target?.chatId ?? "");
  const destination = platform === "discord" ? String(target?.channelId ?? "") : String(target?.threadId ?? "");
  return JSON.stringify(["market-delivery-v1", product, identity, language, platform, container, destination]);
}

function durableDeliveryCoordinates(jobId, generated, target) {
  const targetKey = buildDataReleaseTargetKey(target);
  if (jobId === "weekly-calendar") {
    return {
      deduplicationKey: buildMarketDeliveryIdempotencyKey(jobId, generated, target),
      targetKey,
      event: null,
    };
  }
...
function durableDeliveryKey({ deduplicationKey, targetKey }) {
  return JSON.stringify([deduplicationKey, targetKey]);
}

function canonicalAutomationDestinations(targets) {
  const seen = new Set();
  return targets.filter(isAutomationDestination).filter((target) => {
    const key = buildDataReleaseTargetKey(target);
...
function legacyWeeklySuccessfulTargetKeys(previous, slot) {
  if (previous?.slot !== slot) return new Set();
  const results = Array.isArray(previous.targetResults) ? previous.targetResults : [];
  const successfulTargets = results.length > 0
    ? results.filter((result) => result?.status === "success").map((result) => result.target)
    : previous.status === "success" && Array.isArray(previous.targets) ? previous.targets : [];
  return new Set(successfulTargets.map(buildDataReleaseTargetKey).filter(Boolean));
}
...
export async function runAutomationJob(jobId, options = {}) {
  ...
  if (DURABLE_MARKET_DELIVERY_JOBS.has(jobId) && !options[DATA_RELEASE_RUN_LOCKED]) {
  ...
  const recordRun = createRunRecorder(options, dryRun);
  ...
  if (!dryRun && !force && jobId !== "whale-hourly" && !DURABLE_MARKET_DELIVERY_JOBS.has(jobId) && state[stateKey]?.slot === slot) {
    return recordRun({ job, slot, dryRun, status: "duplicate", message: "当前时间窗口已执行，已阻止重复发送。", target: state[stateKey].target || null });
  }
  ...
  try {
    ...
    let generated = options.demoShowcase === true
      ? buildAcademyDemoShowcaseContent(jobId, {
        now,
        acceptanceBatchId: options.demoAcceptanceBatchId,
      })
      : await buildContent(jobId, now, {
        persist: !dryRun && jobId !== "weekly-calendar",
        repository: options.repository,
        fetchImpl: options.fetchImpl,
        fetchReaction: options.fetchReaction,
        fetchCalendar: options.fetchCalendar,
        fetchOfficialActual: options.fetchOfficialActual,
        fetchWhaleMarketData: options.fetchWhaleMarketData,
        waitFor: options.waitFor,
      });
    if (options.demoShowcase === true) {
      generated = {
        ...generated,
        ...marketEnvelope(jobId, generated.document, now, generated),
        demoShowcase: true,
      };
    }
    if (options.demoShowcase !== true && ["weekly-calendar", "data-release-updates"].includes(jobId) && generated.publishable === true) {
      try {
        generated = await prepareVerifiedMarketPublication(jobId, generated, {
          dryRun, now, repository: options.repository, publicBaseUrl: options.publicBaseUrl,
          fetchImpl: options.publicationFetchImpl ?? options.fetchImpl,
          allowedPublicOrigins: options.allowedPublicOrigins,
        });
      } catch (error) {
        const preview = { ...generated, ...automationTemplateMetadata(jobId), publicationError: error.message, demoShowcase: options.demoShowcase === true, target };
        return recordRun({ job, slot, dryRun, status: "skipped", message: "未发布：发布物页面或图片验证失败。", target, preview });
      }
    }
    const mediaDelivery = buildMarketMediaDelivery(generated, {
      baseUrl: options.publicBaseUrl,
      textOnly: options.textOnly === true,
    });
    const imageUrl = mediaDelivery.defaultUrl;
    let preview = { ...generated, ...automationTemplateMetadata(jobId), imageUrl, mediaDelivery, textOnly: options.textOnly === true, demoShowcase: options.demoShowcase === true, target };
    ...
    if (MARKET_DOCUMENT_JOBS.has(jobId)) {
      try {
        const contentGovernance = await governAutomationContent(jobId, generated, {
          dryRun,
          now,
          publicBaseUrl: options.publicBaseUrl,
          vaultPath: options.obsidianVaultPath,
          system: options.contentProductSystem,
        });
        generated = { ...generated, contentGovernance };
        preview = { ...generated, ...automationTemplateMetadata(jobId), imageUrl, mediaDelivery, textOnly: options.textOnly === true, demoShowcase: options.demoShowcase === true, target };
        if (contentGovernance.approved !== true) {
          return recordRun({
            job,
            slot,
            dryRun,
            status: "skipped",
            message: `未发布：content governance blocked delivery${contentGovernance.reason ? ` — ${contentGovernance.reason}` : ""}`,
            target,
            preview,
          });
        }
      } catch (error) {
        const contentGovernance = { enabled: true, approved: false, reason: error?.message ?? String(error), products: [], channelPlans: [] };
        preview = { ...generated, contentGovernance, ...automationTemplateMetadata(jobId), imageUrl, mediaDelivery, textOnly: options.textOnly === true, demoShowcase: options.demoShowcase === true, target };
        return recordRun({
          job,
          slot,
          dryRun,
          status: "skipped",
          message: `未发布：content governance failed closed — ${contentGovernance.reason}`,
          target,
          preview,
        });
      }
    ...
    if (dryRun) {
      return recordRun({ job, slot, dryRun, status: "success", message: "Dry-run 通过：数据、文案、图片与目标解析正常，未向外部平台发送。", target, preview });
    }

    if (jobId === "whale-hourly" && shouldSuppressWhaleSignal(generated, { force })) {
      return recordRun({ job, slot, dryRun, status: "skipped", message: generated.suppressionReason || "本轮 Market Intelligence Alert 未达到发布门槛。", target, preview });
    }

    if (jobId === "whale-hourly" && !force && !shouldPublishLiquidityUpdate(state[stateKey]?.alertDedupe, generated.alert, now)) {
      return recordRun({ job, slot, dryRun, status: "duplicate", message: "同一流动性事件在 60 分钟窗口内没有达到增量、优先级或生命周期升级条件，已阻止重复发布。", target, preview });
    }
    ...
    if (jobId === "agent-sync-4h") {
      ...
      return recordRun({ job, slot, dryRun, status, message, target: sendResult.targets, preview: { ...preview, deliveryPlans: sendResult.deliveryPlans, targetResults: sendResult.targetResults, sourceHealth, checks: generated.items || [] } });
    }

    const configuredTargets = canonicalAutomationDestinations(targets);
    if (!configuredTargets.length) {
      return recordRun({ job, slot, dryRun, status: "skipped", message: `未找到 ${job.topic} 的群/Topic 绑定，未发送。`, target, preview });
    }
    ...
    const uncertainDeliveryKeys = new Set();
    const assertReleaseLeaseOwned = options[DATA_RELEASE_LEASE_GUARD] ?? (async () => true);
    if (DURABLE_MARKET_DELIVERY_JOBS.has(jobId)) {
      const storedDelivery = await options.repository?.getMeta?.(DATA_RELEASE_DELIVERY_META_KEY);
      const storedDeliveryEntries = Array.isArray(storedDelivery?.entries) ? storedDelivery.entries : [];
      const preexistingDeliveryKeys = new Set(storedDeliveryEntries.map((entry) => entry?.deduplicationKey).filter(Boolean));
      const legacySuccessfulTargetKeys = jobId === "weekly-calendar"
        ? legacyWeeklySuccessfulTargetKeys(state[stateKey], slot)
        : new Set();
      for (const item of configuredTargets) {
        const coordinates = durableDeliveryCoordinates(jobId, generated, item);
        deliveryByTarget.set(item, coordinates);
        const group = deliveryGroups.get(coordinates.deduplicationKey) ?? {
          ...coordinates,
          targetKeys: [],
        };
        if (!group.targetKeys.includes(coordinates.targetKey)) group.targetKeys.push(coordinates.targetKey);
        deliveryGroups.set(coordinates.deduplicationKey, group);
      }
      for (const group of deliveryGroups.values()) {
        durableReceipts.set(group.deduplicationKey, await prepareDataReleaseDelivery({
          repository: options.repository,
      ...
      for (const group of deliveryGroups.values()) {
        if (preexistingDeliveryKeys.has(group.deduplicationKey)) continue;
        for (const targetKey of group.targetKeys) {
          if (!legacySuccessfulTargetKeys.has(targetKey)) continue;
      ...
      for (const item of configuredTargets) {
        const coordinates = deliveryByTarget.get(item);
        const marker = await getDataReleaseSendMarker({
          repository: options.repository,
          deduplicationKey: coordinates.deduplicationKey,
          targetKey: coordinates.targetKey,
          now,
        });
        if (!marker) {
          const receipt = durableReceipts.get(coordinates.deduplicationKey);
          if (receipt?.pendingTargetKeys.includes(coordinates.targetKey)) {
            await releaseDataReleaseTargetClaim({ repository: options.repository, ...coordinates, now });
          }
          continue;
        }
        if (marker.status !== "sent") {
          uncertainDeliveryKeys.add(durableDeliveryKey(coordinates));
          continue;
        }
        try {
          await acknowledgeReleaseTargetWithRetry({
          ...
          await clearDataReleaseSend({ repository: options.repository, ...coordinates, now });
        } catch (error) {
      ...
      if (jobId === "data-release-updates") releaseReceipt = durableReceipts.get(generated.deduplicationKey) ?? null;
      const isReady = (item) => {
        const coordinates = deliveryByTarget.get(item);
        return durableReceipts.get(coordinates.deduplicationKey)?.readyTargetKeys.includes(coordinates.targetKey);
      };
      telegramTargets = telegramTargets.filter(isReady);
      discordTargets = discordTargets.filter(isReady);
    }
    const deliveryPlans = [
      ...buildAutomationTelegramPlans(jobId, generated, telegramTargets, MARKET_DOCUMENT_JOBS.has(jobId) ? mediaDelivery : imageUrl),
      ...buildAutomationDiscordPlans(jobId, generated, discordTargets, MARKET_DOCUMENT_JOBS.has(jobId) ? mediaDelivery : imageUrl)
    ];
    assertNoInternalUrlsInDeliveryPlans(deliveryPlans, options.publicBaseUrl);
    const targetResults = [];
    if (DURABLE_MARKET_DELIVERY_JOBS.has(jobId)) {
      for (const item of configuredTargets) {
        const coordinates = deliveryByTarget.get(item);
        if (!uncertainDeliveryKeys.has(durableDeliveryKey(coordinates))) continue;
        targetResults.push({
          target: item,
          status: "pending",
          messageId: null,
          ...
          releaseTargetKey: coordinates.targetKey,
          deliveryIdempotencyKey: coordinates.deduplicationKey,
    ...
    for (const item of telegramTargets) {
      const coordinates = deliveryByTarget.get(item) ?? durableDeliveryCoordinates(jobId, generated, item);
      const durableDelivery = DURABLE_MARKET_DELIVERY_JOBS.has(jobId);
      if (options.deferDelivery === true) {
        const releaseClaimToken = durableDelivery ? randomUUID() : null;
        if (durableDelivery) await markDataReleaseTargetPending({ repository: options.repository, ...coordinates, claimToken: releaseClaimToken, now });
        targetResults.push({ target: item, status: "pending", messageId: null, messageIds: [], releaseTargetKey: coordinates.targetKey, deliveryIdempotencyKey: coordinates.deduplicationKey, ...(releaseClaimToken ? { releaseClaimToken } : {}) });
        continue;
      }
      ...
      try {
        if (durableDelivery) {
          ...
          await markDataReleaseTargetPending({ repository: options.repository, ...coordinates, now });
          await prepareDataReleaseSend({ repository: options.repository, ...coordinates, now });
        }
        const plan = deliveryPlans.find((entry) => entry.target === item)?.steps ?? [];
        for (const step of plan) {
          if (durableDelivery) await assertReleaseLeaseOwned();
        ...
        if (durableDelivery) {
          await completeDataReleaseSend({ repository: options.repository, ...coordinates, messageIds, now });
          durableSendCompleted = true;
          await acknowledgeReleaseTargetWithRetry({ repository: options.repository, ...coordinates, now, assertLeaseOwned: assertReleaseLeaseOwned });
          await clearDataReleaseSend({ repository: options.repository, ...coordinates, now });
        }
        targetResults.push({ target: item, status: "success", messageId: messageIds[0] || null, messageIds, releaseTargetKey: coordinates.targetKey, deliveryIdempotencyKey: coordinates.deduplicationKey });
      } catch (error) {
        if (durableDelivery && !attemptStarted && messageIds.length === 0) {
          await Promise.allSettled([
            clearDataReleaseSend({ repository: options.repository, ...coordinates, now }),
            releaseDataReleaseTargetClaim({ repository: options.repository, ...coordinates, now }),
          ]);
        }
        const uncertainDelivery = durableDelivery && attemptStarted && !durableSendCompleted;
        targetResults.push({
          target: item,
          status: durableDelivery && (attemptStarted || messageIds.length) ? "pending" : "failed",
          messageId: messageIds[0] || null,
          messageIds,
          error: error.message,
          ...(durableDelivery && durableSendCompleted
            ? { receiptFinalizationPending: true, releaseTargetKey: coordinates.targetKey, deliveryIdempotencyKey: coordinates.deduplicationKey }
            : uncertainDelivery
              ? { receiptExisting: true, releaseTargetKey: coordinates.targetKey, deliveryIdempotencyKey: coordinates.deduplicationKey, deliveryState: "uncertain-delivery", manualReconciliationRequired: true }
              : {})
        });
      }
    }
    for (const item of discordTargets) {
      ...
      if (options.deferDelivery === true) {
        targetResults.push({
          ...
          releaseTargetKey: coordinates.targetKey,
          deliveryIdempotencyKey: coordinates.deduplicationKey,
          error: "Discord does not support the local desktop publisher; use the server Discord sender.",
        });
        continue;
      }
      let durableSendCompleted = false;
    ...
    if (DURABLE_MARKET_DELIVERY_JOBS.has(jobId)) {
      ...
      for (const item of configuredTargets) {
        const coordinates = deliveryByTarget.get(item);
        if (!targetResults.some((row) => row.deliveryIdempotencyKey === coordinates.deduplicationKey && row.releaseTargetKey === coordinates.targetKey)) {
          const receipt = durableReceipts.get(coordinates.deduplicationKey);
          const existingStatus = receipt?.successfulTargetKeys.includes(coordinates.targetKey) ? "success" : "pending";
          targetResults.push({
            target: item,
    ...
    const durableDuplicate = jobId === "weekly-calendar"
      && deliveryPlans.length === 0
      && targetResults.length > 0
      && targetResults.every((item) => item.status === "success" && item.receiptExisting === true);
    const status = uncertain
      ? "manual-reconciliation"
      : durableDuplicate
        ? "duplicate"
        : failed
          ? (succeeded || pending ? "partial" : "failed")
          : pending
            ? "queued"
            : "success";
    ...
    let publicationLifecycleError = null;
    if (succeeded > 0 && generated.contentGovernance?.approved === true) {
      try {
        const publishedProducts = await publishGovernedContent(generated.contentGovernance, {
          vaultPath: options.obsidianVaultPath,
          now,
          receipt: {
            successfulTargets: succeeded,
            totalTargets: targetResults.length,
            messageIds: targetResults.flatMap((item) => item.messageIds ?? []),
          },
        });
        generated = { ...generated, contentGovernance: { ...generated.contentGovernance, products: publishedProducts } };
      } catch (error) {
        publicationLifecycleError = error?.message ?? String(error);
      }
    }
    const effectiveStatus = publicationLifecycleError ? "manual-reconciliation" : status;
    state[stateKey] = {
      slot,
      at: now.toISOString(),
      ...
      targetResults,
      ...(jobId === "whale-hourly" && generated.alert ? {
        alertDedupe: {
          eventGroupKey: generated.alert.eventGroupKey,
          visibleNotional: generated.alert.visibleNotional,
          priority: generated.alert.priority,
          lifecycle: generated.alert.lifecycle,
    ...
    const message = uncertain
      ...
      : durableDuplicate
        ? "完整投递身份已有成功回执，已阻止重复发送。"
        : `内容已发送至 ${succeeded}/${targetResults.length} 个目标${pending ? `，${pending} 个目标等待本机发布桥` : ""}。`;
    return recordRun({ job, slot, dryRun, status: effectiveStatus, message: publicationLifecycleError ? `External delivery succeeded, but Obsidian publication lifecycle requires reconciliation: ${publicationLifecycleError}` : message, target: configuredTargets, preview: {
      ...preview,
      contentGovernance: generated.contentGovernance,
      deliveryPlans,
      targetResults,
      ...(publicationLifecycleError ? { publicationLifecycleError } : {}),
      ...(releaseReceipt ? { deliveryReceipt: releaseReceipt } : {}),
      ...(jobId === "weekly-calendar" ? { deliveryReceipts: [...durableReceipts.values()] } : {}),
    } });
  } catch (error) {
    await recordRun({ job, slot, dryRun, status: "failed", message: error.message });
    throw error;
  }
}
...
export async function buildContent(jobId, now, options = {}) {
  ...
  if (jobId === "whale-hourly") return buildWhaleHourly(now, options);
...
function marketDocumentSelectionCount(templateId, document) {
  if (templateId === "crypto-daily") {
    if (Array.isArray(document?.selectedStories)) return document.selectedStories.length;
...
function posterReactionItems(reaction) {
  return Object.entries(reaction?.prices ?? reaction?.data ?? {}).flatMap(([symbol, value]) => {
    const change = Number(value?.changePercent);
    if (!Number.isFinite(change)) return [];
    return [{ symbol, value: change, label: `${change >= 0 ? "+" : ""}${change.toFixed(2)}%` }];
  });
}

function marketPosterOfficialSource(result = {}) {
  const provenance = result?.eligibility?.actual ?? result?.event?.provenance?.actual;
  const sourceId = String(provenance?.sourceId ?? "").trim();
  const sourceManifest = [
    ...(Array.isArray(result?.sourceManifest) ? result.sourceManifest : []),
    ...(Array.isArray(result?.sources) ? result.sources : []),
  ];
  const matched = sourceId
    ? sourceManifest.find((source) => String(source?.id ?? "").trim() === sourceId)
    : null;
  const direct = result?.event?.source;
  const label = String(
    provenance?.sourceLabel
      ?? matched?.label
      ?? matched?.title
      ?? (typeof direct === "string" ? direct : direct?.label ?? direct?.name)
      ?? "",
  ).trim();
  ...
  return {
    ...
    label,
    url: provenance?.sourceUrl ?? matched?.url ?? direct?.url,
    status: matched?.status ?? direct?.status ?? "verified",
  };
}

function marketPosterSource(source, result = {}) {
  const manifest = [
    ...(Array.isArray(result?.sourceManifest) ? result.sourceManifest : []),
    ...(Array.isArray(result?.sources) ? result.sources : []),
  ];
  const sourceId = String(typeof source === "string" ? source : source?.id ?? "").trim();
  const matched = sourceId
    ? manifest.find((item) => String(item?.id ?? "").trim() === sourceId)
    : null;
  if (!matched) return source;
  if (typeof source === "string") return matched;
  return { ...matched, ...(source && typeof source === "object" ? source : {}) };
}

function enrichedMarketPosterDocument(document, result = {}) {
  if (!document || typeof document !== "object") return document;
  const enrichStory = (story) => story && typeof story === "object"
    ? { ...story, source: marketPosterSource(story.source, result) }
    : story;
  return {
    ...document,
    ...(Array.isArray(document.selectedStories)
      ? { selectedStories: document.selectedStories.map(enrichStory) }
      : {}),
    ...(Array.isArray(document.sections)
      ? { sections: document.sections.map((section) => ({ ...section, story: enrichStory(section?.story) })) }
      : {}),
    ...(Array.isArray(document.days)
      ? {
        days: document.days.map((day) => ({
          ...day,
          events: Array.isArray(day?.events)
            ? day.events.map((event) => ({ ...event, source: marketPosterSource(event?.source, result) }))
            : day?.events,
        })),
      }
      : {}),
    sources: Array.isArray(document.sources) && document.sources.length
      ? document.sources
      : result.sourceManifest ?? result.sources,
  };
}

function marketEnvelope(templateId, document, now, result = {}) {
  const diagnostics = marketDiagnostics(templateId, document, result, now);
  const officialSource = marketPosterOfficialSource(result);
  const posterDocument = {
    ...enrichedMarketPosterDocument(document, result),
    generatedAt: document?.generatedAt ?? result.generatedAt ?? now.toISOString(),
  };
  const posterInput = templateId === "data-release-updates" && document
    ? {
      ...posterDocument,
      event: result.event,
      reactions: posterReactionItems(result.initialReaction),
      reactionSources: (result.initialReaction?.sources ?? []).map((source) => source?.label ?? source?.title ?? source?.id).filter(Boolean),
      source: posterDocument.source ?? officialSource,
      sources: result.sourceManifest ?? result.sources,
      updatedAt: posterDocument.generatedAt ?? now.toISOString(),
    }
    : posterDocument;
  const poster = document
    ? templateId === "crypto-daily"
      ? buildCryptoDailyPosterModel(posterDocument)
      : templateId === "weekly-calendar"
        ? buildWeeklyCalendarPosterModel(posterDocument)
        : templateId === "data-release-updates"
          ? buildDataUpdatePosterModel(posterInput)
          : null
    : null;
  const visualTemplate = selectMarketPosterTemplate({ jobId: templateId, poster });
  const governedPoster = poster && visualTemplate ? { ...poster, visualTemplate } : poster;
  const followUpPoster = templateId === "data-release-updates" && document && result.reaction
    ? buildDataUpdatePosterModel({
      ...posterInput,
      source: result.reaction?.sources?.[0] ?? posterInput.source,
      tapeStatus: result.reaction?.status === "complete" ? "CONFIRMED" : posterInput.tapeStatus,
      reactions: posterReactionItems(result.reaction),
      reactionWindow: result.reaction?.window,
      reactionSources: (result.reaction?.sources ?? []).map((source) => source?.label ?? source?.title ?? source?.id).filter(Boolean),
    })
    : null;
  const followUpTemplate = followUpPoster
    ? selectMarketPosterTemplate({ jobId: templateId, poster: followUpPoster, reaction: result.reaction })
    : null;
  const posterVariants = {
    ...(governedPoster?.visualTemplate ? { [governedPoster.visualTemplate.product]: governedPoster } : {}),
    ...(followUpPoster && followUpTemplate?.product === "market-follow-up"
      ? { "market-follow-up": { ...followUpPoster, visualTemplate: followUpTemplate } }
      : {}),
  };
  const imageKind = governedPoster?.kind ?? null;
  const contentHash = governedPoster
    ? createHash("sha256").update(JSON.stringify({ templateId, poster: governedPoster, posterVariants, document })).digest("hex")
    : null;
  return {
    templateId,
    ...
    diagnostics,
    ...(governedPoster ? { poster: governedPoster, posterVariants, imageKind, metrics: [], contentHash } : {}),
    ...(result.event ? { event: result.event } : {}),
    ...(result.initialReaction ? { initialReaction: result.initialReaction } : {}),
    ...(result.reaction ? { reaction: result.reaction } : {}),
    sourceManifest: Array.isArray(result.sourceManifest) ? result.sourceManifest : [],
    eligibility: result.eligibility ?? null,
    publication: result.publication ?? null,
    articlePath: result.articlePath ?? null,
    imagePath: result.imagePath ?? null,
    articleUrl: result.articleUrl ?? null,
    imageUrl: result.imageUrl ?? null,
    communityDocument: result.communityDocument ?? null,
    ...(result.article ? { article: result.article } : {}),
...
function memoryPublicationRepository() {
  const values = new Map();
  return {
    async getMeta(key) { return values.has(key) ? structuredClone(values.get(key)) : null; },
    async setMeta(key, value) { values.set(key, structuredClone(value)); },
  };
}

export async function prepareVerifiedMarketPublication(jobId, generated, {
  dryRun = true,
  now = new Date(),
  repository,
  publicBaseUrl,
  ...
  if (generated?.publishable !== true) return generated;
  const origin = new URL(resolveAppBaseUrl(publicBaseUrl)).origin;
  const product = jobId === "weekly-calendar" ? "weekly-calendar" : "data-update";
  const slug = generated.article?.slug ?? generated.event?.publicationSlug
    ?? `${generated.event?.indicator ?? generated.event?.id}/${String(generated.event?.releasedAt ?? generated.generatedAt).slice(0, 10)}`;
  const articlePath = generated.article
    ? product === "weekly-calendar" ? weeklyCalendarArticlePath(slug) : dataUpdateArticlePath(...slug.split("/"))
    : null;
  const articleUrl = articlePath ? new URL(articlePath, `${origin}/`).toString() : null;
  const communityDocument = generated.communityDocument ?? (product === "weekly-calendar"
    ? buildWeeklyCalendarCommunityDocument(generated.article, { articleUrl })
    : buildDataUpdateCommunityDocument(generated.article, { articleUrl }));
  const imagePath = editorialAssetPath(product, slug);
  const imageUrl = new URL(imagePath, `${origin}/`).toString();
  const resolvedRepository = dryRun ? memoryPublicationRepository() : (repository || await getDistributionRepository());
  let publication = await createMarketPublication({
    repository: resolvedRepository,
    product,
    slug,
    article: generated.article,
    communityDocument,
    posterModel: generated.poster,
    sourceManifest: generated.sourceManifest,
    now,
  });
  if (!dryRun) {
    const origins = allowedPublicOrigins ?? [origin];
    publication = await captureEditorialImage({ repository: resolvedRepository, product, slug, publicOrigin: origin, allowedPublicOrigins: origins, fetchImpl, now });
    publication = await verifyPublicPublication({ repository: resolvedRepository, product, slug, publicOrigin: origin, allowedPublicOrigins: origins, fetchImpl, now });
    if (publication.status !== "verified") throw new Error("Market publication must be verified before delivery.");
  }
  return {
    ...generated,
    publication,
...
async function buildWeeklyCalendarContent(now, {
  fetchImpl = fetch,
  fetchCalendar = fetchMarketCalendar,
  repository,
  persist = false
} = {}) {
  const range = utcWeekRange(now);
  const result = await fetchCalendar({ from: range.start, to: range.end, now, fetchImpl });
  const reconciled = result.calendar?.events
    ? { events: result.calendar.events, topThree: result.calendar.events.filter((event) => event.publishable !== false).slice(0, 3) }
    : reconcileCalendarEvents(result.events);
  const calendar = result.calendar ?? { source: result.source, events: reconciled.events };
  const eligibility = result.eligibility ?? validateWeeklyPublication(calendar, { now });
  const rankedEvents = reconciled.events.filter((event) => event.publishable !== false);
  const document = buildWeeklyCalendarDocument({ now, events: rankedEvents });
  ...
  const contentPublishability = marketPublishability("weekly-calendar", document, { ...result, events: rankedEvents });
  const publishability = contentPublishability.publishable && !eligibility.publishable
    ? { publishable: false, skipReason: eligibility.reason }
    : contentPublishability;
  const sourceManifest = result.sourceManifest ?? result.sources ?? [];
  const article = publishability.publishable
    ? buildWeeklyCalendarArticle({ document, rankedEvents, sourceManifest, marketSetup: result.marketSetup })
    : null;
  return marketEnvelope("weekly-calendar", document, now, {
    ...
    sourceManifest,
    article,
    deduplicationKey: JSON.stringify(["market-publication-v1", "weekly-calendar", article?.slug ?? document.weekStart, "en"])
  });
}
...
async function buildDataReleaseContent(now, {
  ...
  fetchOfficialActual: fetchOfficialActualImpl = fetchOfficialActualDefault,
  repository,
  persist = false
} = {}) {
  const resolvedRepository = repository || await getDistributionRepository();
  const result = await pollDataReleaseUpdates({
    ...
    eventFilter: isSupportedReleaseEvent,
    fetchCalendar: async ({ from, to, now: checkedAt }) => {
      const calendarResult = await fetchCalendar({ from, to, now: checkedAt, fetchImpl });
      if (calendarResult?.calendar?.events) return calendarResult;
      const reconciled = reconcileCalendarEvents(calendarResult?.events ?? []);
      return { ...calendarResult, events: reconciled.events };
    },
    fetchOfficialActual: ({ event, now: checkedAt }) => fetchOfficialActualImpl({ event, now: checkedAt, fetchImpl }),
  ...
  const event = result.publishable
    ? {
      ...result.event,
      releasedAt: result.event?.releasedAt ?? result.event?.actualObservedAt,
      provenance: {
        ...(result.event?.provenance ?? {}),
        actual: result.eligibility?.actual,
        forecast: result.event?.values?.forecast,
        previous: result.event?.values?.previous,
      },
    }
    : result.event;
  const document = result.publishable
    ? buildDataReleaseDocument({ event, reaction: result.reaction })
    : null;
  ...
  const sourceManifest = [
    ...(result.sourceManifest ?? result.event?.sourceManifest ?? result.sources ?? []),
    ...reactionSources,
  ];
  const tierDecision = event?.tierDecision ?? result.tierDecision;
  const article = result.publishable && tierDecision?.tier === "tier-one"
    ? buildDataUpdateArticle({ document, event, reaction: result.reaction, tierDecision, sourceManifest })
    : null;
  const communityDocument = result.publishable && tierDecision?.tier === "secondary"
    ? buildSecondaryDataUpdateCommunityDocument({ document, event, reaction: result.reaction, tierDecision, sourceManifest })
    : null;
...
export function buildAutomationTelegramPlans(jobId, generated, targets, imageUrl) {
  ...
  const governedPlans = MARKET_DOCUMENT_JOBS.has(jobId) && generated?.contentGovernance?.approved === true
    ? (generated.contentGovernance.channelPlans ?? [])
    : [];
  return (Array.isArray(targets) ? targets : []).filter(isTelegramDestination).map((target) => {
    const steps = MARKET_DOCUMENT_JOBS.has(jobId)
      ? (governedPlans.length
        ? governedPlans.flatMap((plan) => {
          const planImageUrl = mediaUrlForPlan(imageUrl, plan);
          return [
            ...(planImageUrl ? [{ method: "sendPhoto", payload: {
              ...telegramDestinationPayload(target),
              photo: planImageUrl,
            } }] : []),
            ...channelChunksWithFinalCta(plan.telegram?.chunks, target, 4096, { html: true })
              .map(({ content: text, ctaRange }) => ({
                method: "sendMessage",
                payload: {
                  ...telegramDestinationPayload(target),
                  text,
        ...
        }).map((step, stepIndex, allSteps) => {
          if (step.method !== "sendMessage") return step;
          const ctaBoundary = destinationCtaBoundary(step.ctaRange, {
            platform: "telegram",
            method: "sendMessage",
            field: "text",
            stepIndex,
          ...
          const { ctaRange, ...cleanStep } = step;
          return { ...cleanStep, ...(ctaBoundary ? { ctaBoundary } : {}) };
        })
        : [
          ...(mediaUrlForPlan(imageUrl, null) ? [{ method: "sendPhoto", payload: {
            ...telegramDestinationPayload(target),
            photo: mediaUrlForPlan(imageUrl, null),
          } }] : []),
          ...paragraphChunksWithFinalCta(renderTelegramMarketDocument(marketDocument), target, 4096, { html: true })
            .map(({ content: text, ctaRange }, index, chunks) => {
              const photoOffset = mediaUrlForPlan(imageUrl, null) ? 1 : 0;
              const ctaBoundary = destinationCtaBoundary(ctaRange, {
                platform: "telegram",
                method: "sendMessage",
                field: "text",
                stepIndex: index + photoOffset,
                stepCount: chunks.length + photoOffset,
              });
              return {
                method: "sendMessage",
                payload: {
      ...
      : (() => {
        const { content: caption, ctaRange } = appendTargetCtaWithRange(
          generated?.caption,
          target,
          { html: true, limit: 1024 },
        );
        const ctaBoundary = destinationCtaBoundary(ctaRange, {
          platform: "telegram",
          method: "sendPhoto",
    ...
    return {
      target,
      ...(MARKET_DOCUMENT_JOBS.has(jobId) ? { idempotencyKey: buildMarketDeliveryIdempotencyKey(jobId, generated, target) } : {}),
      templateVersion: automationTemplateMetadata(jobId).templateVersion ?? EDITORIAL_TEMPLATE_VERSION,
      contentPolicy: governedPlans.length ? "obsidian-canonical" : "fixed-template",
      ...(governedPlans.length ? {
        contentProductId: governedPlans[0].productId,
        contentHash: governedPlans[0].contentHash,
        contentProductIds: governedPlans.map(({ productId }) => productId),
        contentHashes: governedPlans.map(({ contentHash }) => contentHash),
      } : {}),
      steps,
    };
  });
...
function paragraphChunksWithFinalCta(value, target, limit, options = {}) {
  ...
  if (!cta) return chunks.map((content) => ({ content }));
  let ctaStart;
  if (!chunks.length) {
    chunks.push(cta);
    ctaStart = 0;
    ...
    if (finalChunk.length <= limit) {
      chunks[chunks.length - 1] = finalChunk;
      ctaStart = finalChunk.length - cta.length;
    } else {
      chunks.push(cta);
      ctaStart = 0;
    }
  ...
  return chunks.map((content, index) => ({
    content,
    ...(index === chunks.length - 1 ? { ctaRange: { start: ctaStart, end: content.length } } : {}),
  }));
}

function channelChunksWithFinalCta(value, target, limit, options = {}) {
  const chunks = (Array.isArray(value) ? value : []).map((chunk) => String(chunk));
  if (chunks.length === 0 || chunks.some((chunk) => !chunk || chunk.length > limit)) {
    throw new Error("Governed channel chunks are missing or exceed the platform limit.");
  }
  const cta = targetCtaBlock(target, options);
  if (!cta) return chunks.map((content) => ({ content }));
  const suffix = `\n\n${cta}`;
  let ctaStart;
  if (chunks.at(-1).length + suffix.length <= limit) {
    chunks[chunks.length - 1] += suffix;
    ctaStart = chunks.at(-1).length - cta.length;
  } else {
    if (cta.length > limit) throw new RangeError(`CTA block exceeds the ${limit} character platform limit.`);
    chunks.push(cta);
    ctaStart = 0;
  }
  return chunks.map((content, index) => ({
...
function destinationCtaBoundary(ctaRange, identity) {
  if (!ctaRange) return null;
  return {
    kind: "destination-cta",
    placement: "suffix",
    ...identity,
    start: ctaRange.start,
    end: ctaRange.end,
  };
}
...
export function buildAutomationDiscordPlans(jobId, generated, targets, imageUrl) {
  ...
  return (Array.isArray(targets) ? targets : []).filter(isDiscordDestination).map((target) => {
    if (MARKET_DOCUMENT_JOBS.has(jobId)) {
      const selectedSteps = governedPlans.length
        ? governedPlans.flatMap((plan) => {
          const planImageUrl = mediaUrlForPlan(imageUrl, plan);
          const chunks = channelChunksWithFinalCta(plan.discord?.chunks, target, 2000);
          return chunks.length
            ? chunks.map(({ content, ctaRange }, index) => ({
              content,
              ctaRange,
              ...(index === 0 && planImageUrl ? { imageUrl: planImageUrl } : {}),
            }))
            : planImageUrl ? [{ imageUrl: planImageUrl }] : [];
        })
        : paragraphChunksWithFinalCta(renderDiscordMarketDocument(marketDocument), target, 2000)
          .map(({ content, ctaRange }, index) => ({
            content,
            ctaRange,
            ...(index === 0 && mediaUrlForPlan(imageUrl, null) ? { imageUrl: mediaUrlForPlan(imageUrl, null) } : {}),
          }));
      return {
        target,
        idempotencyKey: buildMarketDeliveryIdempotencyKey(jobId, generated, target),
        ...
        steps: selectedSteps
          .map(({ content, ctaRange, imageUrl: stepImageUrl }, stepIndex, steps) => {
            const ctaBoundary = destinationCtaBoundary(ctaRange, {
              platform: "discord",
              method: "sendMessage",
              field: "content",
            ...
            return {
              method: "sendMessage",
              payload: { ...(content ? { content } : {}), ...(stepImageUrl ? { imageUrl: stepImageUrl } : {}) },
              ...(ctaBoundary ? { ctaBoundary } : {})
            };
          })
      };
...
export function renderAgentUpdateText(update, now = new Date()) {
  ...
  const agent = String(update?.agent || update?.package?.agent || "Creator").trim();
  const title = String(update?.title || "New YouTube video").replace(/\s+/g, " ").trim().slice(0, 180);
  const description = String(update?.description || "").replace(/\s+/g, " ").trim().slice(0, 500);
  const url = String(update?.url || "").trim();
  return [
    `${agent} | ${platform}`,
    `${title} · ${resolvedDate.toISOString().slice(0, 10)}`,
    description,
    `Click here to watch the video 👉 ${url}`
  ].filter(Boolean).join("\n\n");
}
...
export function automationTemplateMetadata(jobId) {
  ...
  if (String(jobId || "") === "whale-hourly") {
    return { templateVersion: MARKET_INTELLIGENCE_ALERT_TEMPLATE_VERSION, contentPolicy: "fixed-template" };
  }
...
async function buildWhaleHourly(now, options = {}) {
  const fetchMarket = options.fetchWhaleMarketData || fetchWhaleMarketData;
  const firstMarket = await fetchMarket();
  if (options.persist !== true) return buildWhaleAlert(firstMarket, now);

  const waitFor = options.waitFor || ((milliseconds) => new Promise((resolve) => setTimeout(resolve, milliseconds)));
  await waitFor(20_000);
  const latestMarket = await fetchMarket();
  const first = buildWhaleAlert(firstMarket, now);
  const latest = buildWhaleAlert(latestMarket, now);
  const firstTimestamp = Date.parse(firstMarket.sourceTimestamp || "");
  const latestTimestamp = Date.parse(latestMarket.sourceTimestamp || "");
  const sameSource = String(firstMarket.source || "").trim() === String(latestMarket.source || "").trim();
  const sameLevel = first.alert.eventGroupKey === latest.alert.eventGroupKey;
  const elapsedSeconds = sameSource && sameLevel && Number.isFinite(firstTimestamp) && Number.isFinite(latestTimestamp)
    ? Math.max(0, Math.floor((latestTimestamp - firstTimestamp) / 1000))
    : 0;
  const lifecycle = sameSource && sameLevel
    ? classifyLiquidityLifecycle(first.alert, latest.alert)
    : "MOVED";
  return buildWhaleAlert({ ...latestMarket, persistenceSeconds: elapsedSeconds, lifecycle }, now);
}
...
export function buildWhaleAlert(market, now = new Date()) {
  ...
  const alert = buildLiquidityAlert({
    ...
    source: {
      provider: sourceName,
      endpoint: market.endpoint || "depth",
      sourceTimestamp: market.sourceTimestamp || observedAt,
      receivedAt: market.receivedAt || observedAt
    },
    markPrice: String(market.markPrice),
    bids: market.bids,
    asks: market.asks,
    persistenceSeconds: market.persistenceSeconds,
    lifecycle: market.lifecycle || "APPEARED",
    priceReactionConfirmed: market.priceReactionConfirmed === true
  });
  const caption = renderMarketIntelligenceAlertText(alert, { html: true });
  const contentHash = createHash("sha256").update(JSON.stringify({
    sourceName,
    ...
    price: alert.price,
    visibleNotional: alert.visibleNotional,
    priority: alert.priority
  })).digest("hex");
  return {
    imageKind: "whale",
    metrics: [`Depth ${alert.depthImbalance}`, `Score ${alert.score}/100`],
    poster: {
      alertType: "LIQUIDITY ALERT",
      pair: "BTC / USDT",
      signal: `VISIBLE ${alert.side}`,
      amount: alert.visibleNotionalLabel,
      price: `$${formatNumber(alert.price)}`,
      status: `${alert.priority || "MONITOR"} · ${alert.bias} · SCORE ${alert.score}`,
      interpretation: alert.interpretation,
      watchNext: alert.watchNext,
      evidenceSnapshot: alert.evidenceSnapshot,
    },
    caption,
    items: { ...market, alert },
    alert,
    publishable: alert.publishable,
    suppressionReason: alert.publishable ? null : `Market Intelligence Alert suppressed: ${alert.publicationGate.reason}.`,
    contentHash,
  };
}
...
async function fetchWhaleMarketData() {
  try {
    ...
    return {
      ...
      sourceTimestamp: new Date(Number(depth.T || depth.E || Date.now())).toISOString(),
      receivedAt,
    ...
    const receivedAt = new Date().toISOString();
    return {
      bids: book.bids.map(([price, contracts]) => [price, multiplyDecimal(contracts, contractBtc, 8)]),
      asks: book.asks.map(([price, contracts]) => [price, multiplyDecimal(contracts, contractBtc, 8)]),
      openInterest: oi?.oiCcy ? String(oi.oiCcy) : multiplyDecimal(oi?.oi || "0", contractBtc, 8),
      funding: Number(funding?.fundingRate || 0) * 100,
      markPrice: mark.markPx,
      source: "OKX",
      endpoint: "/api/v5/market/books",
      sourceTimestamp: new Date(Number(book.ts || Date.now())).toISOString(),
      receivedAt,
      persistenceSeconds: 0
    };
  }
...
async function buildAgentUpdates(now, { persist = true } = {}) {
  ...
  for (const item of packages.filter((pkg) => pkg.platform === "YouTube" && pkg.status === "已启用" && pkg.postMonitoring && pkg.postSyncMode !== 'verified')) {
...
export async function previewSocialSource(source) {
  ...
  if (normalized.postSyncMode === 'verified') {
    const { fetchSocialPosts } = await import('./social-post-sources.mjs');
    const result = await fetchSocialPosts(normalized, { signal: AbortSignal.timeout(45_000) });
    const latest = [...result.posts].sort((a, b) => Date.parse(b.publishedAt) - Date.parse(a.publishedAt))[0];
    return { agent: normalized.agent, platform: normalized.platform, title: latest?.title || 'No ordinary posts available', url: latest?.url || normalized.accountUrl, publishedAt: latest?.publishedAt || '', reliability: result.warning ? 'degraded' : 'stable', ...result };
  }
...
async function sendAgentUpdates(updates, configuredTargets, options = {}) {
  ...
  const resultMap = new Map(targets.map((target) => [targetKey(target), {
    target,
    status: options.deferDelivery ? "pending" : "success",
    messageIds: []
  }]));
  ...
  for (const plan of deliveryPlans) {
    ...
    if (options.deferDelivery) continue;
...
function createRunEntry({ job, slot, dryRun, status, message, target = null, preview = null }) {
  return { id: `${job.id}-${Date.now()}`, jobId: job.id, jobName: job.name, slot, dryRun, status, message, target, preview, createdAt: new Date().toISOString() };
}

function createRunRecorder(options, dryRun) {
  if (dryRun && options.readOnlyPreview === true) {
    return async (details) => createRunEntry(details);
  }
  if (typeof options.runLogWriter === "function") {
    return async (details) => options.runLogWriter(createRunEntry(details));
  }
  return logRun;
}

async function logRun(details) {
  const entry = createRunEntry(details);
  const runs = await readJson(runsPath, []);
  await writeJson(runsPath, [entry, ...(Array.isArray(runs) ? runs : [])].slice(0, 100));
  return entry;
}

export function buildMarketMediaDelivery(generated, options = {}) {
  if (options.textOnly === true) return { defaultUrl: null, byTemplateId: {} };
  const variants = generated?.posterVariants && typeof generated.posterVariants === "object"
    ? Object.values(generated.posterVariants)
    : [];
  const posters = variants.length ? variants : [generated?.poster].filter(Boolean);
  const byTemplateId = {};
  for (const poster of posters) {
    const templateId = poster?.visualTemplate?.id;
    if (!templateId || !poster?.kind) continue;
    const isCanonicalPoster = templateId === generated?.poster?.visualTemplate?.id;
    byTemplateId[templateId] = isCanonicalPoster && generated?.imageUrl
      ? generated.imageUrl
      : buildCardUrl(poster.kind, [], poster, {
        baseUrl: options.baseUrl,
        cacheKey: `${generated?.contentHash ?? "market"}-${templateId}`,
      });
  }
  const defaultUrl = generated?.imageUrl
    ?? Object.values(byTemplateId)[0]
    ?? (generated?.imageKind
      ...
      : null);
  return { defaultUrl, byTemplateId };
}

function mediaUrlForPlan(mediaDelivery, plan) {
  if (typeof mediaDelivery === "string") return mediaDelivery;
  if (!mediaDelivery || typeof mediaDelivery !== "object") return null;
  if (plan && !plan.media) return null;
  if (plan) return mediaDelivery.byTemplateId?.[plan.media.templateId] ?? null;
  return mediaDelivery.defaultUrl ?? null;
}
...
export function buildCardUrl(kind, metrics = [], poster = {}, options = {}) {
  ...
  if (["crypto-daily", "weekly-calendar", "data-update", "whale"].includes(kind) && poster && Object.keys(poster).length) {
    query.set("data", Buffer.from(JSON.stringify(poster)).toString("base64url"));
  }
...
export function assertNoInternalUrlsInDeliveryPlans(deliveryPlans = [], publicBaseUrl) {
  const internalOrigin = resolveAppBaseUrl(publicBaseUrl).toLowerCase();
  for (const plan of Array.isArray(deliveryPlans) ? deliveryPlans : []) {
    for (const step of Array.isArray(plan?.steps) ? plan.steps : []) {
      const publicText = [step?.payload?.text, step?.payload?.content, step?.payload?.caption]
        .filter((value) => typeof value === "string")
        .join("\n")
        .replaceAll("&amp;", "&")
        .toLowerCase();
      if (publicText.includes(internalOrigin)) throw new Error("PUBLIC_MESSAGE_INTERNAL_URL_BLOCKED");
    }
  }
...
async function fetchSocialSourceUnchecked(source, options = {}) {
  const plan = socialFetchPlan(source, options);
  ...
  if (plan.kind === "x-api") {
    try {
      return await fetchLatestXPost(source);
    } catch {
      // A Spaces key may lack credits or timeline access. Keep the existing
      // public-post path available; ownership validation still runs above.
      return fetchSocialSourceUnchecked(source, { hasXToken: false });
    }
  }
