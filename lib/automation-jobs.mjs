  ...
  return (Array.isArray(targets) ? targets : []).filter(isTelegramDestination).map((target) => ({
    ...
    steps: MARKET_DOCUMENT_JOBS.has(jobId)
      ? paragraphChunksWithFinalCta(renderTelegramMarketDocument(generated.document), target, 4096, { html: true })
        .map((text) => {
          const ctaBoundary = destinationCtaBoundary(text, target, "text", { html: true });
          return {
            method: "sendMessage",
            payload: {
              ...telegramDestinationPayload(target),
...
function destinationCtaBoundary(value, target, field, options = {}) {
  const payload = String(value || "");
  const cta = targetCtaBlock(target, options);
  if (!cta || !payload.endsWith(cta)) return null;
  return {
    kind: "destination-cta",
    placement: "suffix",
    field,
    start: payload.length - cta.length,
    end: payload.length,
  };
}
    ...
    if (MARKET_DOCUMENT_JOBS.has(jobId)) {
      return {
        ...
        steps: paragraphChunksWithFinalCta(renderDiscordMarketDocument(generated.document), target, 2000)
          .map((content) => {
            const ctaBoundary = destinationCtaBoundary(content, target, "content");
            return {
              method: "sendMessage",
              payload: { content },
              ...(ctaBoundary ? { ctaBoundary } : {})
