    ...
    if (jobId === "agent-sync-4h") {
      ...
      const message = status === "queued"
        ...
        : sendResult.sent
          ...
          : "已完成抓取，本轮没有新内容。";
      return logRun({ job, slot, dryRun, status, message, target: sendResult.targets, preview: { ...preview, deliveryPlans: sendResult.deliveryPlans, targetResults: sendResult.targetResults } });
    }
...
export function buildAgentUpdateTelegramPlans(updates, targets, now = new Date()) {
  ...
  return (Array.isArray(targets) ? targets : []).filter(isAutomationDestination).map((target) => ({
    target,
    contentPolicy: "fixed-template",
...
async function sendAgentUpdates(updates, configuredTargets, options = {}) {
  let sent = 0;
  const validUpdates = (Array.isArray(updates) ? updates : []).filter((update) => String(update?.url || "").trim());
  const configured = (Array.isArray(configuredTargets) ? configuredTargets : []).filter(isAutomationDestination);
  const assignments = [];
  if (configured.length) {
    assignments.push(...configured.map((target) => ({ target, updates: validUpdates })));
  } else {
    for (const update of validUpdates) {
      const target = await resolveAgentTarget(update.agent);
      if (!isAutomationDestination(target)) continue;
      const key = target.chatType === "channel" ? `${target.chatId}:channel` : `${target.chatId}:${target.threadId}`;
  ...
  const deliveryPlans = assignments.flatMap((item) => buildAgentUpdateTelegramPlans(item.updates, [item.target], options.now));
  ...
  const resultMap = new Map(targets.map((target) => [targetKey(target), { target, status: options.deferDelivery ? "pending" : "success", messageIds: [] }]));
  if (options.deferDelivery === true) {
    return { sent, targets, targetResults: [...resultMap.values()], deliveryPlans, status: deliveryPlans.length ? "queued" : "skipped" };
  }
  const token = tokenForBot("SpeakerBot");
  if (!token) return { sent, targets, targetResults: targets.map((target) => ({ target, status: "failed", error: "SpeakerBot Token 未配置", messageIds: [] })), deliveryPlans, status: "partial" };
  for (const plan of deliveryPlans) {
    const key = targetKey(plan.target);
    for (const step of plan.steps) {
      try {
        const message = await telegramCall(token, step.method, step.payload);
  ...
  return { sent, targets, targetResults, deliveryPlans, status: targetResults.every((item) => item.status === "success") ? "success" : "partial" };
}
...
async function fetchSocialSource(source) {
  ...
  if (plan.kind === "x-profile") {
    ...
    const readerUrl = `https://r.jina.ai/https://x.com/${encodeURIComponent(plan.username)}`;
    const readerResponse = await fetchWithTimeout(readerUrl, { headers: browserHeaders() });
    if (!readerResponse.ok) {
      throw new Error(`${profileError?.message || "X 公开主页不可读"}；Reader 返回 HTTP ${readerResponse.status}`);
    }
