...
const dataReleaseRunLocks = new WeakMap();
...
export async function runAutomationJob(jobId, options = {}) {
  ...
  if (jobId === "data-release-updates" && !options[DATA_RELEASE_RUN_LOCKED]) {
    ...
    const previous = dataReleaseRunLocks.get(lockRepository) ?? Promise.resolve();
    const current = previous.catch(() => undefined).then(() => runAutomationJob(jobId, {
      ...options,
      repository: lockRepository,
      [DATA_RELEASE_RUN_LOCKED]: true
    }));
    dataReleaseRunLocks.set(lockRepository, current);
    return current.finally(() => {
      if (dataReleaseRunLocks.get(lockRepository) === current) dataReleaseRunLocks.delete(lockRepository);
    });
  }
  ...
  try {
    ...
    for (const item of telegramTargets) {
      ...
      try {
        ...
        if (jobId === "data-release-updates") await acknowledgeDataReleaseTarget({ repository: options.repository, deduplicationKey: generated.deduplicationKey, targetKey: buildDataReleaseTargetKey(item), event: generated.event, now });
      } catch (error) {
