...
export async function runAutomationJob(jobId, options = {}) {
  ...
  try {
    ...
    let generated = await buildContent(jobId, now, {
    ...
    if (["weekly-calendar", "data-release-updates"].includes(jobId) && generated.publishable === true) {
      try {
        ...
        const preview = { ...generated, ...automationTemplateMetadata(jobId), publicationError: error.message, target };
        return recordRun({ job, slot, dryRun, status: "skipped", message: "未发布：发布物页面或图片验证失败。", target, preview });
      }
    }
    const imageUrl = generated.imageUrl ?? (generated.imageKind
      ...
      : null);
    let preview = { ...generated, ...automationTemplateMetadata(jobId), imageUrl, target };
    ...
    if (MARKET_DOCUMENT_JOBS.has(jobId)) {
      try {
        ...
        preview = { ...generated, ...automationTemplateMetadata(jobId), imageUrl, target };
        ...
        preview = { ...generated, contentGovernance, ...automationTemplateMetadata(jobId), imageUrl, target };
        return recordRun({
          job,
          slot,
          dryRun,
...
export function buildAutomationTelegramPlans(jobId, generated, targets, imageUrl) {
  ...
  const governedPlan = MARKET_DOCUMENT_JOBS.has(jobId) && generated?.contentGovernance?.approved === true
    ? generated.contentGovernance.channelPlans?.[0]
    : null;
  return (Array.isArray(targets) ? targets : []).filter(isTelegramDestination).map((target) => ({
    ...
    contentPolicy: governedPlan ? "obsidian-canonical" : "fixed-template",
    ...(governedPlan ? { contentProductId: governedPlan.productId, contentHash: governedPlan.contentHash } : {}),
    steps: MARKET_DOCUMENT_JOBS.has(jobId)
      ? [
        ...
        ...(governedPlan
          ? channelChunksWithFinalCta(governedPlan.telegram?.chunks, target, 4096, { html: true })
  ...
  return (Array.isArray(targets) ? targets : []).filter(isDiscordDestination).map((target) => {
    if (MARKET_DOCUMENT_JOBS.has(jobId)) {
      return {
        ...
        ...(governedPlan ? { contentProductId: governedPlan.productId, contentHash: governedPlan.contentHash } : {}),
        steps: (governedPlan
          ? channelChunksWithFinalCta(governedPlan.discord?.chunks, target, 2000)
