...
import { cacheWeeklyCalendar, pollDataReleaseUpdates } from "./data-release-monitor.mjs";
import {
...
export async function runAutomationJob(jobId, options = {}) {
  ...
  try {
    ...
    const telegramTargets = configuredTargets.filter(isTelegramDestination);
    const discordTargets = configuredTargets.filter(isDiscordDestination);
    const deliveryPlans = [
    ...
    for (const item of telegramTargets) {
      if (options.deferDelivery === true) {
        targetResults.push({ target: item, status: "pending", messageId: null, messageIds: [] });
        continue;
      }
    ...
    await writeJson(statePath, state);
    return logRun({ job, slot, dryRun, status, message: `内容已发送至 ${succeeded}/${targetResults.length} 个目标${pending ? `，${pending} 个 Telegram 目标等待本机发布桥` : ""}。`, target: configuredTargets, preview: { ...preview, deliveryPlans, targetResults } });
  } catch (error) {
...
function marketEnvelope(templateId, document, result = {}) {
  return {
    templateId,
...
async function buildCryptoDailyContent(now, { fetchImpl = fetch } = {}) {
  ...
  return marketEnvelope("crypto-daily", document, {
    ...result,
    publishable: true,
...
async function buildWeeklyCalendarContent(now, { fetchImpl = fetch, repository, persist = false } = {}) {
  ...
  return marketEnvelope("weekly-calendar", document, {
    ...result,
    publishable: true,
...
async function buildDataReleaseContent(now, { fetchImpl = fetch, repository, persist = false } = {}) {
  const resolvedRepository = repository || await getDistributionRepository();
  const result = await pollDataReleaseUpdates({
    ...
    fetchReaction: ({ event, now: checkedAt }) => fetchMarketReaction({
  ...
  return marketEnvelope("data-release-updates", document, {
    ...result,
...
export function buildAutomationTelegramPlans(jobId, generated, targets, imageUrl) {
  return (Array.isArray(targets) ? targets : []).filter(isTelegramDestination).map((target) => ({
    ...
    steps: MARKET_DOCUMENT_JOBS.has(jobId)
      ? paragraphChunks(withFinalCta(renderTelegramMarketDocument(generated.document), target, { html: true }), 4096)
...
function withFinalCta(value, target, options = {}) {
  const body = String(value || "").trim();
  const cta = targetCtaBlock(target, options);
  return cta ? (body ? `${body}\n\n${cta}` : cta) : body;
}

function paragraphChunks(value, exclusiveLimit) {
  const paragraphs = String(value || "").trim().split(/\n{2,}/).filter(Boolean);
  const chunks = [];
  let current = "";
  for (const paragraph of paragraphs) {
    if (paragraph.length >= exclusiveLimit) {
      throw new RangeError(`A paragraph exceeds the ${exclusiveLimit - 1} character platform limit.`);
    }
    const candidate = current ? `${current}\n\n${paragraph}` : paragraph;
    if (candidate.length >= exclusiveLimit) {
      chunks.push(current);
      current = paragraph;
    } else {
      current = candidate;
...
export function buildAutomationDiscordPlans(jobId, generated, targets, imageUrl) {
  return (Array.isArray(targets) ? targets : []).filter(isDiscordDestination).map((target) => {
    if (MARKET_DOCUMENT_JOBS.has(jobId)) {
      const rendered = withFinalCta(renderDiscordMarketDocument(generated.document), target);
      return {
        target,
        templateVersion: MARKET_CONTENT_TEMPLATE_VERSION,
        contentPolicy: "fixed-template",
        steps: paragraphChunks(rendered, 2000).map((content) => ({ method: "sendMessage", payload: { content } }))
      };
    }
