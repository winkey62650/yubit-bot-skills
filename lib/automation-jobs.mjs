...
export async function runAutomationJob(jobId, options = {}) {
  ...
  try {
    ...
    return recordRun({ job, slot, dryRun, status, message, target: configuredTargets, preview: {
      ...preview,
      deliveryPlans,
      targetResults,
  ...
  return (Array.isArray(targets) ? targets : []).filter(isDiscordDestination).map((target) => {
    if (MARKET_DOCUMENT_JOBS.has(jobId)) {
      return {
        ...
        contentPolicy: "fixed-template",
        steps: paragraphChunksWithFinalCta(renderDiscordMarketDocument(marketDocument), target, 2000)
