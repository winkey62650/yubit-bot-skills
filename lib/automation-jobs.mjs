...
const appBaseUrl = (process.env.APP_BASE_URL || process.env.NEXT_PUBLIC_APP_URL || "https://yubit-bot-skills-academy.vercel.app").replace(/\/$/, "");

export const AUTOMATION_JOBS = [
  { id: "news-feed", name: "Crypto News", schedule: "最短每 5 分钟", cron: "*/5 * * * *", topic: "YUBIT Updates", bot: "SpeakerBot", content: "新闻图文" },
  { id: "daily-events", name: "Daily Events", schedule: "每日 08:00 UTC", cron: "0 8 * * *", topic: "Market Events", bot: "SpeakerBot", content: "图文同步" },
  { id: "daily-analysis", name: "Daily Analysis", schedule: "每日 08:00 UTC", cron: "0 8 * * *", topic: "Market Analysis", bot: "SpeakerBot", content: "图文分析" },
  { id: "whale-hourly", name: "大户挂单 & 巨鲸数据", schedule: "每日 08:00 UTC", cron: "0 8 * * *", topic: "Smart Money Tracker", bot: "SpeakerBot", content: "图文分析" },
  { id: "agent-sync-4h", name: "代理群信息更新", schedule: "每 4 小时", cron: "15 */4 * * *", topic: "Trading Zone", bot: "SpeakerBot", content: "有更新时发布" }
];
...
export function automationSlot(jobId, date = new Date()) {
  ...
  if (jobId === "daily-events" || jobId === "daily-analysis" || jobId === "whale-hourly") return iso.slice(0, 10);
...
export function distributionTargetsForJob(job, rules = []) {
  ...
  const targets = matchingRules.flatMap((rule) => (rule.targets || []).map((target) => ({
    ...
    const key = `${target.chatId}:${target.threadId || 0}`;
    if (!target.chatId || !target.threadId || seen.has(key)) return false;
    seen.add(key);
    return true;
  });
...
export async function runAutomationJob(jobId, options = {}) {
  ...
  try {
    ...
    const imageUrl = generated.imageKind ? buildCardUrl(generated.imageKind, generated.metrics, generated.poster) : null;
    const preview = { ...generated, imageUrl, target };
    ...
    const configuredTargets = targets.filter((item) => item?.chatId && item?.threadId);
    ...
    for (const item of configuredTargets) {
      const messageIds = [];
      try {
        const plan = jobId === "daily-events"
          ? buildDailyMarketBriefTelegramPlan(generated, item, imageUrl)
...
async function buildDailyEvents(now) {
  ...
  if (process.env.DAILY_EVENTS_API_URL) {
    ...
    const data = await fetchJson(url, { headers: { "user-agent": "Mozilla/5.0", origin: "https://www.tradingview.com", referer: "https://www.tradingview.com/" } });
    payload = { date: day, stories: (data.result || data.events || [])
      .filter((event) => Number(event.importance ?? event.impact ?? 0) >= 1)
      .sort((a, b) => new Date(a.date || a.time || 0) - new Date(b.date || b.time || 0))
      .map((event) => ({
        title: event.title || event.name || "Market event",
        summary: `${event.country || event.currency || "Global"} market event${event.date || event.time ? ` scheduled for ${new Date(event.date || event.time).toISOString().slice(11, 16)} UTC` : ""}.`
      })),
      summary: "Today's brief highlights the most relevant scheduled market catalysts across global markets and crypto.",
      subline: "GLOBAL MARKETS · CRYPTO · KEY CATALYSTS" };
  }
...
export function buildDailyMarketBrief(payload, now = new Date()) {
  ...
  const normalized = rawStories.map((story) => {
    ...
    if (title && summary && !summary.toLowerCase().startsWith(title.toLowerCase())) return `${title}: ${summary}`;
    return summary || title;
  }).filter(Boolean);
  const items = [];
  let used = 0;
  for (const item of normalized) {
    const next = `${items.length + 1}. ${item}`;
    if (used + next.length > 3400 && items.length) break;
    items.push(item);
    used += next.length + 2;
  }
  if (!items.length) items.push("No material market event was available from the configured source at publication time.");
  ...
  const summary = String(body.summary || body.overview || body.dek || items.slice(0, 3).join(" ")).trim();
  const subline = String(body.subline || body.tagline || "GLOBAL MARKETS · CRYPTO · COMPANIES").trim().toUpperCase();
  const caption = `<b>🌅 ${escapeHtml(headline)}</b>\n\n${escapeHtml(summary)}\n\nThe full English brief follows as a second Telegram message. Story count is selected by significance, not a fixed quota.`;
  const fullText = `<b>${escapeHtml(headline)}</b>\n\n${items.map((item, index) => `${index + 1}. ${escapeHtml(item)}`).join("\n\n")}\n\n<i>Market commentary only. Verify figures and claims against primary sources before publishing.</i>`;
  return {
    imageKind: "events",
    metrics: [],
...
export function buildDailyAnalysisSnapshot(rows, now = new Date()) {
  const lines = rows.map((row) => `• <b>${row.symbol}</b> $${formatNumber(row.price)} · ${signed(row.change)}% · ${row.trend} vs SMA20`);
  ...
  return {
    ...
    caption: `<b>📊 DAILY MARKET ANALYSIS · ${dateLabel}</b>\n\n<b>Market regime:</b> ${regime}\n\n${lines.join("\n")}\n\n<b>Key read:</b> ${breadth} ${momentum}\n\n<b>Levels to watch:</b> ${levels}\n<b>Catalyst:</b> ${catalyst}\n\n<i>${escapeHtml(sources)} market data · Educational market commentary only. Not investment advice.</i>`,
    items: rows
  };
}
...
export function buildWhaleAlert(market, now = new Date()) {
  ...
  const sourceUrl = sourceName === "OKX"
  ...
  const action = isBid ? "买单挂入" : "卖单挂入";
  const state = isBid ? "买墙支撑" : "卖墙压力";
  const directionRead = isBid
    ? "如果这笔买单继续增加并保持，短线支撑可能增强；若大单被快速成交或撤销，则要警惕支撑减弱或方向反转。"
    : "如果这笔卖单继续增加，短线可能带来更强卖压；若大单被快速承接或撤销，则要警惕方向反转。";
  const caption = `<b>🐋 巨鲸动了，市场正在重新定价</b>\n\n${timestamp} UTC，BTC/USDT 永续合约订单簿快照显示一笔值得关注的大额挂单：\n\n▪️ <b>异动规模：</b>${formatQuantity(order.qty)} 枚 $BTC，约合 $${formatCompact(order.notional)}\n▪️ <b>关键动作：</b>${action}\n▪️ <b>关键位置：</b>$${formatNumber(order.price)}\n▪️ <b>当前状态：</b>${state}约 $${formatCompact(order.notional)}；前 100 档买卖盘失衡 ${signed(imbalance)}%\n\n${directionRead}\n\n<b>下一步重点观察：</b>$${formatNumber(order.price)} 附近挂单是否成交、增加或撤销。\n\n⚠️ 订单簿快照不等于已完成买卖，挂单也可能随时撤销。以上内容仅为市场信息追踪，不构成投资建议。\n\n📍<b>数据来源：</b><a href="${sourceUrl}">${escapeHtml(sourceName)} Futures order book</a>\n\n#BTC #${escapeHtml(sourceName)} #WhaleAlert #SmartMoney`;
  return {
    imageKind: "whale",
...
async function sendAgentUpdates(updates, configuredTargets) {
  ...
  const resultMap = new Map(targets.map((target) => [`${target.chatId}:${target.threadId}`, { target, status: "success", messageIds: [] }]));
  ...
  for (const update of updates) {
    ...
    for (const target of resolvedTargets) {
      if (!target?.chatId || !target?.threadId) continue;
      ...
      try {
        const message = await telegramCall(token, "sendMessage", {
          chat_id: target.chatId,
          message_thread_id: target.threadId,
          text: `<b>🎯 ${escapeHtml(update.agent)} Update</b>\n\n${escapeHtml(update.title)}\n${escapeHtml(update.description || "")}\n\n${escapeHtml(update.url)}`,
          parse_mode: "HTML",
...
async function resolveTarget(job) {
  ...
  const binding = bindings.find((item) => item.status !== "暂停" && topicMatches(item.topic, job.topic));
  if (!binding) return { configured: false, topic: job.topic };
  const group = groups.find((item) => item.title === binding.group);
  const topic = group?.topics?.find((item) => topicMatches(item.name, binding.topic));
...
function topicMatches(value, expected) {
  const normalized = String(value || "").replace(/^[^\p{Letter}\p{Number}]+/u, "").toLowerCase();
...
export function buildCardUrl(kind, metrics = [], poster = {}) {
  ...
  return `${appBaseUrl}/api/media/card?${query}`;
}
