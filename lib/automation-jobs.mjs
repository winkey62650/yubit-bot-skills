...
export async function runAutomationJob(jobId, options = {}) {
  ...
  try {
    ...
    const imageUrl = options.textOnly === true ? null : generated.imageUrl ?? (generated.imageKind
      ...
      : null);
    let preview = { ...generated, ...automationTemplateMetadata(jobId), imageUrl, textOnly: options.textOnly === true, demoShowcase: options.demoShowcase === true, target };
    ...
    if (MARKET_DOCUMENT_JOBS.has(jobId)) {
      try {
        ...
        preview = { ...generated, ...automationTemplateMetadata(jobId), imageUrl, textOnly: options.textOnly === true, demoShowcase: options.demoShowcase === true, target };
        ...
        preview = { ...generated, contentGovernance, ...automationTemplateMetadata(jobId), imageUrl, textOnly: options.textOnly === true, demoShowcase: options.demoShowcase === true, target };
        return recordRun({
          job,
          slot,
          dryRun,
...
function marketEnvelope(templateId, document, now, result = {}) {
  ...
  const poster = document
    ? templateId === "crypto-daily"
      ? buildCryptoDailyPosterModel(document)
      : templateId === "weekly-calendar"
        ? buildWeeklyCalendarPosterModel(document)
        : templateId === "data-release-updates"
          ? buildDataUpdatePosterModel(document)
          : null
    : null;
  const imageKind = poster?.kind ?? null;
  const contentHash = poster
    ? createHash("sha256").update(JSON.stringify({ templateId, poster, document })).digest("hex")
    : null;
  return {
    templateId,
    ...
    diagnostics,
    ...(poster ? { poster, imageKind, metrics: [], contentHash } : {}),
...
export function buildAutomationTelegramPlans(jobId, generated, targets, imageUrl) {
  ...
  return (Array.isArray(targets) ? targets : []).filter(isTelegramDestination).map((target) => {
    const steps = MARKET_DOCUMENT_JOBS.has(jobId)
      ? [
        ...(imageUrl ? [{ method: "sendPhoto", payload: {
          ...telegramDestinationPayload(target),
          photo: imageUrl
        } }] : []),
        ...(governedPlans.length
          ? governedPlans.flatMap((plan) => channelChunksWithFinalCta(plan.telegram?.chunks, target, 4096, { html: true }))
          : paragraphChunksWithFinalCta(renderTelegramMarketDocument(marketDocument), target, 4096, { html: true }))
        .map(({ content: text, ctaRange }, stepIndex, steps) => {
          const ctaBoundary = destinationCtaBoundary(ctaRange, {
            platform: "telegram",
            method: "sendMessage",
            field: "text",
            stepIndex: stepIndex + (imageUrl ? 1 : 0),
            stepCount: steps.length + (imageUrl ? 1 : 0),
          });
          return {
            method: "sendMessage",
            payload: {
...
export function buildAutomationDiscordPlans(jobId, generated, targets, imageUrl) {
  ...
  return (Array.isArray(targets) ? targets : []).filter(isDiscordDestination).map((target) => {
    if (MARKET_DOCUMENT_JOBS.has(jobId)) {
      const governedChunks = governedPlans.flatMap((plan) => channelChunksWithFinalCta(plan.discord?.chunks, target, 2000));
      return {
        target,
