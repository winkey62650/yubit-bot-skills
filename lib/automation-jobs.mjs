...
export const AUTOMATION_JOBS = [
  ...
  { id: "whale-hourly", name: "大户挂单 & 巨鲸数据", schedule: "每小时检查，重大异动才发布", cron: "0 * * * *", topic: "Smart Money Tracker", bot: "SpeakerBot", content: "英文异动图文" },
...
export async function runAutomationJob(jobId, options = {}) {
  ...
  try {
    ...
    if (jobId === "whale-hourly" && generated.publishable === false) {
...
async function buildDailyEvents(now) {
  ...
  if (process.env.DAILY_EVENTS_API_URL) {
    ...
    const cryptoCount = newsStories.length;
    payload = {
      date: day,
      stories,
      summary: `Today's desk brief prioritizes ${cryptoCount ? `${cryptoCount} timely crypto and market developments` : "the strongest scheduled catalysts"}${calendarStories.length ? ` alongside ${calendarStories.length} high-impact macro releases` : ""}. Each item includes a source link so operators can verify the underlying report before distribution.`,
...
export function buildDailyMarketBrief(payload, now = new Date()) {
  ...
  for (const item of normalized) {
    const next = `${items.length + 1}. ${item.text}`;
  ...
  const caption = `<b>🌅 ${escapeHtml(headline)}</b>\n\n<b>Executive read</b>\n${escapeHtml(summary)}\n\nThe full English brief follows as a second Telegram message, with source links for verification. Story count is determined by significance, not a fixed quota.`;
  const fullText = `<b>${escapeHtml(headline)}</b>\n\n${items.map((item, index) => {
    const category = item.category ? `<b>${escapeHtml(item.category.toUpperCase())}</b> · ` : "";
    const source = item.url && item.source ? `\n<i>Source: <a href="${escapeHtml(item.url)}">${escapeHtml(item.source)}</a></i>` : item.source ? `\n<i>Source: ${escapeHtml(item.source)}</i>` : "";
    return `${index + 1}. ${category}${escapeHtml(item.text)}${source}`;
  }).join("\n\n")}\n\n<i>Market commentary only. Open the source links and verify material figures before publishing.</i>`;
  return {
    imageKind: "events",
    metrics: [],
...
export function buildWhaleAlert(market, now = new Date()) {
  ...
  const caption = `<b>🐋 WHALE ALERT · SMART MONEY SIGNAL</b>\n\nAt ${timestamp} UTC, the BTC/USDT perpetual order book showed a material liquidity concentration:\n\n▪️ <b>Visible size:</b> ${formatQuantity(order.qty)} BTC · approx. $${formatCompact(order.notional)}\n▪️ <b>Key action:</b> ${action}\n▪️ <b>Key level:</b> $${formatNumber(order.price)}\n▪️ <b>Current read:</b> ${state} near $${formatCompact(order.notional)}; top-100 depth imbalance ${signed(imbalance)}%\n\n${directionRead}\n\n<b>What to watch next:</b> Whether liquidity near $${formatNumber(order.price)} is filled, increased or cancelled.\n\n⚠️ An order-book snapshot does not mean a trade has been executed, and visible orders can be changed or cancelled at any time. Market information only; not investment advice.\n\n📍<b>Data source:</b> <a href="${sourceUrl}">${escapeHtml(sourceName)} Futures order book</a>\n\n#BTC #${escapeHtml(sourceName)} #WhaleAlert #SmartMoney`;
