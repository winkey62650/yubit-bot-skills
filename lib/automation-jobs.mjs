...
export async function runAutomationJob(jobId, options = {}) {
  ...
  if (!dryRun && !force && !DURABLE_MARKET_DELIVERY_JOBS.has(jobId) && state[stateKey]?.slot === slot) {
  ...
  try {
    ...
    if (jobId === "whale-hourly" && shouldSuppressWhaleSignal(generated, { force })) {
      return recordRun({ job, slot, dryRun, status: "skipped", message: generated.suppressionReason || "本轮没有达到发布标准的巨鲸或大户挂单异动。", target, preview });
    }
    ...
    if (jobId === "whale-hourly" && generated.contentHash && state[stateKey]?.contentHash === generated.contentHash) {
      ...
      if (Number.isFinite(previousAt) && now.getTime() - previousAt < cooldownMs) {
        return recordRun({ job, slot, dryRun, status: "duplicate", message: "同一巨鲸信号仍在冷却期内，已阻止重复发布。", target, preview });
      }
    }
    ...
    state[stateKey] = { slot, at: now.toISOString(), contentHash: generated.contentHash || null, status: effectiveStatus, targets: configuredTargets, targetResults };
    await writeJson(statePath, state);
