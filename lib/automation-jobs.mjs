...
export function buildDailyMarketBrief(payload, now = new Date()) {
  ...
  const fullText = `<b>${escapeHtml(headline)}</b>\n\n${items.map((item, index) => {
    ...
    const sourceUrl = compactSourceUrl(item.url);
    const source = sourceUrl && item.source ? `\n<i>Source: <a href="${escapeHtml(sourceUrl)}">${escapeHtml(item.source)}</a></i>` : item.source ? `\n<i>Source: ${escapeHtml(item.source)}</i>` : "";
