...
export function createYubitSignature({ apiKey, apiSecret, timestamp, recvWindow, method = "GET", path = "", params }) {
  const payload = encodeQuery(params || {});
  const signature = createHmac("sha256", String(apiSecret || ""))
    .update(`${String(method).toUpperCase()}${path}${timestamp}${apiKey}${recvWindow}${payload}`)
    .digest("hex");
