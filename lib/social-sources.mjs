...
export function isYouTubeSocialSource(source) {
  return detectSocialPlatform(`${source?.platform || ""} ${source?.accountUrl || source?.url || ""}`) === "YouTube";
}
...
export function normalizeSocialPackages(packages) {
  return (Array.isArray(packages) ? packages : [])
    .map((item, index) => {
      ...
      return {
        id: String(item?.id || generatedSocialPackageId(item, index, agent, platform, accountUrl)),
        ...
        frequency: item?.postSyncMode === 'verified' ? '每 5 分钟' : '每小时',
        postMonitoring: item?.postMonitoring !== false,
        ...(item?.postSyncMode === 'verified' ? { postSyncMode: 'verified' } : {}),
...
function generatedSocialPackageId(item, index, agent, platform, accountUrl) {
  const base = normalizeName(agent || item?.name || index + 1);
  const platformSlug = normalizeName(platform || "other");
  const identity = accountUrl || item?.feedUrl || item?.name || String(index + 1);
  const suffix = createHash("sha256").update(String(identity)).digest("hex").slice(0, 8);
  return `social-${base}-${platformSlug}-${suffix}`;
}

export function applySocialPackageMutation(currentPackages = [], mutation = {}) {
  const packages = normalizeSocialPackages(currentPackages);
  const action = String(mutation?.action || "").trim();

  if (action === "upsert") {
    const source = normalizeSocialPackages([mutation?.source || {}])[0];
    if (!source) throw new Error("社媒规则缺少名称或代理");
    const index = packages.findIndex((item) => item.id === source.id);
    if (index < 0) return [...packages, source];
    return packages.map((item, itemIndex) => (itemIndex === index ? source : item));
  }

  const id = String(mutation?.id || "").trim();
  const index = packages.findIndex((item) => item.id === id);
  if (!id || index < 0) {
    throw new Error("未找到需要更新的社媒规则，请刷新页面后重试");
  }

  if (action === "set-status") {
    const status = mutation?.status === "已暂停" ? "已暂停" : "已启用";
    return packages.map((item, itemIndex) =>
      itemIndex === index ? { ...item, status } : item
    );
  }

  if (action === "delete") {
    return packages.filter((item) => item.id !== id);
  }

  throw new Error("不支持的社媒规则操作");
}
...
export function parseXReaderTimeline(markdown, username, { all = false } = {}) {
  ...
  return all ? posts : posts.reduce((newest, candidate) => comparePostIds(candidate.externalId, newest.externalId) > 0 ? candidate : newest);
}
