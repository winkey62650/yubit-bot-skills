...
const BEA_GDP_PATH_PATTERN = /^\/news\/(20\d{2})\/gdp-(?:(?:advance|second|third)-estimate-(1st|2nd|3rd|4th)-quarter(?:-[a-z0-9-]+)?|third-estimate-industries-corporate-profits-state-gdp-and-state-personal-income-(1st|2nd|3rd|4th))$/i;
...
function beaReleaseArticle(html, sourceUrl, expectedPath, expectedHeading) {
  ...
  schema(/\bclass=["'][^"']*(?:\brelease-body\b|\bfield--name-body\b)[^"']*["']/i.test(articleMatch[2]), "BEA release body is missing");
  ...
  return { html: articleMatch[0], text: stripTags(articleMatch[0]), heading, path: articlePath };
}
...
function parseBeaGdp(html, retrievedAt, sourceUrl) {
  ...
  const pathPeriod = capture(article.path, BEA_GDP_PATH_PATTERN, "GDP release path is invalid");
  schema((pathPeriod[2] ?? pathPeriod[3]).toLowerCase() === quarter[1].toLowerCase() && pathPeriod[1] === quarter[2], "GDP release path does not match the heading period");
