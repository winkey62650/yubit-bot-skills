...
function parseBlsCpi(html, retrievedAt) {
  ...
  const headlineMonthly = capture(
    body,
    /\bConsumer Price Index for All Urban Consumers\s*\(CPI-U\)\s+(increased|decreased|rose|fell)\s+([+−-]?\d+(?:\.\d+)?)\s+percent\b/i,
    "Official CPI monthly value is missing",
  );
  const headlineAnnual = capture(
    body,
    /\bOver the last 12 months,\s+the all items index\s+(increased|decreased|rose|fell)\s+([+−-]?\d+(?:\.\d+)?)\s+percent\b/i,
    "Official CPI annual value is missing",
  );
  const coreMonthly = capture(
    body,
    /\bThe index for all items less food and energy\s+(increased|decreased|rose|fell)\s+([+−-]?\d+(?:\.\d+)?)\s+percent\b/i,
    "Official core CPI monthly value is missing",
  );
  const coreAnnual = capture(
    body,
    /\bThe all items less food and energy index\s+(increased|decreased|rose|fell)\s+([+−-]?\d+(?:\.\d+)?)\s+percent over the year\b/i,
...
function parseBeaPce(html, retrievedAt, sourceUrl) {
  ...
  const monthly = capture(
    article.text,
    /\bFrom the preceding month,\s+the PCE price index for\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+(increased|decreased)\s+([+−-]?\d+(?:\.\d+)?)\s+percent\s*\.\s*Excluding food and energy,\s+the PCE price index\s+(increased|decreased)\s+([+−-]?\d+(?:\.\d+)?)\s+percent\b/i,
    "Official PCE monthly values are missing",
  );
  const annual = capture(
    article.text,
    /\bFrom the same month one year ago,\s+the PCE price index for\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+(increased|decreased)\s+([+−-]?\d+(?:\.\d+)?)\s+percent\s*\.\s*Excluding food and energy,\s+the PCE price index\s+(increased|decreased)\s+([+−-]?\d+(?:\.\d+)?)\s+percent from one year ago\b/i,
...
function parseBeaGdp(html, retrievedAt, sourceUrl) {
  const article = beaReleaseArticle(
    html,
    sourceUrl,
    /^\/news\/20\d{2}\/gdp-(?:advance|second|third)-estimate-\d(?:st|nd|rd|th)-quarter(?:-[a-z0-9-]+)?$/i,
    /^GDP \((?:Advance|Second|Third) Estimate\),\s+(?:1st|2nd|3rd|4th) Quarter 20\d{2}$/i,
  );
  const quarter = capture(article.heading, /^GDP \((?:Advance|Second|Third) Estimate\),\s+(1st|2nd|3rd|4th) Quarter (20\d{2})$/i, "GDP release quarter is missing");
  const releasePeriod = `Q${quarter[1][0]} ${quarter[2]}`;
  const publishedAt = easternReleaseTimestamp(article.html);
  const match = capture(article.text, /\breal gross domestic product\s*\(GDP\)\s+(increased|decreased)\s+at an annual rate of\s+([+−-]?\d+(?:\.\d+)?)\s+percent\b/i, "Official headline GDP value is missing");
...
function beaReleaseUrl(indexHtml, indicator) {
  ...
  const textPattern = isPce
    ? /^Personal Income and Outlays,\s+(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+20\d{2}$/i
    : /^GDP \((?:Advance|Second|Third) Estimate\),\s+(?:1st|2nd|3rd|4th) Quarter 20\d{2}(?:\b.*)?$/i;
