...
export function normalizeDestinationCta(input = {}) {
  ...
  return {
    ...
    threadId: platform === "telegram" ? null : (input.threadId == null || input.threadId === "" ? null : Number(input.threadId)),
    ...
    topicName: platform === "telegram" ? "" : String(input.topicName || input.channelName || ""),
    ctaEnabled,
    ctaText,
    ctaUrl,
  };
...
export async function loadDestinationCtaRegistry(repository) {
  ...
  // A group-level value saved by the new UI wins over any legacy Topic-level value.
  entries.filter(([key]) => /^telegram:[^:]+$/.test(key) || key.startsWith("discord:")).forEach(add);
...
export async function saveDestinationCtaRegistry(repository, configs = []) {
  ...
  for (const input of Array.isArray(configs) ? configs : []) {
    ...
    const key = destinationCtaKey(config);
    if (!key) throw new Error("CTA 配置缺少有效的频道或群组。");
    registry[key] = config;
  }
