...
export const PUBLISHER_HEARTBEAT_STALE_MS = 15 * 60_000;

export function buildBotAccessScopes(bots = []) {
  const scopes = new Map();
  for (const bot of Array.isArray(bots) ? bots : []) {
    for (const group of Array.isArray(bot?.groups) ? bot.groups : []) {
      const chatId = String(group?.chatId || group?.id || "").trim();
      if (!chatId) continue;
      const title = group?.title || group?.name || chatId;
      const current = scopes.get(chatId) || {
        chatId,
        title,
        isForum: false,
        bound: false,
      ...
      if ((!current.title || current.title === chatId) && title !== chatId) current.title = title;
      current.isForum = current.isForum || group?.isForum === true;
      current.bound = current.bound || group?.bound === true;
      if (bot?.name && !current.components.includes(bot.name)) current.components.push(bot.name);
      scopes.set(chatId, current);
    }
  }
  return [...scopes.values()].sort((a, b) => (
    String(a.title).localeCompare(String(b.title), "zh-CN") || a.chatId.localeCompare(b.chatId)
  ));
}
...
export function getBotOperationalStatus({ bot, group, generatedAt, now = Date.now() }) {
  ...
  if (!group) {
    return { label: "API 可用", tone: "green", detail: "后台身份已核验；群权限请查看当前可访问范围" };
  }
