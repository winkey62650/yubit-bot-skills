      ...
      const mergedTopics = [
        ...configuredTopics.map((topic) => {
          ...
          return {
            ...
            canSendMessages: dialog?.canSendMessages === true && live?.canSendMessages === true
          };
        }),
        ...[...liveTopics.values()]
          ...
          .map((topic) => ({
            ...
            canSendMessages: dialog?.canSendMessages === true && topic?.canSendMessages === true
          }))
      ];
      return {
        ...(configured || {}),
        ...
        publishUnavailableReason: dialog?.publishUnavailableReason || (dialog?.canSendMessages === true ? "" : "message_permission_unavailable"),
        topics: mergedTopics,
...
export function filterTelegramComposerTargets(groups = [], search = "") {
  const query = String(search).trim().toLocaleLowerCase();
  if (!query) return groups;

  return groups.flatMap((group) => {
    if (String(group?.title || "").toLocaleLowerCase().includes(query)) return [group];

    const matchingOptions = (Array.isArray(group?.options) ? group.options : [])
      .filter((option) => String(option?.label || "").toLocaleLowerCase().includes(query));
    return matchingOptions.length > 0 ? [{ ...group, options: matchingOptions }] : [];
  });
}
...
export function accountCanSendToTarget(dialogs = [], target = "") {
  try {
    assertAccountCanSendToTargets(dialogs, [target]);
    return true;
  } catch {
    return false;
  }
