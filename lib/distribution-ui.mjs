...
export function buildPublisherStatusChecks(publisher = {}) {
  ...
  const sessionReady = publisher.authorized === true && Boolean(publisher.lastSeenAt || publisher.lastVerifiedAt);
  const approvedTargets = Array.isArray(publisher.approvedTargetIds) ? publisher.approvedTargetIds : [];
  const routingReady = publisher.routingReady !== false && approvedTargets.length > 0;
  ...
  return [
    {
      ...
      ok: identityReady,
      detail: username ? `当前账号：${username}` : "未检测到 Telegram 发布账号"
    },
    {
      key: "bridge",
      label: "本机发布桥",
...
export function buildDistributionTargetOptions(groups = []) {
  return (Array.isArray(groups) ? groups : []).flatMap((group) => {
    ...
    return orderedDistributionTopics(group?.topics || [])
...
export function buildDistributionSourceOptions(groups = []) {
  return (Array.isArray(groups) ? groups : []).flatMap((group) => {
    ...
    const wholeGroup = {
      key: `${group.chatId}:0`,
      label: `${groupName} / 整群`,
      source: { chatId: String(group.chatId), threadId: "", groupName, topicName: "整群" }
    };
    const topics = orderedDistributionTopics(group?.topics || [])
      .filter((topic) => Number(topic.threadId || topic.topicId) > 0)
      .map((topic) => ({
