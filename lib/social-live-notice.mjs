...
export function renderSocialLiveNotice(source, live, _now) {
  ...
  return [`${d.title} · ${date}`, d.startedAt ? `Live now. Started: ${d.startedAt.replace('T', ' ').slice(0, 16)} UTC` : 'Live now.', `Click here to watch live 👉 ${d.url}`, '@everyone'].join('\n\n');
}

export function renderDiscordLiveNotice(source, live, _now, { historical = false } = {}) {
  const d = details(source, live);
  return {
    content: [`**${d.title} · ${d.startedAt ? formatDate(new Date(d.startedAt)) : 'Live now'}**`, historical ? 'Livestream recording available.' : d.startedAt ? `Live now. Started: ${d.startedAt.replace('T', ' ').slice(0, 16)} UTC` : 'Live now.', `Click here to watch ${historical ? 'the stream' : 'live'} 👉 ${d.url}`, '@everyone'].join('\n\n'),
    allowedMentions: { parse: [] },
    embeds: [{
      title: d.title,
      url: d.url,
      color: 0xED4245,
      description: `[${historical ? 'Watch the stream' : 'Watch live'}](${d.url})`,
