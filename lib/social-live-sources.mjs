...
async function youtubeFeedIds(channelId, { fetchImpl, signal }) {
  ...
  const channelIds = [...text.matchAll(/<yt:channelId>\s*([^<]+)\s*<\/yt:channelId>/g)].map(match => match[1].trim());
  if (!channelIds.length || channelIds.some(id => id !== channelId)) throw new Error('YouTube Feed 归属校验失败');
...
export async function detectSocialLive(source, { env = process.env, fetchImpl = fetch, signal, discovery } = {}) {
  ...
  let feedOk = false, searchOk = false, warning = '';
  if (discovery) {
    try {
      for (const id of await youtubeFeedIds(channelId, options)) ids.add(id);
      feedOk = true;
    } catch (error) {
      if (!discovery.search) throw error;
      warning = '频道动态暂不可用，本轮使用官方搜索补查；快速检测可能延迟。';
    }
  }
  let pageToken;
  ...
  if (pageToken) throw new Error('YouTube 直播列表超过单次检查上限');
   searchOk = true;
     ...
     if (!feedOk) throw error;
     warning = '完整搜索补查失败；已继续核验频道动态和已知直播，未出现在动态中的直播可能延迟发现。';
   }
  }
  const broadcasts = [];
  ...
  return { provider: status.provider, broadcasts, scheduledCount, trackedVideoIds, ...(discovery ? { strategy: feedOk ? searchOk ? 'youtube-feed-search-api' : 'youtube-feed-api' : 'youtube-search-api', ...(warning ? { warning } : {}) } : {}) };
}
