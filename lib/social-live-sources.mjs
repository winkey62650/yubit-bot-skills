...
export async function detectSocialLive(source, { env = process.env, fetchImpl = fetch, signal } = {}) {
  ...
  for (let i = 0; i < values.length; i += 50) {
    const details = await get('videos', { part: 'snippet,liveStreamingDetails', id: values.slice(i, i + 50).join(',') });
    ...
    for (const item of details.items) {
      ...
      broadcasts.push({ platform: 'YouTube', format: 'Live', ownerId: channelId, id: item.id, title: String(item.snippet.title).slice(0, 300), url: `https://www.youtube.com/watch?v=${item.id}`, startedAt: live.actualStartTime, state: 'live' });
    }
  }
  return { provider: status.provider, broadcasts, scheduledCount: 0 };
}
