...
export function buildWeeklyCalendarCommunityDocument(article, { articleUrl } = {}) {
  ...
  return {
    ...
    title: `📌 Weekly Market Calendar — ${article.slug}`,
    articleUrl: url,
    nodes: [
      { type: "heading", text: `📌 Weekly Market Calendar — ${article.slug}`, level: 1 },
...
function reactionRecords(reaction) {
  ...
  return REACTION_SYMBOLS.flatMap((symbol) => {
    ...
    return record && Number.isFinite(changePercent) ? [{ symbol, changePercent, ...record }] : [];
  });
}
...
function reactionWindow(reaction, event, document) {
  ...
  return { start, end, providers: unique(records.map((record) => record.source ?? record.provider)) };
}
...
function releaseSourceCandidates(event, reaction, manifest) {
  ...
  return dedupeSources([
    ...
    ...records.map((record) => ({ id: record.provider, label: record.source ?? record.provider, url: record.url ?? record.sourceUrl, type: "market-observation" })),
  ]);
}
...
function releaseCommunityNodes(article, { includeLink, articleUrl } = {}) {
  ...
  const factLines = [
    `1. Actual: ${text(facts.actual)}${facts.previous !== undefined ? ` | Previous: ${text(facts.previous)}` : ""}.`,
  ];
  if (facts.forecast !== undefined) {
    factLines.push(`2. Named consensus: ${text(facts.forecast)}${facts.surprise !== undefined ? ` | Difference: ${text(facts.surprise)}` : ""}.`);
  }
  factLines.push(`${factLines.length + 1}. ${text(article.marketConfirmation.summary)}`);
  const nodes = [
    ...
    ...factLines.map((line) => ({ type: "paragraph", text: line })),
