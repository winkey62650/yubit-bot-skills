...
const PROVIDER_NAMES = Object.freeze({
  ...
  "dxy-yahoo-finance": "Yahoo Finance",
});
...
function isReactionProviderEntry(provider) {
  if (!provider || typeof provider !== "object") return false;
  const status = text(provider.status).toLowerCase();
  return ["ok", "degraded", "timeout", "error"].includes(status)
    || Object.hasOwn(provider, "checkedAt")
    || Object.hasOwn(provider, "lastSuccessAt")
    || Object.hasOwn(provider, "freshnessSeconds");
}

function sameHttpsOrigin(left, right) {
  try {
...
function trustedProviderLookup(sourceManifest) {
  const providers = (Array.isArray(sourceManifest) ? sourceManifest : []).filter(isReactionProviderEntry);
  const byAlias = new Map();
  for (const [index, provider] of providers.entries()) {
    const manifestId = text(provider.id ?? provider.sourceId);
    const manifestName = canonicalProviderName(provider.name ?? provider.label ?? provider.provider ?? manifestId);
    const candidate = {
      index,
      id: manifestId || providerKey(manifestName),
      name: manifestName,
...
function reactionRecords(reaction, sourceManifest) {
  ...
  return REACTION_SYMBOLS.flatMap((symbol) => {
    ...
    const sourceReference = text(record?.source);
    const providerReference = text(record?.provider);
    const references = unique([sourceReference, providerReference]);
    const matches = references.map(providerManifest.find);
    const manifestProvider = matches[0] ?? null;
    const identitiesAgree = matches.every((match) => match && match.index === manifestProvider?.index);
    const trustedProvider = manifestProvider?.usable && identitiesAgree ? manifestProvider : null;
    const providerName = trustedProvider?.name || canonicalProviderName(sourceReference || providerReference);
    const suppliedSourceUrl = text(record?.sourceUrl ?? record?.url);
    const directSourceUrl = absoluteHttpsUrl(suppliedSourceUrl);
    const manifestAllowsRecord = !providerManifest.present || (references.length > 0 && Boolean(trustedProvider));
    const directUrlAllowed = !suppliedSourceUrl
      || !providerManifest.present
      || (Boolean(directSourceUrl) && sameHttpsOrigin(directSourceUrl, trustedProvider?.sourceUrl));
    const sourceUrl = manifestAllowsRecord && directUrlAllowed
      ? directSourceUrl || trustedProvider?.sourceUrl
      : "";
    const providerId = trustedProvider?.id || text(record?.provider);
...
export function buildSecondaryDataUpdateCommunityDocument(input = {}) {
  ...
  const sourceManifest = input.sourceManifest ?? input.reaction?.sources ?? [];
