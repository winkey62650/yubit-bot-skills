...
function assertFieldProvenanceMatches(field, adoptedValue, fieldName) {
  ...
  for (const key of ["value", "rawValue"]) {
    ...
    if (!factValuesMatch(adoptedValue, field[key])) {
...
export function buildWeeklyCalendarArticle({ document, rankedEvents, sourceManifest, marketSetup } = {}) {
  ...
  for (const event of Array.isArray(rankedEvents) ? rankedEvents : []) {
    ...
    const semanticIdentity = [
      ...
      normalizeIdentityPart(event?.jurisdiction ?? event?.country) || "global",
      timestamp,
    ].join("|");
...
function reactionRecords(reaction) {
  ...
  return REACTION_SYMBOLS.flatMap((symbol) => {
    ...
    return record && changePercent !== null && providerName && sourceUrl
      ? [{ ...record, symbol, changePercent, providerName, sourceUrl, ...(beforePriceAt ? { beforePriceAt, observedAt } : {}) }]
      : [];
  });
}
...
function releaseSlug(event) {
  ...
  const supplied = hasSuppliedSlug ? text(event.slug) : text(event?.indicator) || text(event?.id);
  const slug = canonicalReleaseSlug(supplied);
  if (hasSuppliedSlug) {
    const indicator = canonicalReleaseSlug(text(event?.indicator ?? event?.id));
    const jurisdiction = text(event?.jurisdiction ?? event?.country).toLowerCase().replace(/[^a-z0-9]+/gu, "-").replace(/^-|-$/gu, "");
    const expected = jurisdiction && jurisdiction !== "global" && !indicator.startsWith(`${jurisdiction}-`)
...
function dataArticleModel({ document, event, reaction, tierDecision, sourceManifest, requireTierOne }) {
  ...
  const candidates = reactionRecords(reaction);
