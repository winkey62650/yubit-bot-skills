...
function isInternalSentenceBoundary(sentence, index) {
  ...
  return /^(?:["')\]]*)?(?:\s+\S|(?=[A-Z0-9]))/u.test(suffix);
}
...
const EMBEDDED_URL = /(?:https?:\/\/|www\.)/iu;
...
function weeklyPublicationIdentity(document) {
  ...
  const suppliedSlug = hasSuppliedSlug
    ? canonicalWeek(document.slug)
    : hasSuppliedWeek
      ? canonicalWeek(document.week)
      : null;
  if (suppliedSlug !== null && suppliedSlug !== slug) {
    throw new TypeError("Weekly Calendar supplied slug must match its date range identity.");
  }
...
export function buildWeeklyCalendarArticle({ document, rankedEvents, sourceManifest, marketSetup } = {}) {
  ...
  for (const event of Array.isArray(rankedEvents) ? rankedEvents : []) {
    ...
    const canonicalId = text(event?.id ?? event?.sourceId).toLowerCase();
    const identityKey = canonicalId || [eventIndicator(event), text(event?.jurisdiction ?? event?.country).toLowerCase(), timestamp].join("|");
    const fingerprint = stableFingerprint({ ...event, id: canonicalId || undefined, scheduledAt: timestamp });
    const existing = uniqueEvents.get(identityKey);
    if (existing && existing.fingerprint !== fingerprint) {
      throw new Error(`Weekly Calendar duplicate event identity conflict: ${identityKey}.`);
    }
    if (!existing) uniqueEvents.set(identityKey, { event, fingerprint });
  }
  const events = [...uniqueEvents.values()].map(({ event }) => event)
...
function reactionBounds(reaction, event, document) {
  ...
  const starts = REACTION_SYMBOLS.map((symbol) => source?.[symbol]?.beforePriceAt).filter((value) => text(value));
  const ends = REACTION_SYMBOLS.map((symbol) => source?.[symbol]?.observedAt).filter((value) => text(value));
...
function reactionRecords(reaction, bounds) {
  const source = reaction?.prices ?? reaction?.data ?? reaction ?? {};
  return REACTION_SYMBOLS.flatMap((symbol) => {
    ...
    const providerName = canonicalProviderName(record?.source ?? record?.provider);
    const sourceUrl = absoluteHttpsUrl(record?.sourceUrl ?? record?.url);
    const hasBefore = Boolean(text(record?.beforePriceAt));
    const hasObserved = Boolean(text(record?.observedAt));
    let timestampsAreValid = !hasBefore && !hasObserved;
    if (hasBefore && hasObserved) {
      try {
        const beforePriceAt = iso(record.beforePriceAt, "Data Update observation start");
        const observedAt = iso(record.observedAt, "Data Update observation end");
        timestampsAreValid = beforePriceAt < observedAt
          && (!bounds || (beforePriceAt >= bounds.start && observedAt <= bounds.end));
      } catch {
        timestampsAreValid = false;
      }
    }
    return record && changePercent !== null && providerName && sourceUrl && timestampsAreValid
      ? [{ symbol, ...record, changePercent, providerName, sourceUrl }]
      : [];
  });
}
...
function releaseSlug(event) {
  const supplied = event && Object.hasOwn(event, "slug")
    ? text(event.slug)
    : text(event?.indicator) || text(event?.id);
...
function factsRecord(event) {
  ...
  const facts = {
    ...
    provenance: {
      ...
      ...(values.previous !== undefined && values.previous !== null && text(values.previous)
        ? { previous: fieldProvenance(provenance.previous ?? (event?.values?.previous && typeof event.values.previous === "object" ? event.values.previous : null)) }
        : {}),
    },
  };
  if (values.forecast !== undefined && values.forecast !== null && text(values.forecast)) {
    ...
    facts.provenance.forecast = fieldProvenance(provenance.forecast ?? (event?.values?.forecast && typeof event.values.forecast === "object" ? event.values.forecast : null));
    if (comparison) {
...
function dataArticleModel({ document, event, reaction, tierDecision, sourceManifest, requireTierOne }) {
  ...
  const bounds = reactionBounds(reaction, event, document);
  const records = reactionRecords(reaction, bounds);
