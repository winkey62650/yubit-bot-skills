...
const COUNTRY_FINANCIAL_MODIFIERS = new Set([
  "cpi", "pce", "gdp", "treasury", "dollar", "labour", "labor", "inflation", "jobs", "employment",
  "rate", "rates", "yield", "yields", "economy", "economic", "market", "markets", "policy", "government",
  "regulation", "regulator", "etf",
]);
...
function releaseComparison(actual, forecast) {
  const actualMeasure = measuredValue(actual);
  const forecastMeasure = measuredValue(forecast);
  if (!actualMeasure || !forecastMeasure || actualMeasure.dimension !== forecastMeasure.dimension) return null;
  const baseDifference = actualMeasure.baseValue - forecastMeasure.baseValue;
  const difference = baseDifference / actualMeasure.scale;
  const suffix = actualMeasure.dimension === "percent" ? "pp" : actualMeasure.unit;
  return {
    baseDifference,
...
function reactionRecords(reaction) {
  ...
  return REACTION_SYMBOLS.flatMap((symbol) => {
    ...
    return record && Number.isFinite(changePercent) && providerName && sourceUrl
...
export function buildDataUpdateCommunityDocument(article, { articleUrl } = {}) {
  ...
  return {
    ...
    articleUrl: url,
    nodes: fitTelegramCommunityNodes(releaseCommunityNodes(article, { includeLink: true, articleUrl: url })),
  };
}
...
export function buildSecondaryDataUpdateCommunityDocument(input = {}) {
  ...
  return {
    ...
    generatedAt: article.publishedAt,
    title: `📌 Data Update — ${article.facts.title}`,
    nodes: fitTelegramCommunityNodes(releaseCommunityNodes(article, { includeLink: false })),
  };
}
