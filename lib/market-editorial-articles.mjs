...
function oneSentence(value, fallback) {
  ...
  const boundaryProbe = sentence.replace(/\b(?:[A-Za-z]\.){2,}/gu, (abbreviation, offset, input) => {
    const nextWord = input.slice(offset + abbreviation.length).match(/^\s*([A-Za-z][A-Za-z0-9-]*)/u)?.[1] ?? "";
    const continuesSentence = /^[a-z]/u.test(nextWord) || /^[A-Z0-9]{2,}$/u.test(nextWord);
