...
export async function updateJson(pathname, updater, fallback) {
  const previous = updateQueues.get(pathname) || Promise.resolve();
  const run = previous.catch(() => {}).then(async () => {
    const current = await readJson(pathname, fallback);
    const next = await updater(current);
    await writeJson(pathname, next);
    return readJson(pathname, next);
  });

  updateQueues.set(pathname, run);
  try {
    return await run;
  } finally {
    if (updateQueues.get(pathname) === run) updateQueues.delete(pathname);
  }
}
