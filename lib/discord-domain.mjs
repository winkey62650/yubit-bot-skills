...
export function buildDiscordDemoSnapshot({ guild, channels = [], messagesByChannel = {}, capturedAt } = {}) {
  const guildId = String(guild?.id || "").trim();
  const guildName = String(guild?.name || "").trim();
  if (!guildId || guildName !== DISCORD_DEMO_GUILD_NAME) {
    throw new Error(`Discord Demo Server must be named ${DISCORD_DEMO_GUILD_NAME}.`);
  }
