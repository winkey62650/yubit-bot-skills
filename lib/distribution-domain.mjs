...
export function buildStandardProductionDistributionRules(groups = [], {
  ...
  const automations = STANDARD_AUTOMATION_SPECS.map((spec) => {
    const desired = {
      ...
      targets: [
        standardEndpoint(sourceGroup, sourceTopics.get(spec.topicNumber), spec.topicNumber),
        standardEndpoint(targetGroup, targetTopics.get(spec.topicNumber), spec.topicNumber),
      ],
      enabled: false,
      status: "ready",
    };
