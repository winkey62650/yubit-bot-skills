...
export function migrateMarketContentRules(rules = [], nowValue = new Date()) {
  ...
  for (const release of migrated.filter((rule) => rule.kind === "automation" && rule.contentType === "data-release-updates")) {
    ...
    const parentId = release.importedFrom && calendarIds.has(release.importedFrom)
      ...
      : derivedCalendar?.id;
    if (!parentId) {
      if (!release.importedFrom) unlinkedReleases.push(release.id);
      continue;
    }
  ...
  if (calendars.length && unlinkedReleases.length) {
    throw new Error(`Market content migration conflict: unlinked data-release-updates rules: ${unlinkedReleases.join(", ")}`);
  }
