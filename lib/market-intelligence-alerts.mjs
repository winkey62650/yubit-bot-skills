...
function snapshotRows(rows, focus, limit = 4) {
  const selected = rows.slice(0, limit);
  if (focus?.side === rows[0]?.side && !selected.some((row) => row === focus)) {
  ...
  return selected.map((row) => ({
    ...
    visibleNotional: formatFixed(row.cents, 2),
    visibleNotionalLabel: `$${formatMoney(row.cents)}`,
    isFocus: row === focus,
  }));
}

function buildEvidenceSnapshot({ source, markPrice, bids, asks, focus }) {
  return {
    provider: source.provider,
    sourceTimestamp: source.sourceTimestamp,
    markPrice: String(markPrice),
    rows: [
      ...snapshotRows(asks, focus).reverse(),
      ...snapshotRows(bids, focus),
    ],
  };
}
