...
const source={id:'ajc',name:'Average Joe Crypto X',agent:'Average Joe Crypto',platform:'X',accountUrl:'https://x.com/AvrgJoeCrypto',status:'已启用',postMonitoring:true,postSyncMode:'verified',targets:[{platform:'discord',guildId:'g',channelId:'updates'}]};
const post={platform:'X',ownerId:'avrgjoecrypto',id:'2099320669901119724',title:'New post',url:'https://x.com/AvrgJoeCrypto/status/2099320669901119724',publishedAt:'2026-09-14T02:01:00Z'};
test('real repository persists one Discord receipt across restart; old notices cannot be replayed',async()=>{
 ...
 try{
  let repository=new JsonDistributionRepository();let sent=0;const options={loadSources:async()=>[source],fetchPosts:async()=>({ownerId:post.ownerId,posts:[post]}),deliver:args=>deliverSocialPost({...args,discordSender:async(channel,payload)=>{sent++;assert.equal(channel,'updates');assert.match(payload.content,/NEW POST/);return {id:'123'}}})};
