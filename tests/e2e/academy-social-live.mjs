...
try {
 ...
 await page.locator('article').filter({hasText:source.name}).getByRole('button',{name:'编辑',exact:true}).click();
 assert.equal(await page.getByRole('checkbox',{name:'同时监控直播开播',exact:false}).isChecked(),true);
 ...
 assert.equal(report.errors.length,0);report.checks={legacyDefaultOff:true,persistAndReload:true,targetsPreserved:true,missingCredentials422:true,readOnlyLivePreview:true,anonymous401:true,manualPublisher403:true};report.outcome='passed';
