...
test("whale preview exposes the approved poster and operating copy before publishing", () => {
  ...
  assert.match(preview.headline, /LIQUIDITY ALERT/);
  assert.doesNotMatch(preview.headline, /WHALE|SMART MONEY/i);
  assert.match(preview.caption, /FACT/);
  assert.match(preview.caption, /Visible size/);
  assert.match(preview.caption, /Depth share/);
  assert.match(preview.caption, /INTERPRETATION/);
  assert.match(preview.caption, /WATCH NEXT/);
  assert.match(preview.caption, /SOURCE/);
