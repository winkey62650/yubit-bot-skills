...
export default function LoginPage() {
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || t("login.failed"));
      router.replace(result.home || "/group-config");
      router.refresh();
    } catch (loginError) {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#f4f8f6] px-5 py-12">
      <section className="relative w-full max-w-md rounded-2xl border border-ops-line bg-white p-7 shadow-[0_22px_70px_rgba(16,24,21,0.10)] md:p-9">
        ...
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-ops-muted">{t("login.subtitle")}</p>
          </div>
        </div>
        ...
        <p className="mt-6 text-center text-xs leading-5 text-ops-muted">{t("login.notice")}</p>
      </section>
    </main>
  );
}
