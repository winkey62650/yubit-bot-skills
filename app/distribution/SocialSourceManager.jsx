...
export default function SocialSourceManager({ packages, targetOptions = [], publisherName = "当前发布身份", busy, onPersist, onNotice }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/social-posts', { signal: controller.signal }).then(async response => {
      ...
      if (!response.ok || !data.ok) throw new Error(data.error || '普通更新状态读取失败');
      setPostSources(data.sources || []); setPostStatusError('');
    }).catch(error => { if (!controller.signal.aborted) setPostStatusError(error.message); });
