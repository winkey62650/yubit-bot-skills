    ...
    <div className="grid gap-5 p-5 xl:grid-cols-[minmax(320px,.82fr)_minmax(0,1.18fr)]">
      ...
      <div className="overflow-hidden rounded-lg border border-ops-line">
        ...
        <div className="divide-y divide-ops-line">{packages.length ? packages.map((item) => {
          const sourceLabel = item.feedUrl || item.platform === "YouTube" ? "官方 Feed" : item.platform === "X" ? "公开时间线" : "有限检测";
          const usable = sourceLabel !== "有限检测";
          return <article className="p-4" key={item.id}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong>{item.name}</strong><StatusPill tone={item.status === "已启用" ? "green" : "amber"}>{item.status}</StatusPill><StatusPill tone={usable ? "green" : "amber"}>{sourceLabel}</StatusPill></div><p className="mt-2 text-sm text-ops-muted">{item.agent} · {item.platform} · 每小时</p><p className="mt-1 truncate text-xs text-ops-muted">{item.feedUrl || item.accountUrl || "未填写地址"}</p></div><div className="flex shrink-0 flex-wrap gap-2"><SourceButton onClick={() => { setForm(item); setPreview(null); }}>编辑</SourceButton><SourceButton onClick={() => updatePackages(packages.map((source) => source.id === item.id ? { ...source, status: source.status === "已启用" ? "已暂停" : "已启用" } : source), item.status === "已启用" ? "代理来源已暂停。" : "代理来源已启用。")}>{item.status === "已启用" ? "暂停" : "启用"}</SourceButton><SourceButton danger onClick={() => window.confirm("确认删除这条代理来源？") && updatePackages(packages.filter((source) => source.id !== item.id), "代理来源已删除。")}>删除</SourceButton></div></div></article>;
