...
function TraderManagement({ data, busy, traderForm, setTraderForm, accountForm, setAccountForm, verifySymbol, setVerifySymbol, onPost, onCopy }) {
  ...
  async function saveAccountForm(event) {
    ...
    const result = await onPost({ action: "save-account", account: { ...accountForm, id: accountForm.id || undefined } }, "只读账户已加密保存");
  ...
  return <div className="grid items-start gap-5 xl:grid-cols-2">
    ...
    <div className="grid gap-5">
      <Card className="p-5 md:p-6"><SectionHead title={accountForm.id ? "编辑 YUBIT 只读账户" : "新增 YUBIT 只读账户"} desc="一个账户可关联多个 Trader；Trader 提交订单号时系统会从关联账户中核验。" /><div className="mt-4 rounded-lg border border-[#e7c883] bg-[#fff9e9] p-4 text-sm leading-6 text-[#6f551d]"><strong>安全要求：</strong>创建 API 时必须关闭交易、转账和提现权限，仅保留查询权限。凭证只在服务端加密保存，页面不会回显 API Secret。</div><form className="mt-5 grid gap-4" onSubmit={saveAccountForm}><Field label="账户名称"><input className={inputClass} onChange={(event) => setAccountForm({ ...accountForm, label: event.target.value })} placeholder="例如 Trader A 主账户" required value={accountForm.label} /></Field><Field label="API Key"><input autoComplete="off" className={inputClass} onChange={(event) => setAccountForm({ ...accountForm, apiKey: event.target.value })} placeholder={accountForm.id ? "留空表示不更换" : "只读 API Key"} required={!accountForm.id} type="password" value={accountForm.apiKey} /></Field><Field label="API Secret"><input autoComplete="new-password" className={inputClass} onChange={(event) => setAccountForm({ ...accountForm, apiSecret: event.target.value })} placeholder={accountForm.id ? "留空表示不更换" : "只读 API Secret"} required={!accountForm.id} type="password" value={accountForm.apiSecret} /></Field><fieldset className="grid gap-2"><legend className="text-sm font-bold text-ops-muted">关联 Trader</legend>{!data.traders.length ? <p className="text-sm text-ops-muted">请先新增 Trader。</p> : data.traders.map((trader) => <label className="flex items-center gap-2 rounded-lg border border-ops-line p-3 text-sm font-bold" key={trader.id}><input checked={accountForm.traderIds.includes(trader.id)} onChange={() => setAccountForm({ ...accountForm, traderIds: toggleValue(accountForm.traderIds, trader.id) })} type="checkbox" />{trader.displayName}</label>)}</fieldset>{accountForm.id ? <Field label="账户状态"><select className={inputClass} onChange={(event) => setAccountForm({ ...accountForm, status: event.target.value })} value={accountForm.status}><option value="pending">待验证</option><option value="verified">已验证</option><option value="invalid">验证失败</option><option value="disabled">已停用</option></select></Field> : null}<FormActions busy={busy === "save-account"} editing={Boolean(accountForm.id)} onCancel={() => setAccountForm(emptyAccount)} /></form></Card>
      <Card className="p-5 md:p-6"><SectionHead title="只读账户" desc={`${data.metrics.verifiedAccounts || 0} 个验证通过`} />{!data.accounts.length ? <Empty text="暂无只读账户。账户保存后请立即验证查询权限。" /> : <div className="mt-4 grid gap-3">{data.accounts.map((account) => <div className="rounded-lg border border-ops-line p-4" key={account.id}><div className="flex items-start justify-between gap-3"><div><strong>{account.label}</strong><button className="mt-1 block break-all text-left font-mono text-xs text-ops-accent" onClick={() => onCopy(account.apiKeyMasked, "脱敏 API Key")} type="button">{account.apiKeyMasked || "Key 已加密"}</button><p className="mt-1 text-xs text-ops-muted">关联 {account.traderIds?.length || 0} 名 Trader · {account.lastVerifiedAt ? `上次验证 ${formatDate(account.lastVerifiedAt)}` : "尚未验证"}</p></div><StatusPill tone={account.status === "verified" ? "green" : "amber"}>{accountStatus(account.status)}</StatusPill></div>{account.lastErrorCode ? <p className="mt-2 break-all text-xs text-[#9d3128]">{account.lastErrorCode}</p> : null}<div className="mt-3 flex flex-wrap gap-2"><SmallButton onClick={() => editAccount(account)}>编辑</SmallButton><input aria-label="验证币种" className={`${inputClass} min-h-9 w-28`} onChange={(event) => setVerifySymbol(event.target.value.toUpperCase())} value={verifySymbol} /><SmallButton disabled={busy === "verify-account"} onClick={() => onPost({ action: "verify-account", accountId: account.id, symbol: verifySymbol }, "YUBIT 只读查询权限验证成功")}>{busy === "verify-account" ? "验证中…" : "验证只读权限"}</SmallButton></div></div>)}</div>}</Card>
    </div>
  </div>;
}
...
function SystemHealth({ data, busy, onPost }) {
  ...
  return <div className="grid gap-5">
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><HealthCard title="数据库" ok={health.database?.ok} value={health.database?.ok ? "连接正常" : "连接异常"} detail={health.database?.driver || health.database?.errorCode || "未配置"} /><HealthCard title="SpeakerBot" ok={health.speakerBot?.ok} value={health.speakerBot?.username ? `@${health.speakerBot.username}` : health.speakerBot?.configured ? "已配置" : "未配置"} detail={health.speakerBot?.webhookConfigured ? `Webhook 正常 · 待处理 ${health.speakerBot.pendingUpdates || 0}` : health.speakerBot?.errorCode || "Webhook 未配置"} /><HealthCard title="订单追踪" ok={!health.scheduler?.errorCode || health.scheduler?.errorCode === "NOT_RUN_YET"} value={health.scheduler?.lastRunAt ? "调度已运行" : "等待首次运行"} detail={health.scheduler?.lastRunAt ? `上次 ${formatDate(health.scheduler.lastRunAt)}` : health.scheduler?.errorCode || "尚无记录"} /><HealthCard title="上线准备" ok={Boolean(data.metrics.enabledTraders && data.metrics.verifiedAccounts && data.metrics.enabledDestinations)} value={`${data.metrics.enabledTraders || 0} Trader · ${data.metrics.verifiedAccounts || 0} 账户`} detail={`${data.metrics.enabledDestinations || 0} 个发布目标`} /></div>
    <Card className="p-5 md:p-6"><SectionHead title="SpeakerBot 接收入口" desc="Webhook 只接收 SpeakerBot 私聊消息，并校验 Telegram Secret Token。不会在页面展示 Bot Token 或 API Secret。" /><button className="mt-4 min-h-11 rounded-lg bg-ops-accent px-5 text-sm font-black text-white disabled:opacity-50" disabled={busy === "configure-webhook"} onClick={() => { if (window.confirm("确认用当前线上地址配置 SpeakerBot Webhook？")) onPost({ action: "configure-webhook" }, "SpeakerBot Webhook 配置成功"); }} type="button">{busy === "configure-webhook" ? "配置中…" : "配置 SpeakerBot Webhook"}</button></Card>
