...
export default function DiscordManualPage() {
  ...
  const [manualCtaText, setManualCtaText] = useState("");
  ...
  async function publish() {
    ...
    if (!manualContent.trim() && !manualCtaText.trim() && !manualCtaUrl.trim() && !manualImageUrl.trim() && !manualImageFile) return setError("请输入消息正文、CTA、图片链接或选择本地图片。");
    if (!isAllowedManualCtaUrl(manualCtaUrl)) return setError("CTA 链接必须使用 http 或 https。");
    ...
    try {
      ...
      formData.append("ctaText", manualCtaText);
      formData.append("ctaUrl", manualCtaUrl);
  ...
  return <ConsoleShell>
    <PageHeader title="Discord 手动信息发布" desc="Trader 可填写正文、CTA 与链接，也可附加图片，并发送到已通过实时检测的频道。" action={<button type="button" onClick={checkHealth} className="rounded-lg border border-ops-line bg-white px-4 py-2 text-sm font-black">重新实时检测</button>} />
      ...
      <Card className="p-6">
        ...
        <div className="mt-5 grid gap-4">
          ...
          <div className="rounded-xl border border-ops-line bg-ops-soft/50 p-4">
            <h3 className="text-sm font-black">CTA 与链接（可选）</h3>
            <p className="mt-1 text-xs text-ops-muted">填写后会追加到本次消息正文末尾；留空则不追加。</p>
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <Field label="CTA 文案"><input className={inputClass} value={manualCtaText} onChange={(event) => setManualCtaText(event.target.value)} placeholder="例如：立即加入 YUBIT" /></Field>
              <Field label="CTA 链接"><input type="url" className={inputClass} value={manualCtaUrl} onChange={(event) => setManualCtaUrl(event.target.value)} placeholder="https://…" /></Field>
            </div>
          </div>
        ...
        <button type="button" disabled={busy || manualChannelIds.length === 0 || (!manualContent.trim() && !manualCtaText.trim() && !manualCtaUrl.trim() && !manualImageUrl.trim() && !manualImageFile)} onClick={publish} className="mt-5 rounded-lg bg-ops-accent px-5 py-2.5 text-sm font-black text-white disabled:opacity-50">{busy ? "正在发布…" : `发送到 ${manualChannelIds.length} 个频道`}</button>
