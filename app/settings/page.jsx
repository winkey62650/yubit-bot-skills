...
export default function SettingsPage() {
  return (
    <ConsoleShell>
      <PageHeader title="系统设置" desc="配置程序健康检查、定时监控和 Lark 告警推送。" />
      <section className="mb-5 grid gap-0 overflow-hidden rounded-lg border border-ops-line bg-white shadow-ops md:grid-cols-4">
        <MetricBox label="检查频率" value="每 5 分钟" sub="可按业务低频调整" />
        <MetricBox label="监控程序" value="5" sub="控制台 / 新闻 / 信号 / 建群 / Lark" />
        <MetricBox label="告警渠道" value="Lark" sub="Webhook 推送" />
        <MetricBox label="当前状态" value="待配置" sub="填入 Webhook 后启用" />
      </section>

      <Card className="p-6">
        <h2 className="text-xl font-black">Lark 监控配置</h2>
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          <Field label="Lark Webhook"><input className={inputClass} placeholder="https://open.larksuite.com/open-apis/bot/v2/hook/..." /></Field>
          <Field label="检查频率"><select className={inputClass}><option>每 5 分钟</option><option>每 15 分钟</option><option>每 30 分钟</option><option>每 1 小时</option></select></Field>
          <Field label="告警模式"><select className={inputClass}><option>异常才推送</option><option>每次检查都推送</option><option>每日汇总</option></select></Field>
          <Field label="连续失败阈值"><select className={inputClass}><option>1 次失败立即告警</option><option>2 次连续失败告警</option><option>3 次连续失败告警</option></select></Field>
          <Field label="监控环境"><select className={inputClass}><option>本地控制台</option><option>生产环境</option><option>全部环境</option></select></Field>
          <Field label="状态"><select className={inputClass}><option>启用</option><option>暂停</option></select></Field>
        </div>
        <div className="mt-5 flex flex-wrap gap-3">
          <button className="rounded-lg bg-ops-accent px-5 py-3 text-sm font-black text-white">保存设置</button>
          <button className="rounded-lg border border-ops-accent px-5 py-3 text-sm font-black text-ops-accent">测试发送监控</button>
        </div>
      </Card>
